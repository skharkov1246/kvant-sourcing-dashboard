// Гейт доступа к сайту «Базовый проект ГОКа»: HTTP Basic Auth перед отдачей статики.
...
// BASIC_AUTH_PASS (секрет Cloudflare Pages проекта kvant-gok), общий для всех
...
// тем же методом»): внутри проектные материалы и контрагенты.
  ...
  async fetch(request, env) {
    ...
    if (!pass && !env.BASIC_AUTH_USERS) {
      return new Response(page(
        "Базовый проект ГОКа · КВАНТ",
        "<p>Доступ закрыт: не задан пароль сайта.</p>" +
        "<p>Владельцу: Cloudflare Pages → проект <code>kvant-gok</code> → Settings → " +
    ...
    if (!(await userOk(request, env, SITE))) {
      return new Response(page(
        "Базовый проект ГОКа · КВАНТ",
        "<p>Материалы базового проекта золото-медного ГОКа.</p>" +
        ...
        "Логин и пароль выдаёт владелец.</p>"),
        {
          ...
          headers: {
            ...
            "WWW-Authenticate": 'Basic realm="GOK KVANT", charset="UTF-8"',
            "Content-Type": "text/html; charset=utf-8",
            "Cache-Control": "no-store",
          },
        });

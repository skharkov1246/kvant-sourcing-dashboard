// Гейт сайта «Базовый проект ГОКа»: вход только через Cloudflare Access (портал КВАНТ).
...
// Внутри проектные материалы и контрагенты.
...
export default {
  async fetch(request, env) {
    ...
    if (!who) return denyPage("Базовый проект ГОКа · КВАНТ");
    if (!(await siteAllowed(request, env, SITE))) return denyPage("Базовый проект ГОКа · КВАНТ");

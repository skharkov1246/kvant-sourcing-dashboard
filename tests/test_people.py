...
def test_итоги_роли_считают_конверсию_и_взвешенный_пайплайн_отдельно():
    ...
    assert t["won"] == 1 and t["lost"] == 1 and t["winRate"] == 50
    # взвешенный пайплайн — оценка: половина проработки при конверсии 50%
    assert t["weightedRaw"] == round(t["presaleRaw"] * 0.5)

...
# внутри ячеек, таблица строками cell…row. Все три нашлись 23.09.2026 прогоном
# каскада по корпусу: «Н?а?с?о?с?» вместо «Насос», ячейки по строке на каждую,
# числа из ячеек пропали.
СЛЭШ = b"\\"


def по_lo(слово: str) -> bytes:
    """Слово так, как его пишет LibreOffice: знак Юникода и «?» (hex 3f) запасным."""
    return b"".join(СЛЭШ + b"u%d" % ord(ч) + СЛЭШ + b"'3f" if ord(ч) > 127 else ч.encode()
                    for ч in слово)


def ячейка(слово: str) -> bytes:
    return СЛЭШ + b"pard" + СЛЭШ + b"intbl{" + СЛЭШ + b"loch\n" + по_lo(слово) + b"}" + СЛЭШ + b"cell"


def ряд_rtf(*слова: str) -> bytes:
    return (СЛЭШ + b"trowd" + СЛЭШ + b"cellx1000" + b"".join(ячейка(с) for с in слова)
            + СЛЭШ + b"row")


RTF_LO = (b"{" + СЛЭШ + b"rtf1" + СЛЭШ + b"ansi" + СЛЭШ + b"uc1"
          + b"{" + СЛЭШ + b"*" + СЛЭШ + b"generator LibreOffice;}"
          + b"{" + СЛЭШ + b"*" + СЛЭШ + b"pgdsctbl{" + СЛЭШ + b"pgdsc0 Default;}}"
          + b"{" + СЛЭШ + b"loch\n" + по_lo("Насос") + b"}" + СЛЭШ + b"par "
          + ряд_rtf("Цена", "Кол", "Наименование") + ряд_rtf("1500", "2", "Задвижка") + b"}")


def test_rtf_запасной_знак_после_юникода_пропускается():
    текст = readers.text_from_rtf(RTF_LO)
    assert "Насос" in текст and "?" not in текст, текст
    assert "*" not in текст and "Default" not in текст, "пропускаемое назначение утекло"


def test_rtf_таблица_остаётся_строками_с_числами():
    """Перевод строки исходника — не текст, но завершает управляющее слово:
    удалить его значит склеить «loch<LF>1500» в одно слово «loch1500»."""
    строки = readers.rows_from_rtf(RTF_LO)
    assert ["Цена", "Кол", "Наименование"] in строки, строки
    assert ["1500", "2", "Задвижка"] in строки, строки


def test_rtf_uc0_без_запасных_знаков():
    """uc0 — запасных знаков нет, и следующая буква не должна съедаться."""
    b = b"{" + СЛЭШ + b"rtf1" + СЛЭШ + b"uc0" + СЛЭШ + b"u%d X}" % ord("Н")
    assert readers.text_from_rtf(b) == "НX"

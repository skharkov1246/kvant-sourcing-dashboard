...
# Домены и ИНН дилеров выдуманы; ИНН — с верной контрольной суммой.
РАЗВЕДКА = [{"oem_key": "zentrix", "dealers": [
    {"company": "Ромашка Технолоджи", "role": "официальный дилер: продажи и сервис",
     "domain": "romashka-tech.test", "domain_source": "https://romashka-tech.test/"},
    {"company": "Zentrix Nordic AB", "role": "дочерняя компания изготовителя"},
    {"company": "Лютик Инжиниринг", "role": "изготовитель неоригинальных аналогов", "domain": "lyutik-eng.test"},
    {"company": "ООО «Восток-Z» (ИНН 7700000104)", "role": "дочерняя компания изготовителя"},
    {"company": "Василёк Дистрибуция", "role": "официальный дистрибьютор", "domain": "zentrix.kz"},
    {"company": "Одуванчик", "role": "по своему сайту — официальный дилер", "domain": "oduvanchik.test"},
    {"company": "Почтовый дилер", "role": "официальный дилер", "domain": "gmail.com"},
...
КОРПУС = [
    ...
    # Дилер разведки, сведённый сильным ключом (домен, ИНН записи; dealer_link).
    # Имя официальным дилером не делает — выше «Ромашка Технолоджи» по имени.
    ("ООО Альфа", "Zentrix", {"домены": ["sales@romashka-tech.test"]}, orl.ТРЕЙДЕР, "официальный дилер"),
    ("Zentrix GmbH", "Zentrix", {"домены": ["https://www.romashka-tech.test/"]}, orl.ТРЕЙДЕР,
     "официальный дилер"),
    ("ООО Бета", "Zentrix", {"домены": ["zentrix.kz"]}, orl.ТРЕЙДЕР, "официальный дилер"),
    ("ООО Альфа", "Zentrix", {"домены": ["lyutik-eng.test"]}, orl.ТРЕЙДЕР, "дилер бренда по разведке"),
    ("ООО Альфа", "Zentrix", {"домены": ["oduvanchik.test"]}, orl.ТРЕЙДЕР, "дилер бренда по разведке"),
    ("ООО Альфа", "Zentrix", {"инн": ["7700000104"]}, orl.ПРЯМОЕ, "разведка: тот же бренд"),
    ("ООО Альфа", "Kvarcton", {"домены": ["romashka-tech.test"]}, orl.ТРЕЙДЕР, "имя не бренд"),
    ("ООО Альфа", "Zentrix", {"домены": ["shop.romashka-tech.test"]}, orl.ТРЕЙДЕР, "имя не бренд"),
    ("ООО Альфа", "Zentrix", {"инн": ["7700000105"]}, orl.ТРЕЙДЕР, "имя не бренд"),
...
def test_своя_площадка():
    ...
    assert not orl.своя_площадка("изготовитель альтернативных (не оригинальных) деталей под номера бренда")
    assert not orl.своя_площадка("изготовитель и продавец альтернативных деталей")


def test_официальный_дилер():
    for роль_ in ("официальный дилер: продажи и сервис", "уполномоченный дилер (морской сектор)",
                  "эксклюзивный дилер", "официальный (назначенный) дистрибьютор", "дистрибьютор бренда",
                  "лицензиат и комплектовщик", "Premium Partner (Platinum)", "Authorized Service Facility",
                  "местный дилер (по указателю изготовителя)",
                  "официальный провайдер по локатору. По своему сайту — дистрибьютор"):
        assert orl.официальный_дилер(роль_), роль_
        assert orl.вид_дилера(роль_) == orl.ОФИЦИАЛЬНЫЙ, роль_
    for роль_ in ("независимый дистрибьютор запчастей", "неофициальный продавец", "не официальный дилер",
                  "бывший официальный дилер", "официальный дистрибьютор (статус не подтверждён)",
                  "по собственному заявлению — официальный дилер", "по своему сайту — официальный дистрибьютор",
                  "называет себя официальным дилером", "перепродавец запчастей", "продавец аналогов",
                  "изготовитель альтернативных деталей", "поставщик запчастей и ремонт", "",
                  "прекращение деятельности: дилер"):
        assert not orl.официальный_дилер(роль_), роль_
    # Своя площадка — не дилер вовсе.
    assert not orl.официальный_дилер("официальное подразделение: продажи")
    assert orl.вид_дилера("дочерняя компания изготовителя") == orl.СВОЯ
    assert orl.вид_дилера("независимый продавец") == orl.ДИЛЕР
...
МУТАЦИИ = {
    ...
    "без дилеров по сильному ключу": lambda mp: mp.setattr(orl, "дилер_по_ключу", lambda *a: set()),
    "официальный — любой дилер": lambda mp: mp.setattr(orl, "официальный_дилер", lambda r: True),
    "официальных нет": lambda mp: mp.setattr(orl, "официальный_дилер", lambda r: False),
    # Общий домен отсеян с обеих сторон (запись разведки и компания КП):
    # мутация снимает оба отсева — иначе держит второй.
    "общий домен сводит": lambda mp: (
        mp.setattr(orl.sys.modules["library.dealer_link"], "общий_домен", lambda h: False),
        mp.setattr(orl, "узел", lambda a: str(a or "").rsplit("@", 1)[-1].lower())),
    "без ИНН из текста записи": lambda mp: mp.setattr(orl.sys.modules["library.supplier_link"], "инн_из_текста",
                                                      lambda *т: set()),
}

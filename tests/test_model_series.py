...
def test_несведённые_написания_изготовителя(capsys):
    с = спр()
    карта = {"kelton": "kelton"}
    итог = bm.Итог()
    # Сведённое к бренду — в списке нет, сколько бы сделок ни было.
    for i in range(5):
        bm.учесть(итог, сделка=f"k{i}", изготовитель="Kelton", текст="Кольцо", карта=карта, спр=с)
    # Несведённое в трёх сделках под двумя написаниями одного ключа.
    for i, н in enumerate(["Zarnitsa Werk", "ZARNITSA-WERK", "Zarnitsa Werk"]):
        bm.учесть(итог, сделка=f"z{i}", изготовитель=н, текст="Шайба", карта=карта, спр=с)
    bm.учесть(итог, сделка="z0", изготовитель="Zarnitsa Werk", текст="Гайка", карта=карта, спр=с)
    # Указание к закупке — в списке с родом «указание».
    for i in range(bm.MIN_DEALS):
        bm.учесть(итог, сделка=f"u{i}", изготовитель="по спецификации заказчика", текст="Болт",
                  карта=карта, спр=с)
    # Редкое — одна сделка при MIN_DEALS ≥ 2: не печатается, считается числом.
    bm.учесть(итог, сделка="r1", изготовитель="Редкобренд Уникум", текст="Втулка", карта=карта, спр=с)

    св = bm.свод(итог, с, {})
    ключи = {x["ключ"]: x for x in св["несведённые"]}
    assert "kelton" not in ключи
    assert set(ключи) == {"zarnitsawerk", "поспецификациизаказчика"}
    z = ключи["zarnitsawerk"]
    assert (z["строк"], z["сделок"], z["написаний"]) == (4, 3, 2)
    assert z["написание"] == "Zarnitsa Werk" and z["род"] == "марка?"
    assert ключи["поспецификациизаказчика"]["род"] == "указание"
    assert св["несведённых_ключей"] == 3 and св["несведённых_редких"] == 1
    assert св["несведённые"][0]["ключ"] == "zarnitsawerk"   # по сделкам
    # номера сделок в свод не попадают
    текст = json.dumps(св, ensure_ascii=False)
    assert '"z1"' not in текст and '"r1"' not in текст

    bm.печать("СПРОС", св, "сделок")
    вывод = capsys.readouterr().out
    assert "Несведённые написания изготовителя" in вывод
    assert "Zarnitsa Werk" in вывод
    assert "Редкобренд" not in вывод and "редкобренд" not in вывод


def test_род_несведённого_написания():
    def род(s):
        return bm.род_написания(s, bm.codes_sql.ключ_написания(s))

    assert род("По спецификации") == "указание"
    assert род("аналог") == "указание"
    assert род("Любой") == "указание"
    assert род("эквивалент") == "указание"
    assert род("РФ") == "страна"
    assert род("Россия") == "страна"
    assert род("123456") == "номер"
    assert род("Не указан") == "незнание"
    assert род("Zarnitsa Werk") == "марка?"
    assert род("---") == "пусто"

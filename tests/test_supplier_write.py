...
def test_схема_применяется_к_рабочей_базе_а_не_только_в_ci():
    """Схема, которую никто не применяет к проду, — это отсутствующие таблицы.

    Найдено при подготовке первой записи 20.09.2026: suppliers_schema.sql стоял
    ИСКЛЮЧИТЕЛЬНО в шаге проверок, где поднимается одноразовая база CI. К рабочей
    базе он не применялся ни одним прогоном, то есть таблиц sup_entity в проде не
    существовало, и запись упала бы на первой вставке. Транзакция откатила бы всё
    и ничего не сломала — но и не записала.

    Проверка смотрит, что файл применяется там, где адрес базы берётся из секрета
    SUPABASE_DB_URL, а не из локально поднятого PostgreSQL.
    """
    схема = "library/supabase/suppliers_schema.sql"
    рабочие = []
    for путь in sorted((ROOT / ".github" / "workflows").glob("*.yml")):
        текст = путь.read_text(encoding="utf-8")
        if схема not in текст:
            continue
        # шаг с CI-базой узнаётся по локальному адресу; рабочий — по секрету
        for кусок in текст.split("- name:"):
            if схема in кусок and "SUPABASE_DB_URL" in кусок and "127.0.0.1" not in кусок:
                рабочие.append(путь.name)
                break
    assert рабочие, (
        f"{схема} не применяется к рабочей базе ни одним прогоном — "
        "таблиц в проде не будет, а запись упрётся в их отсутствие")

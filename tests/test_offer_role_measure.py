...
def test_что_изменила_связь_дилеров(реестр):
    """A/B в одном проходе: тот же реестр без сильных ключей дилеров разведки."""
    строки = [("KV-5", "ООО Гамма", "Zentrix", None, None, ["romashka-tech.test"], None, 2),
              ("KV-6", "ООО Дельта", "Zentrix", None, None, ["zentrix.kz"], None, 1),
              ("KV-7", "ООО Эпсилон", "Zentrix", None, None, None, ["7700000104"], 4)]
    итог = orm.Итог()
    orm.судить(итог, строки + СТРОКИ, {"7": "zentrix"}, реестр)
    assert итог.дилеры == {
        ((orl.ТРЕЙДЕР, "имя не бренд"), (orl.ТРЕЙДЕР, "официальный дилер")): 2,
        ((orl.ПРЯМОЕ, "домен: тот же бренд"), (orl.ТРЕЙДЕР, "официальный дилер")): 1,
        ((orl.ТРЕЙДЕР, "имя не бренд"), (orl.ПРЯМОЕ, "разведка: тот же бренд")): 4}
    вывод = io.StringIO()
    with redirect_stdout(вывод):
        orm.печать(итог, "проба", 1)
    текст = вывод.getvalue()
    assert "сменили роль: 5 (было прямое → трейдер: 1); сменили только причину: 2" in текст
    for имя in ("Гамма", "Дельта", "romashka", "zentrix.kz", "7700000104", "KV-"):
        assert имя not in текст, имя

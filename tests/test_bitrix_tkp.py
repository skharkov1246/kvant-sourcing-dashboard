...
# --- валюта файла ----------------------------------------------------------
# Замер 18.09.2026: из 317 цен выгрузки «Энергосети», взятых по колонке, валюта
# в самой строке была ровно у одной. Значит валюту надо брать из файла, иначе
# всякая цена молча читается долларом. Корпуса придуманы (правило 18 CLAUDE.md).

def test_валюта_файла_берётся_когда_в_строке_её_нет():
    rows = [("л1", 1, ["Артикул", "Кол-во", "Цена, USD", "Сумма"]),
            ("л1", 2, ["ZZ-100500", "10", "5.00", "50.00"])]
    cur, why = T.file_currency(rows)
    assert cur == "USD" and "единственная" in why
    pr = T.price_rows(rows, T.header_map(rows), (cur, why))
    assert pr and all(p["currency"] == "USD" for p in pr)
    assert any(p["currency_source"] == "файл" for p in pr)


def test_знак_иены_без_кода_валюту_не_устанавливает():
    """Юань и иена различаются в 22,8 раза — молча выбрать нельзя."""
    rows = [("л1", 1, ["ZZ-2", "2", "545.20¥", "1090.40¥"])]
    cur, why = T.file_currency(rows)
    assert cur == T.YEN_MARK and "кода валюты нет" in why


def test_код_рядом_со_знаком_иены_снимает_неоднозначность():
    rows = [("л1", 1, ["Total in CNY"]), ("л1", 2, ["ZZ-3", "2", "545.20¥"])]
    assert T.file_currency(rows)[0] == "CNY"


def test_две_валюты_в_файле_валюту_не_назначают():
    """Смешанное предложение бывает; подставить одну — выдать догадку за факт."""
    rows = [("л1", 1, ["цена, USD"]), ("л1", 2, ["итого, руб"]),
            ("л1", 3, ["ZZ-4", "2", "100.00", "200.00"])]
    cur, why = T.file_currency(rows)
    assert cur == "" and "несколько валют" in why


def test_валюта_строки_сильнее_валюты_файла():
    rows = [("л1", 1, ["Артикул", "Кол-во", "Цена", "Сумма"]),
            ("л1", 2, ["Quotation in USD"]),
            ("л1", 3, ["ZZ-100500", "2", "100,00 руб", "200,00 руб"])]
    cur, why = T.file_currency(rows)
    pr = T.price_rows(rows, T.header_map(rows), (cur, why))
    row = [p for p in pr if p["pn"] == "ZZ-100500"]
    assert row and row[0]["currency"] == "RUB"
    assert row[0]["currency_source"] == "строка"

...
insert into lib_demand (deal_id, item_name, part_number, qty) values
  ...
  -- Разрядка текста PDF — голый стандарт, не размер.
  ('D-13', 'Учебный болт', 'G B / T 5 7 8 3 - 2 0 0 0', 1);
...
def test_спрос_по_классам(итог):
    ...
    # Стандарт — только голый: «DIN 933», слитное «DIN47125» и разрядка.
    assert с["размер"]["строк"] == 1 and с["стандарт"] == {"строк": 3, "ключей": 3,
...
def test_стандарт_с_размером(итог):
    ...
    ст = и["стандарт"]
    с = ст["спрос"]["счёт"]
    assert с["ключей"] == 5                       # гост…, din47125, din933, iso…, gbt…
    assert с["ключей_только_голых"] == 2 and с["ключей_смешанных"] == 1
    assert с["разрядка"] == 1

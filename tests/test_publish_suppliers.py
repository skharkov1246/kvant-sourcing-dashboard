...
# ── Очередь на ИНН ───────────────────────────────────────────────────────────
# Отложенные до ИНН сущности были на странице ЧИСЛОМ. Раздать по числу работу
# нельзя: чтобы снять спор, надо открыть конкретные карточки Bitrix, а какие
# именно — знал только журнал прогона, где печаталась одна длина списка.
# Корпус придуман (правило 18).

ЖДУТ_ИНН = [
    {"reason": "домен склеил разноимённые карточки портала",
     "names": ["Учебный завод", "Учебный завод, филиал"],
     "keys": ["101", "102"]},
    {"reason": "несколько правовых форм",
     "names": ["Учебная мастерская"],
     "keys": ["102", "103"]},          # 102 повторяется: одна карточка, не две
    {"reason": "без карточек", "names": ["Ничья"], "keys": []},
]


def test_очередь_инн_отдаётся_списком_карточек_а_не_числом():
    снимок = собрать(КОРПУС, ПРИЗНАКИ, 7, (), ЖДУТ_ИНН)
    очередь = снимок["inn_queue"]
    # Строка без карточек выпадает: заполнять в ней нечего.
    assert len(очередь) == 2
    assert очередь[0]["cards"] == ["101", "102"]
    assert очередь[0]["reason"] == "домен склеил разноимённые карточки портала"
    assert очередь[0]["names"] == ["Учебный завод", "Учебный завод, филиал"]


def test_карточка_в_двух_сущностях_считается_одной_единицей_работы():
    """Счётчик меряет ручную работу, а не строки: 101, 102, 103 — это три."""
    снимок = собрать(КОРПУС, ПРИЗНАКИ, 7, (), ЖДУТ_ИНН)
    assert снимок["totals"]["inn_cards"] == 3
    assert снимок["totals"]["inn_entities"] == 2


def test_пустая_очередь_не_заводит_разделов():
    """Пустой раздел на странице читается как «список есть, он пуст» — враньё,
    если очереди просто нет. Ключей быть не должно вовсе."""
    снимок = собрать(КОРПУС, ПРИЗНАКИ, 0, (), ())
    assert "inn_queue" not in снимок
    assert "inn_cards" not in снимок["totals"]


def test_очередь_не_ломается_на_пустых_и_нестроковых_значениях():
    мусор = [{"reason": None, "names": [None, "", "Имя"], "keys": [0, " ", 77, "77"]}]
    снимок = собрать(КОРПУС, ПРИЗНАКИ, 0, (), мусор)
    # 0 отбрасывается как пустой, " " — как пробельный, 77 и "77" — одна карточка.
    assert снимок["inn_queue"][0]["cards"] == ["77"]
    assert снимок["inn_queue"][0]["names"] == ["Имя"]

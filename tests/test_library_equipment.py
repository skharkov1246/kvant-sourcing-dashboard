...
def test_муфта_редуктор_и_регулятор_значат_разное_и_разбираются_закрытым_списком():
    """Три слова, на которых общее правило ошибалось измеримо: 143 расхождения
    из 243. «Муфта» — и соединительная муфта вала, и резьбовая муфта труб, и
    термоусадочная кабельная; «редуктор» — и мультипликатор, и переходник труб;
    «регулятор» — и топливный, и регулятор напряжения. Примеры придуманы."""
    # соединительная муфта вала — ротор
    assert eq.unit_of("Муфта соединительная привода насоса").startswith("rotor")
    # резьбовая арматура — крепёж, как её размечают инженеры
    assert eq.unit_of("Муфта переходная (редукционная) 1/2 x 3/8") == "fasteners"
    assert eq.unit_of("Переходник труба–труба (редуктор) 1 x 3/4") == "fasteners"
    assert eq.unit_of("Тройник штуцерный, прямой проход 1/4 x 1/4") == "fasteners"
    # кабельная муфта — электрика, а не ротор
    assert eq.unit_of("Термоусадочная муфта BK 6.5ID").startswith("controls")
    # редуктор в смысле мультипликатора остаётся в пакете
    assert eq.unit_of("Редуктор главного привода") == "package.gearbox"
    # регулятор напряжения — электрика, регулятор топлива — топливная система
    assert eq.unit_of("Автоматический регулятор напряжения").startswith("controls")
    assert eq.unit_of("Регулятор давления топливного газа").startswith("fuel")


def test_закрытый_список_ведёт_в_существующий_узел_и_остаётся_закрытым():
    """Список решает исход раньше общих правил, поэтому ошибка в нём дороже.
    Два ограничения: узел обязан существовать, а сам список — не разрастаться
    в общее правило (правило 7: обвинять можно только закрытым списком)."""
    есть = узлы()
    нет = {u for u, _ in eq.СНАЧАЛА if u not in есть}
    assert not нет, f"закрытый список ссылается на несуществующие узлы: {sorted(нет)}"
    assert sum(len(w) for _, w in eq.СНАЧАЛА) <= 20, "список перестал быть закрытым"

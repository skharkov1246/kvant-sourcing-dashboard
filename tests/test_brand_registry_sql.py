...
# ФАЗЗ-КОРПУС ПО КЛАССАМ ЗНАКОВ. 26.09.2026 гейт засева (прогон 36221302235,
# часть 5) отменил запись на одном написании из писем лидов: у базы на ICU
# lower('İ') даёт «i» с отдельной точкой U+0307, точка — не буква, и «İco»
# теряло «co» как правовую форму. У libc lower('İ') = 'i', и тесты на C.UTF-8
# этого не видели. Поэтому каждый класс гоняется и в свёртке базы, и в ICU
# (und-x-icu), и в libc C.utf8: ключ не должен зависеть от поставщика свёртки.
def _блок(a, b):
    return [chr(i) for i in range(a, b)]


КЛАССЫ_ЗНАКОВ = {
    "Latin-1": _блок(0xA0, 0x100),
    "Latin Extended-A": _блок(0x100, 0x180),
    "Latin Extended-B": _блок(0x180, 0x250),
    "Greek": _блок(0x370, 0x400),
    "Cyrillic": _блок(0x400, 0x530) + ["ё", "Ё", "і", "ї", "є", "ґ"],
    "General Punctuation": _блок(0x2000, 0x2070),
    "спецпробелы": ["\u00a0", "\u2009", "\u200b", "\ufeff", "\u202f", "\u3000"],
    "полноширинные": _блок(0xFF01, 0xFF5F),
    "лигатуры": _блок(0xFB00, 0xFB07),
    "комбинирующие": _блок(0x300, 0x370),
    "особые регистры": ["ß", "ẞ", "İ", "ı", "ς", "Σ", "\u212a", "\u212b", "ǅ", "ǈ"],
    "римские и надстрочные": _блок(0x2150, 0x2190) + ["²", "³", "¹", "\u2070", "\u2074"],
}
ФОРМЫ_ФАЗЗА = ("{}", "{}ао", "ао{}", "{}co", "co{}", "{}ab", "{}sa x", "x{}y", "A{}Б", "1{}2",
               "{0}{0}", "Siemens{}AG", "ООО {}Ромашка", "İ{}", "{}İco")
СВЁРТКИ = ("своя базы", "und-x-icu", "C.utf8")


@pytest.mark.parametrize("свёртка", СВЁРТКИ)
@pytest.mark.parametrize("класс", sorted(КЛАССЫ_ЗНАКОВ))
def test_ключ_совпадает_на_фазз_корпусе(база, класс, свёртка):
    строки = [ф.format(з) for з in КЛАССЫ_ЗНАКОВ[класс] for ф in ФОРМЫ_ФАЗЗА]
    строки += [з + "co" + з + "llc" + з for з in КЛАССЫ_ЗНАКОВ[класс]]
    conn = подключить()
    with conn.cursor() as c:
        if свёртка != СВЁРТКИ[0]:
            c.execute("select 1 from pg_collation where collname = %s", (свёртка,))
            if not c.fetchone():
                conn.close()
                pytest.skip(f"в базе нет свёртки {свёртка}")
        колл = "" if свёртка == СВЁРТКИ[0] else f' collate "{свёртка}"'
        c.execute(f"select s, lib_brand_key(s{колл}) from unnest(%s::text[]) s", (строки,))
        разные = [(н, к) for н, к in c.fetchall() if к != codes_sql.ключ_написания(н)]
    conn.close()
    assert not разные, f"{класс}/{свёртка}: расхождений {len(разные)}, первое {разные[0]!r}"
...
def test_гейт_отменяет_запись(засев, capsys):
    conn, _, _ = засев
    было = счёт(conn)
    ...
    assert счёт(conn) == было
    # Провал ключа печатает класс расхождения, но не само написание (правило 17).
    вывод = capsys.readouterr().out
    assert "среда базы: версия" in вывод
    assert "расхождение ключа: строк ×1" in вывод
    assert "расхождение ключа: записанный ключ ≠ ключ Python сейчас" in вывод
    assert "Выдумка" not in вывод and "неправильныйключ" not in вывод

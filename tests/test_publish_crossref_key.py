...
def test_вхолостую_раскладывает_все_ключи(перекрёстный, tmp_path, monkeypatch, capsys):
    ...
    # Наборы читателя: шесть запросов, имена СП-176, ячейки изготовителя
    # спроса (СПРОС_БРЕНДЫ_SQL), реестр брендов (None — по словарю-файлу),
    # карта написаний базы для роли и домены с ИНН компаний.
    monkeypatch.setattr(перекрёстный, "читать_базу",
                        lambda dsn: ([строка], [], [], [], [], [], {}, [], None,
                                     ([], "файлы репозитория"),
                                     {"KV-S-000001-1": (["uchebnyzavod.example"], [])}))
    ...
    # Бренд позиции (П1, П2) — долей и по источникам; имя бренда в журнал не идёт.
    assert "с определённым брендом 1 из 1" in журнал and "ключ бренда — словарь-файл" in журнал
    assert "SKF" not in журнал
    # Роль и приведённая цена — числами (правило 0: сколько получили и сколько нет).
    assert "роль получили 1 из 1 предложений" in журнал
    assert "компаний реестра с доменом или ИНН: 1" in журнал
    assert "с ценой 1 → приведено 1; без приведения 0" in журнал
    assert "uchebnyzavod" not in журнал

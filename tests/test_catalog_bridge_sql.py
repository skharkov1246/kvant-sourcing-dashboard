...
def cur():
    ...
    c.execute(КОРПУС)
    м = скрипт()
    c.execute(м.БАЗА, (м.FEED,))
    # Подготовленные таблицы ключей каталога — часть замера, а не оптимизация
    # мимо него: без них мосты соединяются по выражению, план становится
    # квадратичным и прогон падает по таймауту (так и случилось 22.09.2026).
    c.execute(м.ПОДГОТОВКА)
    yield c
    c.execute(f'drop schema if exists "{СХЕМА}" cascade')
    c.close()
    conn.close()
...
def test_подготовленные_таблицы_держат_весь_каталог(cur):
    """Ключи каталога разложены один раз, а не считаются на каждую строку.

    Проверка не «быстро», а «полно»: раскладка обязана охватить ВСЕ детали, иначе
    ускорение куплено потерей связей. У детали с двумя псевдонимами обе строки
    должны быть на месте — схлопывание массива в первый элемент прошло бы мимо
    счётчиков, потому что второй псевдоним в котировках не встречается.
    """
    cur.execute("select count(*) from мост_основной")
    assert cur.fetchone()[0] == 5              # все пять деталей корпуса
    cur.execute("select count(*) from мост_псевдоним")
    assert cur.fetchone()[0] == 2              # SK-12 и SK12A у одной детали
    cur.execute("select count(*) from мост_альтернатива")
    assert cur.fetchone()[0] == 3              # 180205, AMBIG у 6205 и AMBIG у fag6205
    cur.execute("select count(*) from мост_наш_номер")
    assert cur.fetchone()[0] == 1              # kv_no заполнен у одной детали
    cur.execute("select ключ from мост_псевдоним order by 1")
    assert [r[0] for r in cur.fetchall()] == ["sk12", "sk12a"]
...
def test_расхождение_ключа_каталога_видно(cur):
    """id детали не всегда равен ключу её каталожного номера — это надо считать.

    Замер по живой базе показал странное: мост по сырому catalog_no связывает
    643 артикула, а соединение по lib_parts.id — 614. Больше, чем по id, мост по
    catalog_no находить не может, если id и есть нормализованный catalog_no.
    Значит у части деталей это не так, и такая деталь из соединения по id
    выпадает МОЛЧА: связи нет, и это неотличимо от «детали в каталоге нет».

    В корпусе расходятся две детали из пяти — там, где в номере есть цифра после
    разделителя: «bolt» при «BOLT-8» даёт ключ «bolt8», «lonely» при «LONELY-1» —
    «lonely1». Там, где разделитель просто снимается («sealkit» при «SEAL-KIT»),
    ключ совпадает, и это стоит держать в голове: расхождение создаёт не дефис, а
    часть номера, потерянная при выдаче id.
    """
    м = скрипт()
    cur.execute(м.РАСХОЖДЕНИЕ_КЛЮЧА)
    деталей, иной, иной_непустой, иных_ключей = cur.fetchone()
    assert деталей == 5
    # 6205 → «6205», fag6205 → «fag6205», sealkit → «sealkit» совпадают.
    # bolt → «bolt8» и lonely → «lonely1» — нет.
    assert иной == 2
    assert иной_непустой == 2
    assert иных_ключей == 2

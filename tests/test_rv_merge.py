...
def test_повторный_приём_того_же_разбора_не_удваивает_запись():
    """Один и тот же результат разведки можно принять дважды — записи одна.

    Замер 18.09.2026: файл добора приняли второй раз, чтобы прочитать
    счётчики, и в 24 строках блок «ДОБОР ЦЕНЫ» встал дважды — примечание
    выросло до 4 428 знаков. Торговые поля перезаписались теми же значениями и
    вреда не нанесли; врало ровно пояснение, то есть то, по чему человек судит
    о строке. Первая редакция проверки сравнивала блок целиком и не сработала:
    в его заголовке стоит ПРЕЖНИЙ вердикт, а первый приём его уже изменил.
    Сверять надо текст разбора. Корпус придуман.
    """
    import rv_merge as rm

    old = {"pn": "ZZ-5100", "band_verdict": "НЕЧЕМ ПРОВЕРИТЬ — страниц нет",
           "note": "Первый проход: номер не встретился ни в одном перечне."}
    new = {"pn": "ZZ-5100", "band_verdict": "НЕ ПОДТВЕРЖДЕНА — страница есть, цены нет",
           "note": "Второй проход: карточка по номеру открыта, цены на ней нет."}
    rm.apply_update(old, new)
    first = old["note"]
    assert first.count("ДОБОР ЦЕНЫ") == 1
    rm.apply_update(old, dict(new))
    assert old["note"] == first, "повторный приём того же разбора изменил пояснение"
    assert old["note"].count("ДОБОР ЦЕНЫ") == 1


def test_другой_разбор_по_той_же_строке_ложится_второй_записью():
    """Защита от дубля не должна глотать НОВЫЙ проход по той же строке.

    Иначе следующая разведка, нашедшая цену там, где прошлая не нашла, молча
    потеряет доказательство — а именно оно решает судьбу строки.
    """
    import rv_merge as rm

    old = {"pn": "ZZ-5200", "band_verdict": "НЕЧЕМ ПРОВЕРИТЬ",
           "note": "Первый проход: страниц нет."}
    rm.apply_update(old, {"pn": "ZZ-5200", "band_verdict": "НЕ ПОДТВЕРЖДЕНА",
                          "note": "Второй проход: страница открыта, цены нет."})
    rm.apply_update(old, {"pn": "ZZ-5200", "band_verdict": "ВЕРНА",
                          "note": "Третий проход: цена напечатана на карточке продавца."})
    assert old["note"].count("ДОБОР ЦЕНЫ") == 2
    assert "Третий проход" in old["note"] and "Второй проход" in old["note"]


def test_вердикт_о_середине_вилки_на_приёме_отклоняется(tmp_path, monkeypatch):
    """«Завышена» при цене ВНУТРИ вилки — утверждение о середине, а не о вилке.

    Добавлено 18.09.2026 по живому случаю: разведка нашла 0,53 USD за штуку при
    вилке 0,5–3,0 и записала «ЗАВЫШЕНА», потому что по середине вилки строка
    стоит 9 USD, а по найденной цене 2,64. Оба числа верны, но в сводке
    «завышена» читается как «вилку надо опускать». Прежде это ловил тест гейта,
    то есть уже после записи в набор. Корпус придуман.
    """
    import rv_merge as rm

    ask = {"ZZ6100": {"pn": "ZZ-6100", "usd_lo": 0.5, "usd_hi": 3.0}}
    inside = {"pn": "ZZ-6100", "band_verdict": "ЗАВЫШЕНА — по середине вилки выходит дороже",
              "note": "корпус", "price_low": 0.53, "price_high": 0.53,
              "price_source": "https://example-shop.test/p/ZZ-6100"}
    why = rm.check(inside, ask, {"ZZ6100"}, update=True)
    assert "ВНУТРИ вилки" in why, why

    # Границы вилку задевают — вердикт остаётся любым: там утверждение уже о ней.
    outside = dict(inside, price_low=4.0, price_high=4.0,
                   band_verdict="ЗАНИЖЕНА — дороже верхней границы")
    assert rm.check(outside, ask, {"ZZ6100"}, update=True) == ""

    # ВЕРНА с оговоркой про середину проходит: это правильный способ сказать то же.
    right = dict(inside, band_verdict="ВЕРНА — цена внутри вилки; завышена её середина")
    assert rm.check(right, ask, {"ZZ6100"}, update=True) == ""

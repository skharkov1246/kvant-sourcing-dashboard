...
def test_число_не_влезающее_в_колонку_не_записывается():
    """Полный переразбор 35769505964 упал на «numeric field overflow».

    lib_prices.total объявлена numeric(18,4) — держит меньше 10^14. Число крупнее
    в неё не влезает физически, и прогон падал после нескольких сотен файлов.

    Починка НЕ в обрезке до предела. Сто триллионов за позицию — не дорогая
    деталь, а промах разбора: склеенные колонки, номер телефона, принятый за
    сумму, длинный артикул, прочитанный числом. Строка с таким числом не
    заслуживает доверия целиком и не пишется вовсе — по собственному правилу
    модуля «цифра без основания хуже пустоты».

    Тот же предел держит и цену за единицу, хотя колонка price объявлена без
    точности и приняла бы такое число МОЛЧА. Это хуже отказа: оно утянет любое
    среднее, и заметить его будет нечем.
    """
    цк = quotes.колонки_цены(["№", "Наименование", "Кол-во",
                              "Цена за ед., EUR", "Сумма, EUR"])
    # Нормальная строка проходит.
    ц = quotes.цена_строки(["1", "Подшипник", "4", "312,50", "1 250,00"],
                           цк, 4.0, "EUR")
    assert ц and ц["price"] == 312.50 and ц["total"] == 1250.00
    # Сумма не влезает — строка не пишется целиком.
    assert quotes.цена_строки(["1", "Подшипник", "4", "312,50",
                               "999999999999999,00"], цк, 4.0, "EUR") is None
    # Цена не влезает — тоже.
    assert quotes.цена_строки(["1", "Подшипник", "4", "123456789012345",
                               "500,00"], цк, 4.0, "EUR") is None
    # На границе — влезает и пишется: предел взят из объявления колонки, а не
    # придуман, поэтому и граница ровно та же.
    край = quotes.цена_строки(["1", "Подшипник", "1", "99999999999999",
                               "99999999999999"], цк, 1.0, "EUR")
    assert край and край["price"] == 99999999999999.0
    assert quotes.ПРЕДЕЛ_ЦЕНЫ == 10 ** 14
    # И текстовый путь держит тот же предел.
    assert quotes.цена_из_текста(
        "Подшипник 6205  2 шт  99999999999999999,00  199999999999999998,00") is None
    норм = quotes.цена_из_текста("Подшипник 6205  4 шт  312,50  1 250,00")
    assert норм and норм["price"] == 312.50


def test_предел_совпадает_с_объявлением_колонки():
    """Предел не выдуман: он прочитан из схемы, и расхождение с ней — ошибка."""
    схема = (ROOT / "library" / "supabase" / "schema.sql").read_text()
    m = re.search(r"alter table lib_prices add column if not exists total\s+"
                  r"numeric\((\d+),\s*(\d+)\)", схема)
    assert m, "объявление колонки total в схеме не найдено"
    точность, дробная = int(m.group(1)), int(m.group(2))
    assert quotes.ПРЕДЕЛ_ЦЕНЫ == 10 ** (точность - дробная), (
        f"колонка держит меньше 10^{точность - дробная}, "
        f"а предел в коде {quotes.ПРЕДЕЛ_ЦЕНЫ}")

...
МУТАЦИИ = [
    ...
    # Правило сборки 26.09.2026 (crossref.сравнима): цена за штуку и за комплект
    # несравнимы; две карточки Битрикса одной сущности — один поставщик.
    ("nomenclature", "n.cmp_basket", lambda о: подр(о, 0)["list"][1].update(n="компл")),
    ("nomenclature", "n.cmp_basket", lambda о: подр(о, 0)["list"][1].update(e=подр(о, 0)["list"][0]["e"])),
    ("nomenclature", "n.cmp_miss", lambda о: поз(о, 0).pop("cmp")),
    ("nomenclature", "n.sup", lambda о: поз(о, 0).update(sup=1)),
    ("nomenclature", "n.sup", lambda о: поз(о, 1).update(sup=1)),
    ...
    # Страница, рисующая столбец в каждой строке (без перечня СКРЫВАЕМЫЕ_СТОЛБЦЫ):
    # нынешняя его прячет, и тот же пустой столбец дефектом не считается
    # (test_столбец_скрытый_страницей_не_мёртвый).
    ("nomenclature", "n.col_dead", lambda о: ([поз(о, i).update(models=[]) for i in range(len(КОДЫ))],
                                              о.update(_nomenclature_html=СТРАНИЦА_БЕЗ_СКРЫТИЯ))),
...
def test_мутация(вкладка, код, правка):
    ...
    т = {x.ид: x for x in pa.ревизия(с, СЕЙЧАС, о["_dict"], lib2=lib2,
                                      страница_библиотеки=о.get("_library_html"),
                                      страница_номенклатуры=о.get("_nomenclature_html"))}[вкладка]
    п, д = т.счета[код]
...
def test_сравнимость_настоящей_сборки_ревизия_не_обвиняет():
    """n.cmp_co: «сравнимы» при одной компании НАСТОЯЩАЯ сборка не ставит.

    Ревизия 25.09.2026 (прогон 36168124416) — 3 654 таких позиции: правило
    сборки считало цены, а не компании. Корпус выдуман: два КП одной компании,
    две компании с ценой у одной и две компании с ценами — сравнима одна."""
    from tests.test_crossref_snapshot import строка
    строки = [строка("ax100", "AX-100", "1001", цена=10.0, валюта="USD", карточка="5001"),
              строка("ax100", "AX-100", "1001", цена=11.0, валюта="USD", карточка="5002"),
              строка("bx200", "BX-200", "1001", цена=20.0, валюта="USD", карточка="5001"),
              строка("bx200", "BX-200", "1001", цена=21.0, валюта="USD", карточка="5002"),
              строка("bx200", "BX-200", "1002", карточка="5003"),
              строка("cx300", "CX-300", "1001", цена=30.0, валюта="USD", карточка="5001"),
              строка("cx300", "CX-300", "1002", цена=31.0, валюта="USD", карточка="5003"),
              # Правило 26.09.2026: штука против комплекта, две карточки одной
              # сущности, «usd» против «USD» — сборка решает, ревизия согласна.
              строка("dx400", "DX-400", "1001", цена=4.0, валюта="USD", ед="шт", карточка="5001"),
              строка("dx400", "DX-400", "1002", цена=40.0, валюта="USD", ед="компл", карточка="5003"),
              строка("ex500", "EX-500", "1003", цена=5.0, валюта="USD", сущность="KV-S-000009-1",
                     карточка="5004"),
              строка("ex500", "EX-500", "1004", цена=6.0, валюта="USD", сущность="KV-S-000009-1",
                     карточка="5005"),
              строка("fx600", "FX-600", "1001", цена=6.0, валюта="usd", ед="pcs", карточка="5001"),
              строка("fx600", "FX-600", "1002", цена=7.0, валюта="USD", ед="шт", карточка="5003")]
    снимок = crossref.собрать(строки, собран=СОБРАН)
    сырьё = {k: json.dumps(v, ensure_ascii=False).encode("utf-8")
             for k, v in crossref.разложить(снимок).items()}
    т = pa.ревизия_номенклатуры(pa.Снимки(Память(сырьё)), СЕЙЧАС)
    assert т.счета["n.cmp_co"] == [2, 0]
    assert т.счета["n.cmp_miss"] == [4, 0]
    for код in ("n.cmp_basket", "n.totals", "n.sup"):
        п, д = т.счета[код]
        assert п > 0 and д == 0, код
...
def test_роль_и_приведённая_цена_настоящей_сборки_ревизия_не_обвиняет():
    ...
    assert т.доли["u.o_dealer"] == (0, 4)
    assert т.доли["u.o_norm"] == (4, 4) and т.доли["u.o_norm_index"] == (3, 4)


def test_официальный_дилер_настоящей_сборки_ревизия_считает():
    """Дилер разведки, сведённый доменом с компанией КП, — код «официальный
    дилер» в снимке; ревизия его принимает (n.o_role) и считает (u.o_dealer)."""
    from tests.test_crossref_role import РОЛИ_С_ДИЛЕРОМ, РЯДЫ_ИНДЕКСОВ, Р_БРЕНДОВ, строка
    строки = [строка("zx100", "101", сущность="KV-S-000001-1", сущность_имя="Zentrix GmbH", оем="Zentrix",
                     цена=122, валюта="RUB"),
              строка("zx100", "102", сущность="KV-S-000002-2", сущность_имя="Ромашка Трейд", оем="Zentrix",
                     цена=204, валюта="EUR")]
    снимок = crossref.собрать(строки, собран=СОБРАН, реестр=Р_БРЕНДОВ, на_дату=СОБРАН[:10],
                              роли=РОЛИ_С_ДИЛЕРОМ, ряды_индексов=РЯДЫ_ИНДЕКСОВ)
    сырьё = {k: json.dumps(v, ensure_ascii=False).encode("utf-8")
             for k, v in crossref.разложить(снимок).items()}
    т = pa.ревизия_номенклатуры(pa.Снимки(Память(сырьё)), СЕЙЧАС)
    п, д = т.счета["n.o_role"]
    assert п == 2 and д == 0
    assert т.доли["u.o_dealer"] == (1, 2) and т.доли["u.o_direct"] == (1, 2)
...
def test_нормальные_значения_на_корпусе_не_обвиняются():
    ...
    def правка(о):
        ...
        подр(о, 2)["list"][1].update(n="т.")          # обе цены за тонну — позиция сравнима
...
# ── /nomenclature: редкий столбец, который страница прячет ───────────────────
СТРАНИЦА_НОМЕНКЛАТУРЫ = pa.СТРАНИЦА_НОМЕНКЛАТУРЫ.read_text(encoding="utf-8")
# Прежняя страница: столбцы рисовались в каждой строке, перечня нет.
СТРАНИЦА_БЕЗ_СКРЫТИЯ = re.sub(r'var СКРЫВАЕМЫЕ_СТОЛБЦЫ\s*=\s*\[[^\]]*\];', "", СТРАНИЦА_НОМЕНКЛАТУРЫ)


def test_скрываемые_столбцы_читаются_со_страницы():
    assert pa.скрываемые_столбцы_страницы(СТРАНИЦА_НОМЕНКЛАТУРЫ) == {"models", "alts"}
    assert pa.скрываемые_столбцы_страницы(СТРАНИЦА_БЕЗ_СКРЫТИЯ) == frozenset()
    assert pa.скрываемые_столбцы_страницы("<html></html>") == frozenset()
    # Закомментированный перечень — не поведение.
    assert pa.скрываемые_столбцы_страницы('  // var СКРЫВАЕМЫЕ_СТОЛБЦЫ = ["models"];') == frozenset()


def test_столбец_скрытый_страницей_не_мёртвый():
    о = корпус()
    for i in range(len(КОДЫ)):
        поз(о, i).update(models=[])
    с = pa.Снимки(Память(закодировать(о)))
    т = {x.ид: x for x in pa.ревизия(с, СЕЙЧАС, о["_dict"], lib2=lib2,
                                      страница_номенклатуры=СТРАНИЦА_НОМЕНКЛАТУРЫ)}["nomenclature"]
    п, д = т.счета["n.col_dead"]
    assert п > 0 and д == 0


# ── u.o_m_self: прямое предложение — правилом library/offer_role.py ──────────

def test_изготовитель_сам_по_правилу_роли(monkeypatch):
    """Компания КП — сам бренд позиции (дочка «ООО «СКФ»» при изготовителе SKF) —
    прямое; «СКФ Сервис» — трейдер. Сравнение строк без формы не сводило их вовсе."""
    чистый_ = прогон()["nomenclature"].доли["u.o_m_self"]
    assert чистый_[0] == 0 and чистый_[1] > 0
    прямых = прогон(lambda о: ком(о, 0).update(name="ООО «СКФ»"))["nomenclature"].доли["u.o_m_self"]
    assert прямых[0] > 0 and прямых[1] == чистый_[1]
    # Прежнее правило — строка без формы против строки — того же не видело.
    assert pa.норм(pa.без_формы("ООО «СКФ»")) != pa.норм(pa.без_формы("SKF"))
    трейдер = прогон(lambda о: ком(о, 0).update(name="ООО «СКФ Сервис»"))["nomenclature"].доли["u.o_m_self"]
    assert трейдер[0] == 0
    # Мутация: правило роли подменено — метрика обязана это увидеть (правило одно).
    monkeypatch.setattr(pa.offer_role, "роль_предложения", lambda *a, **k: {"role": "прямое"})
    все = прогон()["nomenclature"].доли["u.o_m_self"]
    assert все[0] == все[1] > 0

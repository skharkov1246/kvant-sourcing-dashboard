...
def test_единица_счёта_на_листе_не_одна():
    """Правило разбора обязано читать ВСЕ единицы, какие печатает лист.

    Оплачено занижением замера 18.09.2026: правило искало только «pcs» и молча
    теряло 37 ценовых строк из 372 — каждую десятую, причём самые дорогие.
    Потерянными оказались наши же номера: клапан 186232-3000 за 11 000 EUR,
    модуль 1034549-20 за 5 000 EUR. Замер «совпало 78 номеров» был занижен, и
    занижен в дорогой части листа, где цена одной строки решает вердикт. После
    правки — 89 номеров.

    Потеря была молчаливой: строка просто не совпадала с правилом, никакой
    ошибки при этом не возникало. Поэтому проверяются все формы разом.
    """
    from stocklist_cross import parse

    cases = [
        ("<p>AB-12-3 Seal, Lip, 2.0 Od 1pce 400.00EURO each</p>", 1, 400.0),
        ("<p>CD-45 Valve, Ball, 1.0 In 5pcs 1.500.00EURO ea</p>", 5, 1500.0),
        ("<p>EF-7 Gasket, Rnd, 3.0 Id 20stk 75.00EURO ea</p>", 20, 75.0),
        ("<p>GH-8 Lamp, White, 15 W 3st 20.00EURO each</p>", 3, 20.0),
        # пробела перед числом может не быть вовсе
        ("<p>IJ-9 Rtd, 6.0 In, Pt100, Sgl Elem10pcs 500.00EURO each</p>", 10, 500.0),
    ]
    for page, qty, price in cases:
        got = parse(page)
        assert len(got) == 1, f"строка не разобрана вовсе: {page}"
        assert got[0]["qty_listed"] == qty and got[0]["price"] == price, (
            f'разобрано неверно: {got[0]} для {page}')


def test_цена_за_комплект_не_делится_на_фасовку_молча():
    """«1set (21) 226.200.00EURO set» — это цена КОМПЛЕКТА, а не штуки.

    Фасовка сохраняется отдельным полем, а деление на неё — вывод, который
    делается там, где рядом стоит наше количество, и с оговоркой. Молчаливое
    деление здесь дало бы цену за штуку, которой на листе не написано.
    """
    from stocklist_cross import parse

    got = parse("<p>KL-1 Injector, Gas, Conventional 1set (21) 226.200.00EURO set</p>")
    assert len(got) == 1
    r = got[0]
    assert r["qty_listed"] == 1 and r["pack"] == 21 and r["price"] == 226200.0
    assert r.get("price_per") == "комплект", "цена за комплект не помечена как цена комплекта"

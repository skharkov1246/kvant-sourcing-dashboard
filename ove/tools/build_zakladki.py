    ...
    return bd.table([[bd.cell(
        [bd.p([bd.run("ЧЕРНОВИК — рабочий материал базового инжиниринга", bold=True, size=26, color=AMBER)]),
         bd.p([bd.run("Не является выпущенной документацией. Ведомость собрана из единого реестра "
                      "закладных решений (zakladki.json); формулировки требований подлежат переносу "
...
def build() -> Path:
    ...
    body.append(bd.p([bd.run("Сводка: ", bold=True, size=19),
                      bd.run(f"закладок {len(items)} (Лот №1 — {counts[1]}, №2 — {counts[2]}, "
                             ...
                             f"Реестр обновлён {zk.get('updated', '—')}; ведомость сформирована {today}.",
                             size=19)]))
    ...
    body.append(bd.p([bd.run("Сформировано ove/tools/build_zakladki.py из ove/data/zakladki.json "
                             "(реестр закладных решений); источник формулировок — «Реестр предложений "
                             "по повышению эффективности, автоматизации и контролю», ред. 2.0 от "
                             "28.08.2026 (разд. 1 — базовый состав, разд. 2 — «Подготовка в базовом "
                             "составе», разд. 3 — закладные части).",

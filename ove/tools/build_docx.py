...
SEV_INTRO = {
    ...
    "minor": "Опечатки, неверные ссылки и несогласованные обозначения, выявляемые при экспертизе.",
}
...
def clean(text) -> str:
    """Снять редакционные пометки в квадратных скобках — след внутренней вычитки."""
    t = re.sub(r"\s*\[\s*[Пп]равка[^\]]*\]", "", str(text or ""))
...
def item_block(it, answers) -> str:
    ...
    if it.get("essence"):
        xml.append(p([run("На что влияет: ", bold=True), run(clean(it["essence"]))]))
    v41 = it.get("v41")
    if v41 and v41.get("s") != "new":
        ...
        xml.append(p([run("Статус по редакции 25.08.2026 (ТЗ v4.1 / ЦИМ ред.2): ", bold=True, color=v41_color),
                      run(clean(v41.get("t", "")), color=v41_color)]))
    xml.append(p([run("Вопрос Заказчику: ", bold=True), run(clean(it.get("ask", "")))]))
...
def build() -> Path:
    ...
    body = [
        ...
        p([run(f"Составлено {today}.", color="666666", size="20")]),
        p("1. Общие сведения", style="Heading1"),
        p([run("Реестр сводит расхождения между документами конкурсной пакетной документации "
               "Заказчика и вопросы, требующие решения Заказчика для продолжения работ. Каждая "
               "позиция содержит две дословные цитаты из документов Заказчика (редакция А и "
               "редакция Б), оценку влияния на проектные решения и вопрос Заказчику. Все цитаты "
               "проверены на дословное вхождение в исходные документы.", size="20")]),
        p([run("Основание: ", bold=True),
           run("ТЗ на БИ v4.1 от 25.08.2026, ТЗ на ЦИМ ред.2 и приложения; реестр сверен с новой "
               "редакцией (снятые позиции помечены, новые расхождения — C-94…C-101; адреса цитат "
               "в исторических позициях приведены по редакции v3.6).", color="666666", size="20")]),
        p([run("Порядок работы с реестром. ", bold=True),
           run("По каждой позиции Заказчик фиксирует, какая редакция принимается за проектную "
               "(или иное решение), в строке «Решение Заказчика». Реестр ведётся Исполнителем "
               "со сквозной нумерацией позиций и передаётся Заказчику по мере пополнения; "
               "решения, зафиксированные на рабочих встречах, входят в следующую ревизию "
        ...
        p([run("Сводка: ", bold=True),
           run(f"расхождений {len(items)} — блокирующих {counts['blocker']}, существенных {counts['major']}, "
               f"формальных {counts['minor']}; решений Заказчика зафиксировано {n_ans}. "
               f"Дополнительно {len(tender.get('questions_to_customer', []))} общих вопросов "
               f"без парных цитат.")]),
    ]
    ...
    body.append(p(f"{sec_no + 1}. Открытые позиции", style="Heading1"))
    n_open = len(items) - n_ans
    for t in (f"Решение Заказчика не получено по {n_open} позициям реестра — определяется на "
              "стадии БИ по итогам рабочих встреч и переписки с Заказчиком.",
              f"Блокирующие расхождения ({counts['blocker']} позиций) закрываются в первую очередь: "
              "без принятого решения проектные решения и оценка стоимости по соответствующим "
              "узлам не выпускаются.",
              "Позиции, снятые новой редакцией технического задания, сохранены в реестре с "
              "пометкой о статусе — для прослеживаемости принятых изменений.",
              "Реестр пополняется по мере анализа исходных данных; сквозная нумерация позиций "
              "сохраняется между ревизиями."):
        body.append(p([run("— ", bold=True), run(t)]))

...
def draft_plate(width) -> str:
    return bd.table([[bd.cell(
        [bd.p([bd.run("ЧЕРНОВИК — для запроса ТКП", bold=True, size=26, color=AMBER)]),
         bd.p([bd.run("Не является выпущенной документацией. Значения — из ИД/ТЗ Заказчика и решений "
                      "БИ (equipment.json, bi_lot*.json); жёлтые поля «уточняется по ТКП» заполняет "
                      "поставщик в составе технико-коммерческого предложения.", size=16, color=AMBER)])],
...
def build_perechen(eq, pj) -> Path:
    ...
    head = ["Позиция", "Наименование", "Ключевые параметры", "Кол-во", "Лот", "Примечание"]
    ...
    body.append(bd.p([bd.run("Сводка: ", bold=True, size=19),
                      bd.run(f"позиций {len(items)} (Лот №1 — {counts[1]}, №2 — {counts[2]}, №3 — {counts[3]}, "
                             f"№4 — {counts[4]}, вне лотов — {counts[0]}); ключевых {n_key} (полужирным). "
                             f"Сформировано {today}.", size=19)]))
    ...
    body.append(bd.p([bd.run("Сформировано ove/tools/build_specs.py из ove/data/equipment.json; "
                             "опросные листы по ключевым классам — файлы ove75-ol-*.docx рядом.",
...
def build_ol(idx, spec, eq, pj, lots, bi_all) -> tuple[Path, int, int]:
    ...
    body.append(bd.table(rows, widths))
    body.append(bd.p([bd.run("Жёлтым выделено «уточняется по ТКП» — заполняет поставщик; остальные значения — "
                             "исходные требования Заказчика (ИД/ТЗ) и решения БИ КВАНТ.", size=17, color=AMBER)]))
    ...
    for b in ("ТКП с ценой, сроком изготовления и условиями поставки (DDP/DAP — указать);",
              "заполненный настоящий опросный лист (жёлтые поля) и опросный лист завода при наличии;",
    ...
    body.append(bd.p([bd.run(f"Источники данных: equipment.json (перечень по ИД/ТЗ) и bi_lot{spec['lot']}.json "
                             f"(инженерные решения БИ); сборка — ove/tools/build_specs.py, {today}.",
                             size=16, color="666666")], style="Muted"))

...
def index_doc(sup, files_eqs) -> tuple:
    ...
    body = [
        ...
        pp(f"Запросов {len(positions)}; файлы ove75-rfq-<NN>-<слаг>.docx в этой же папке: "
           f"русское письмо + английский перевод + техническое приложение по данным "
           f"equipment.json. В письмах нет цен и имён поставщиков; кандидаты ({n_cand} записей "
           f"по позициям) — только в настоящем реестре. Срок ответа по всем запросам — "
           f"3 недели с даты получения."),
        pp("Классы рынка: " + " · ".join(f"{k} — {v}" for k, v in legend.items()), 17, GRAY),
    ]
    widths = [600, 3300, 600, 3400, 6670]
    head = ["№", "Позиция запроса / файл", "Лот", "Классы оборудования (по перечню БИ)",
            "Кандидаты-поставщики (suppliers.json)"]
    ...
    for n, (pos, fname, eqs) in enumerate(files_eqs, 1):
        ...
        name_paras = [bd.p([bd.run(pos["name"], bold=True, size=17)]),
                      bd.p([bd.run(f"{fname} · класс рынка {tier}", size=15, color=GRAY)])]
        if eqs:
    ...
    body.append(pp("Сформировано ove/tools/build_rfq.py из ove/data/suppliers.json и "
                   "ove/data/equipment.json. Лоты: " +

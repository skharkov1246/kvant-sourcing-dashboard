...
//     ссылкой «в прежнем разделе»; машина и узел — на карточки /p#model=,
//     /p#unit=, а раздел библиотеки — второй ссылкой;
...
test("выдача по видам: код и бренд рядом, строка ведёт на карточку, прежний раздел — второй ссылкой", async () => {
  ...
  for (const [вид, карточка, прежний] of [["kvps-ent", "/p#supplier=KV-S-000011-1", "/suppliers#e=KV-S-000011-1"],
                                           ...
                                           // Шаг 3: машина и узел — на свои карточки, библиотека — вторым.
                                           ["kvps-ent", "/p#model=vm400", "/library#segment=gtu"],
                                           ["kvps-ent", "/p#unit=hot.liner", "/library#section=component"]]) {

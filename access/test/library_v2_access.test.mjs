...
async function emptyV2(env) {
  const kinds = ["knowledge", "supplier", "price", "component"];
  const raw = JSON.stringify({ version: 2, revision: "synthetic-v2", published_at: "2026-01-01T00:00:00Z",
    article_count: 0, counts_by_confidence: {}, directory: null, segments: [{ id: "v2-only-segment", name: "Synthetic v2", note: "",
      article_count: 0, counts_by_confidence: {}, counts_by_kind: Object.fromEntries(kinds.map((k) => [k, 0])),
      indexes: Object.fromEntries(kinds.map((k) => [k, { catalog: null, search: null }])) }] });
  const hash = Buffer.from(await crypto.subtle.digest("SHA-256", encoder.encode(raw))).toString("hex");
  env.ACL.box.set("library:v2:blob:" + hash, raw);
  env.ACL.box.set("library:v2:current", JSON.stringify({ version: 2, revision: "synthetic-v2", published_at: "2026-01-01T00:00:00Z",
    manifest: { sha256: hash, bytes: Buffer.byteLength(raw) } }));
}
...
test("v2 routes retain Access/ACL gate and v1 publication cannot fork the v2 library", async () => {
  const env = envFor();
  await publish(env);
  const old = env.ACL.box.get("library:v1");
  await emptyV2(env);
  const read = await call(env, "/api/library/v2");
  assert.equal(read.status, 200);
  assert.equal((await read.json()).segments[0].id, "v2-only-segment");
  const result = await publish(env);
  assert.equal(result.status, 409);
  assert.equal((await result.json()).error, "library_v2_use_owner_drafts");
  assert.equal(env.ACL.box.get("library:v1"), old);
  const article = sample("draft:12345678-1234-1234-1234-123456789012", "v2-only-segment").articles[0];
  article.sources.kind = "knowledge";
  const draft = await call(env, "/admin/library/drafts", OWNER, { method: "POST",
    headers: { Origin: ORIGIN, "Content-Type": "application/json" }, body: JSON.stringify({ article }) });
...
test("Access JWT is verified before library pages, API and publication", async () => {
  ...
  for (const path of ["/library", "/library/", "/library.html", "/api/library", "/admin/library", "/admin/library/drafts",
    "/api/library/v2", "/api/library/v2/articles", "/api/library/v2/article?id=synthetic-first"]) {
...
test("knowledge permission required even for authenticated direct links", async () => {
  const env = envFor();
  await publish(env);
  for (const path of ["/library", "/library/", "/library.html", "/api/library", "/api/library/v2",
    "/api/library/v2/articles", "/api/library/v2/article?id=synthetic-first"]) {

...
## 12.3а Fable 5 в реестре: докупленная мощность и её границы

По решению заказчика в контур добавлен `claude-fable-5` ($10/$50 за 1M токенов —
вдвое дороже opus-5, самая сильная модель линейки). Правило распределения то же,
что и для всего реестра: **модель ставится туда, где её профиль совпадает с
профилем задачи**, а не туда, где она эффектнее выглядит.

| Роль | Было | Стало | Почему |
|---|---|---|---|
| `dispute-agent` — арбитраж спорной приёмки | opus-5 xhigh | **fable-5 xhigh** | Длинные автономные ходы с tool use — родной режим Fable; латентность в L3 не важна, кейсов ~100/мес |
| `doc-writer` — проект претензии | opus-5 | **fable-5 high** | Ошибка в претензионном документе дороже токенов |
| `protocol-assistant` — ассистент технолога | — | **fable-5 high** (новая роль) | Сборка протокола съёмки из чек-листов Р-05 — редкая, глубокая, несрочная работа |
| `acceptance-extractor`, `escalation-judge` | sonnet-5 / opus-5 | **без изменений** | См. ниже — запрет жёсткий |
| `session-auditor`, `defect-classifier` | haiku / sonnet | **без изменений** | Массовый поток: ×2 к цене без выигрыша в качестве на этих задачах |

**Куда Fable не ставится, и это закреплено кодом, а не пожеланием.** Одиночный
ход Fable на высоком effort может занимать минуты — это нормальный режим модели,
а не сбой. Синхронный приёмочный контур живёт с SLA p95 < 12 с; привязка Fable
к ролям `acceptance-extractor` / `escalation-judge` **отвергается валидацией
реестра** (`backend/app/model_registry.py`, тест
`test_fable_is_banned_from_synchronous_roles`).

Три ограничения Fable, материализованные в реестре:

1. **30-дневное хранение у Anthropic — обязательное требование модели.**
   Тенант с политикой zero-data-retention не может обслуживаться Fable вообще:
   реестр автоматически откатывает его на opus-5, фиксируя причину отката в
   логе вызова — заказчик вправе спросить, почему его спор разбирала другая
   модель. Привязка Fable без запасной модели отвергается на этапе
   конфигурации, а не в рантайме.
2. **`thinking` не передаётся.** У Fable рассуждение всегда включено; явное
   значение параметра — 400. `request_params()` реестра просто не добавляет
   поле.
3. **Server-side fallback обязателен.** Классификаторы Fable могут отказать на
   доброкачественном контенте — а наш контент это в том числе фото деталей с
   нефтехимических объектов, паспорта сред с H₂S и кислотами (Р-10 п. 3.0).
   Каждый вызов несёт `fallbacks: [claude-opus-4-8]`: отказ классификатора
   превращается в деградацию, а не в потерю разбора.

**Цена докупки.** dispute-agent ~$4,5–5/кейс × 100 ≈ $470 (замена $200);
doc-writer 750 × ~$0,26 ≈ $195 (замена $85); protocol-assistant ~$25/мес.
Итого счёт Claude растёт с ~$1 190 до ~$1 590/мес — с наценкой посредника
**+45 тыс ₽/мес, или +0,9 ₽ на позицию** (2,67 → ~3,6 ₽). На фоне полной
себестоимости 68 ₽/позицию докупка Fable — это статистический шум, покупающий
максимальное качество ровно в тех точках, где ошибка стоит реальных денег:
спор с поставщиком и претензионный документ.

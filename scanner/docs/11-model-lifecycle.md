...
> **Оговорка о единице учёта.** Цифры ниже посчитаны на приёмку, потому что
> исходная проработка велась в этих единицах. Сквозная экономика системы
> считается на **позицию** (строку SKU): 2000 приёмок × ~25 позиций = 50 000
> позиций в месяц — см. §12.1. При переводе в позиции доля MLOps в полной
> себестоимости падает, но вывод не меняется: контур доминируют люди, а не
> вычисления, и при текущем объёме он не окупается.

...
_RATE_WINDOW_S = 10
# на IP за окно: ~30 rps устойчиво; настраивается без релиза
_RATE_MAX = int(os.environ.get("KVANT_RATE_MAX", "300"))
...
def _make_store() -> Any:
    ...
    Без KVANT_PG_DSN процесс работает в памяти: это режим дев-стенда и
    тестов. На сервере отсутствие переменной означало бы, что данные
    исчезают при каждом перезапуске контейнера, — поэтому строка
    подключения обязана быть в .env прод-стенда.
    """
    dsn = os.environ.get("KVANT_PG_DSN")
    if not dsn:
        return Store()

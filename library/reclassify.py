...
#
# То же у распознавания (library/ocr.py): строка скана, заменённая повтором
# (ocr.RULE_ЗАМЕНА) или снятая откатом прогона (ocr.RULE_ОТКАТ), — номенклатура
# прежней редакции, и словарь её узнает.
НЕ_СНИМАТЬ_СЛОВАРЁМ = ("переразбор v2", "распознавание заменено", "откат распознавания")

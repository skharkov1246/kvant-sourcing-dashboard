...
dict/oem.json, атлас изготовителей zip/data/oem_atlas.json и справочник марок
портала СП-176 (его ведут люди). Остальные — OEM_ALIAS, компании-изготовители
...
пометки незнания library/equipment.OEM_JUNK и, для написаний из данных, отсев
запроса /brands (codes_sql.brand_pipeline) — тот же, что у страницы. Всё прочее
неразрешённое — «в очереди».
...
from library import codes_sql, crossref, equipment
...
def план_файлов(словарь, атлас, алиасы=None) -> План:
    ...
    п = План()
    # 1. Словарь: каждая запись — бренд, каждое написание — строка реестра.
...
def написания_компаний(компании, карта: dict[str, set]) -> list[dict]:

...
def text_from_rtf(b: bytes) -> str:
    ...
    # ПЕРЕВОДЫ СТРОК ИСХОДНИКА — НЕ ТЕКСТ. В RTF абзац задаёт \par, а голые CR/LF
    # писатель ставит где угодно для удобства чтения разметки: LibreOffice рвёт
    # ими каждую ячейку («{\loch<LF>150000}\cell»), и таблица рассыпалась по
    # строке на ячейку. «\» перед переводом строки — это \par по спецификации.
    # Удалять перевод строки нельзя: он ЗАВЕРШАЕТ управляющее слово, и
    # «\loch<LF>1» без него стало бы словом «\loch1» — число из ячейки пропало
    # бы вместе с разметкой. Поэтому он меняется на невидимый разделитель,
    # который снимается после разбора управляющих слов.
    текст = re.sub(r"\\\r?\n", r"\\par ", текст).replace("\r\n", "\x01").replace("\n", "\x01")
    текст = текст.replace("\r", "\x01")
    текст = вырезать_звёздные(текст)
    текст = юникод_rtf(текст)
    текст = re.sub(r"\\'([0-9a-fA-F]{2})",
                   lambda m: bytes([int(m.group(1), 16)]).decode("cp1251", "replace"), текст)
    # Ячейка таблицы — табуляцией, строка таблицы и абзац — переводом строки:
    # так строки таблицы остаются строками, и rows_from_rtf режет их на ячейки.
    текст = re.sub(r"\\cell\b ?", "\t", текст)
    текст = re.sub(r"\\(?:par|line|row)\b ?", "\n", текст)
    текст = re.sub(r"\\[a-zA-Z]+-?\d* ?", " ", текст)
    текст = текст.replace("{", " ").replace("}", " ").replace("\x01", "")
    строки = [re.sub(r" +", " ", "\t".join(я.strip() for я in s.split("\t"))).strip(" \t")
              for s in текст.splitlines()]
    return "\n".join(s for s in строки if s)


def rows_from_rtf(b: bytes) -> list[list[str]]:
    """Строки таблиц RTF: строка текста с ячейками через табуляцию — строка таблицы.

    Строки без табуляции (абзацы вокруг таблицы) идут одной ячейкой: шапку
    ищут ворота indexer.header_map, и абзац над таблицей им не мешает.
    """
    строки = [s.split("\t") for s in text_from_rtf(b).splitlines()]
    return строки if any(len(s) > 1 for s in строки) else []


def вырезать_звёздные(текст: str) -> str:
    r"""Группы {\*\имя ...} — «пропускаемые назначения» RTF — вырезаются целиком.

    Спецификация RTF требует от читателя, не знающего назначения, пропустить
    всю группу. Их десятки (закладки, стили страниц, данные полей, картинки
    \shppict), и до 23.09.2026 из них в текст утекали «\*» и служебные имена.
    Отображаемый текст полей стоит в \fldrslt — без звёздочки, он остаётся.
    """
    out: list[str] = []
    i, n = 0, len(текст)
    while i < n:
...
_ЮНИКОД = re.compile(r"\\uc(\d+) ?|\\u(-?\d+) ?")
_ЗАМЕНА_ЗНАКА = re.compile(r"\\'[0-9a-fA-F]{2}|[^\\{}]")


def юникод_rtf(текст: str) -> str:
    r"""\uN — знак Юникода; за ним \ucN (по умолчанию один) ЗАПАСНЫХ знаков.

    Запасной знак пишется для старых читателей и обязан пропускаться. LibreOffice
    пишет кириллицу как «\u1053\'3f» — буква и «?» запасным знаком. Прежнее
    правило снимало только буквальный «?», а «\'3f» разворачивалось в «?» после
    каждой буквы: «Н?а?с?о?с?». Нашлось 23.09.2026 прогоном каскада по корпусу.
    """
    out: list[str] = []
    запасных, i = 1, 0
    for m in _ЮНИКОД.finditer(текст):
        if m.start() < i:
            continue
        out.append(текст[i:m.start()])
        i = m.end()
        if m.group(1) is not None:
            запасных = int(m.group(1))
            continue
        out.append(chr(int(m.group(2)) % 65536))
        for _ in range(запасных):
            з = _ЗАМЕНА_ЗНАКА.match(текст, i)
            if not з:
                break
            i = з.end()
    out.append(текст[i:])

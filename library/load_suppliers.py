...
SOURCES = [
    ...
    # Сводный файл заявки (листы «Энергосети» и «НВН»): 106 продавцов, которых
    # в отдельных обходах нет. Без карточки продавца проверка наличия повисает
    # без исполнителя — ребро «деталь → у кого есть» ставить не на что.
    ("gt/data/ship_lukoil.json", "rows", "продавцы по сводной заявке ЛУКОЙЛ"),

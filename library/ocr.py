...
_ОТБОР = """
select file_id,
       case when {СМЕШАННЫЙ} and coalesce(rows_found, 0) > 0
 ...
 where ocr_at is null
   ...
   and (
     ...
     or ({СМЕШАННЫЙ} and kind = 'pdf' and status = 'разобран'
...
def отбор(есть_pdf_mixed: bool = True) -> str:
    ...
    живая база 23.09.2026 встала в режим только чтения раньше, чем миграцию
    применили: отбор с pdf_mixed падал на первом же запросе, и распознавание не
    работало вовсе (прогон 35906312186). Без колонки смешанный файл по базе не
...
def учесть_слияние(сч: Counter, rec: dict, items: list[dict], с_ценой: int,
    ...
    if rec["status"] == "не скачался":
        ...
        # КАЧЕСТВО, А НЕ ТОЛЬКО ЧИСЛО. Строка скана без артикула и без количества
        # — чаще подпись, штамп или проза сертификата, чем позиция: сумма строк
        # сама по себе не мерило (CLAUDE.md, правило 0а).
        сч["строк с артикулом"] += sum(1 for it in items if it.get("part_number"))
        сч["строк с количеством"] += sum(1 for it in items if it.get("qty"))
...
def таблица_слияния(сч: Counter) -> list[str]:
    ...
    out = ["", "СЛИЯНИЕ СО СКАНАМИ (смешанные PDF с позициями разбора):", ш,
           ...
           f"    {'  строк с артикулом':38s}{'':>8}{'':>9}{сч['строк с артикулом']:>9}",
           f"    {'  строк с количеством':38s}{'':>8}{'':>9}{сч['строк с количеством']:>9}",

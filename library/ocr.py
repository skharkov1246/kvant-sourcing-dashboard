...
# ПОТОКОВ НЕ БОЛЬШЕ, ЧЕМ ЯДЕР, СКОЛЬКО БЫ НИ ПОПРОСИЛИ. Холостой прогон 22.09.2026
# показал цену этого: прогон разбора и прогон распознавания берут число потоков из
# одного входа `workers`, разумного для СКАЧИВАНИЯ (там ждут сеть). Для tesseract он
# губителен: распознавание упирается в процессор, а у раннера GitHub два ядра. При
# WORKERS=12 из 132 файлов части 91 (69 %) вернули «таймаут распознавания» — не
# потому что файлы тяжёлые, а потому что каждый процесс получил шестую часть ядра.
# Полезный выход при этом был 253 позиции и 4 строки цены на 132 файла.
#
# Предел ставится ЗДЕСЬ, а не в прогоне: про то, что распознавание процессорное,
# знает этот модуль, а не вызывающий. Вызывающий может попросить больше — и не
# получит.
...
def main() -> int:
    ...
    # ОБРЕЗКУ ПОТОКОВ НАДО НАЗВАТЬ. Молчаливая обрезка невидима, и следующий прогон
    # попросит те же двенадцать, не узнав, что дело было не в файлах.
    if WORKERS < ЗАПРОШЕНО:
        print(f"потоков распознавания: {WORKERS} (просили {ЗАПРОШЕНО}, ядер"
              f" {os.cpu_count()}). Распознавание процессорное: потоков больше, чем"
              " ядер, дают не скорость, а таймауты — на этом 22.09.2026 сгорели"
              " 69 % файлов части.", flush=True)
    if not want:

...
def main() -> int:
    ...
    if indexer.SOURCE == "rfq":
        # Обход карточек запросов — один сплошной список, разбить его нельзя
        # (см. reparse.py). Работает часть 1; хеш файла здесь применять НЕЛЬЗЯ,
        # иначе одна часть возьмёт лишь свою двенадцатую, а остальное потеряется.
        if SHARD != 0:
            print(f"часть {SHARD + 1}: по предложениям работает только часть 1 — "
                  "обход карточек запросов не разбивается. Выходим без работы.")
            return 0

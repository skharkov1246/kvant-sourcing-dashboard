...
--
--    Повторный засев меняет строку ровно в одном случае: была «в очереди» или
--    «спорно», стала «разрешено». Прежнее состояние сохраняется в prev_*, и откат
--    прогона возвращает его, а не удаляет строку.

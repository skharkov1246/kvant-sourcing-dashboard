...
def build_procedures() -> dict[str, dict]:
    ...
    # Методы диагностики, которых в разведке OEM нет вовсе. Без них семь признаков
    # остаются без подтверждения: проверять «металл в масле» этапом ремонта
    # горячего тракта нельзя, а связь ради связи хуже её отсутствия — она врёт.
    # Справочник-заготовка, как и признаки: основание связки хранится в источнике
    # (правило 16 — сохраняй, почему получилось значение), иначе через месяц
    # заготовка станет неотличима от измерения.
    for r in (load("library/data/methods.json").get("rows") or []):
...
def main() -> int:
    ...
    с_подтверждением = {r[0] for r in рёбра_оп}
    print(f"  связей «признак → чем подтвердить»: {len(рёбра_оп)} "
          f"у {len(с_подтверждением)} признаков")
    без_подтверждения = len(symptoms) - len(с_подтверждением)
    if без_подтверждения:
        print(f"  БЕЗ ПОДТВЕРЖДЕНИЯ: {без_подтверждения} — метода в справочнике "
              f"операций нет, признак висит в воздухе")
    if потерянные:

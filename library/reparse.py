...
def main() -> int:
    ...
    # СКОЛЬКО ЦЕН ЛЕЖИТ ВО ВЛОЖЕНИЯХ СДЕЛОК — ЭТО ИЗМЕРЕНИЕ, А НЕ ЗАПИСЬ.
    # Разборщик достаёт цену из таблицы файла независимо от источника, но пишет
    # её в базу только для карточек запросов (indexer.py: «if ц and SOURCE ==
    # 'rfq'»). Во вложениях сделок цена бывает и ценой поставщика, и бюджетом
    # заказчика, поэтому писать их без разбора нельзя. Здесь они только
    # считаются: очередь работ по деньгам указывает на файлы сделок, и прежде
    # чем снимать ограничение, нужно знать, о скольких ценах идёт речь.
    ...
    print("цены НЕ записаны: разборщик пишет их только для карточек запросов.")
    print("Во вложении сделки цена бывает и предложением поставщика, и бюджетом")
    print("заказчика — снимать ограничение можно только измерив, чего там больше.")

...
def main() -> int:
    ...
    # ПЕРЕРАЗБОР ПРЕДЛОЖЕНИЙ ИДЁТ ОДНОЙ ЧАСТЬЮ, И ЭТО НЕ ЛЕНЬ. У сделок обход
    # портала разбивается до запроса подробностей: collect_refs берёт ids[shard::
    # shards] и спрашивает только свою долю. У карточек запросов обход — один
    # сплошной список (collect_refs_rfq читает все 21 865 карточек постранично),
    # и разбить его тем же способом нельзя: каждая часть всё равно прочитала бы
    # список целиком. Двенадцать одинаковых обходов одновременно — ровно то, на
    # чём 21.09.2026 умерли все двенадцать частей с HTTP 429.
    #
    # Поэтому по предложениям работает часть 0, остальные сразу выходят. Скачивание
    # внутри части остаётся параллельным (WORKERS), так что теряется не всё.
    # Распараллелить это можно, только разбив сам обход портала, — отдельная
    # правка с отдельным замером, а не побочный эффект переразбора.
    if indexer.SOURCE == "rfq":
        if SHARD != 0:
            print(f"часть {SHARD + 1}: по предложениям работает только часть 1 — "
                  "обход карточек запросов не разбивается, и двенадцать "
                  "одновременных обходов дают HTTP 429. Выходим без работы.")
            return 0
        refs = indexer.collect_refs_rfq(DAYS)
    else:

...
def ключи_ячейки(ячейка, карта, разложение=None) -> list[str]:
    ...
    «несколько» — даёт свои бренды (разложение_словаря). Часть-материал
    («Viton», «SS316») бренда не даёт тоже."""
    разложение = разложение or {}
    out = []
    ячейка = str(ячейка or "")
    # Хвост-страна («Sandvik Tamrock, ФИНЛЯНДИЯ», «Siemens - Germany») снимается
    # общим правилом codes_sql.без_страны, если ячейка целиком не сводится сама:
    # страна — не бренд, и отдельного ключа ей не положено.
    if codes_sql.ключ_написания(ячейка) not in карта:
        ячейка = codes_sql.без_страны(ячейка) or ячейка
    for часть in _ДЕЛЕНИЕ.split(ячейка):
        k = codes_sql.ключ_написания(часть or "")
        if len(k) < 2:
            continue
        # Материал («Viton», «PTFE», «AISI 316») бренда не даёт — если это не
        # написание бренда словаря (library/materials.py).
        if k not in карта and materials.ячейка_материал(часть, короткие=False):
            continue
        if k in карта:
            ключи = [карта[k]]
...
def собрать(коды: dict, каталог: dict | None = None, *, словарь=None, цепочка=None,
    ...
    сводка = {
        ...
        **({"published_at": собран} if собран else {}),
        ...
        "parts": КОРЗИН,
        # Слова облака: только марки, дубли одной марки — одним словом.
        "cloud": облако(список_брендов, карта, записи, ряды),
    }
...
# ЗАЧЕМ. Владелец 25.09.2026 по снимку облака /brands («1 545 найдено»): «Ты
# уверен, что это все бренды?» Среди крупных слов стояли «%% массовых», «г.Москва»,
# «Заполняется участникком», «Не применимо», «СОЕДИНЕННОЕ КОРОЛЕВСТВО», «турбина»,
# а одна марка шла тремя словами (CAT, Caterpillar, «Caterpillar Original & New»).
# Ревизия (прогон 36168124416): u.cloud_clean = 55 из 120.
#
# ПОМЕТКА, А НЕ УДАЛЕНИЕ (CLAUDE.md, правило 5). Бренд снимка не пропадает: он
# остаётся в списке поиска и в карточке, получает поле nb («не марка: причина») и
# в облако не идёт. Дубль получает поле cg — ключ слова облака, в которое сведён;
# его коды складываются в это слово, написания видны в карточке.
#
# ОБВИНЯЕТ ТОЛЬКО ЗАКРЫТЫЙ СПИСОК (правило 7), и только бренд вне словаря: бренд
# словаря и написание справочника рядов сильнее любой пометки. Причины — правила,
# которые уже судят поле изготовителя: oem_kind (поле формы, указание, материал,
# номер), codes_sql (незнание, стоп-слова, страна, шесть слов), equipment
# (пометки незнания; «турбина», «двигатель», «агрегат» из _NOISE) и типы машин
# справочника рядов. Дополнения ДОП_* — слова, которых этим спискам не хватило
# на снимке владельца; они только помечают слово облака и в запрос /brands этой
# правкой не переносятся: запрос отсеял бы строку, а облако её лишь не показывает.
#
# СЛИЯНИЕ — ТОЛЬКО ЧЕРЕЗ СЛОВАРЬ, не по похожести строк. Ключ бренда, его имя, имя
# без хвоста-страны (codes_sql.без_страны) и имя без хвоста из закрытого списка
# («Original», «& New», «International», правовая форма, страна) ищутся среди
# написаний справочника рядов (name, aliases, field_aliases — как
# scripts/brand_models.свести_дубли; context_aliases не сводят) и затем в карте
# словаря брендов. Разные записи справочника не сливаются никогда: Siemens AG и
# Siemens Energy — два слова.

ФАЙЛ_РЯДОВ = "dict/model_series.json"
ОБЛАКО_СЛОВ = 120

ПРИЧИНЫ_НЕ_МАРКИ = (
    "поле формы", "пометка незнания или стоп-слово", "страна или город",
    "указание к закупке", "материал", "номер вместо имени", "единица или процент",
    "общее слово оборудования", "описание длиннее шести слов",
)

# Причины, которые обвиняют и запись словаря брендов (закрытые списки без
# толкования: заглушка формы и пометка незнания).
ПРИЧИНЫ_СЛОВАРЯ = frozenset(("поле формы", "пометка незнания или стоп-слово"))

# Строки шаблона анкеты («Компонентный состав рабочей среды», «Заполняется
# участником», «при большем количестве компонентов добавьте строки»).
ДОП_ФОРМЫ_RE = r"^(заполняется\s|компонентн\w*\s+состав|при\s+большем\s+количестве)"
# «Не применимо» и заголовок колонки «OEM P/N», разрезанный по косой.
# «Original & New» — указание «оригинал» без марки (одиночное «Original» уже
# стоп-слово codes_sql.JUNK_WORDS).
ДОП_НЕЗНАНИЯ_RE = (r"^(не\s+применим\w*|неприменимо|oem\s+p(/?n)?|p/?n"
                   r"|original\s*(&|and|и)\s*new)$")
# Страны, которых нет в codes_sql.COUNTRIES; город с сокращением «г.».
ДОП_СТРАНЫ = frozenset(("turkiye", "türkiye", "соединенное королевство",
                        "united kingdom of great britain and northern ireland"))
ГОРОД_RE = r"^(г|гор)\.\s*[а-я]"
# Процент вместо имени: «%% массовых», «% weight» — заголовок колонки состава.
ПРОЦЕНТ_RE = r"^%"
# Хвост имени, который марку не меняет: «Caterpillar Original & New», «Atlas
# Copco Original», «Warman International». Сводит, только если остаток — написание
# словаря или справочника рядов.
ХВОСТ_ИМЕНИ = frozenset(("original", "genuine", "new", "oem", "international", "and", "и"))
# Правовая форма в конце КЛЮЧА: написанная слитно («NormetGroup,FINLAND» → ключ
# normetgroup) или та, которой нет в codes_sql.LEGAL_FORMS_KEY («FLOWSERVE
# CORPORATION» → flowservecorporation; ключ запроса общий с базой, и эта правка
# его не меняет). Снимается с конца ключа, только если остаток — от трёх знаков
# и находится в словаре или справочнике рядов.
СЛИТНЫЕ_ФОРМЫ = ("incorporated", "corporation", "company", "limited", "group", "gmbh", "ltd", "llc", "inc")
# Слова equipment._NOISE, которые называют оборудование (остальные — «сток»,
# «пакет», «общая» — пометки, а не оборудование).
_ОБОРУДОВАНИЕ_ШУМА = ("гту", "гтд", "турбина", "турбины", "двигатель", "агрегат")


def _норм(s) -> str:
    return " ".join(str(s or "").lower().replace("ё", "е").split()).strip(" .,;:")


@functools.lru_cache(maxsize=1)
def _слова_оборудования() -> frozenset:
    """Общие слова оборудования: типы машин справочника рядов («насос»,
    «компрессор», «редуктор») и слова оборудования из equipment._NOISE."""
    слова = {w for w in equipment._NOISE if w in _ОБОРУДОВАНИЕ_ШУМА}
    ряды = читать_файл(ФАЙЛ_РЯДОВ) or {}
    слова |= {_норм(t) for t in ряды.get("types") or []}
    return frozenset(слова)


def _страна_или_город(t: str, имя) -> bool:
    if re.match(codes_sql.COUNTRY_RE, t) or re.match(ГОРОД_RE, t) or t in ДОП_СТРАНЫ:
        return True
    чч = oem_kind.части(имя) or [str(имя or "")]
    return all(oem_kind.страна(ч) or _норм(ч) in ДОП_СТРАНЫ for ч in чч)


def не_марка(имя, ключ) -> str | None:
    """Причина «не марка» из закрытого списка ПРИЧИНЫ_НЕ_МАРКИ или None.

    Судит само имя; бренд словаря сюда не передают — он защищён."""
    t = _норм(имя)
    if not t:
        return None
    if oem_kind.поле_формы(имя) or re.match(ДОП_ФОРМЫ_RE, t):
        return "поле формы"
    if (t in codes_sql.JUNK_WORDS or t in equipment.OEM_JUNK or re.match(codes_sql.UNKNOWN_RE, t)
            or re.match(ДОП_НЕЗНАНИЯ_RE, t)):
        return "пометка незнания или стоп-слово"
    if _страна_или_город(t, имя):
        return "страна или город"
    if oem_kind.указание(имя):
        return "указание к закупке"
    if oem_kind.материал(имя):
        return "материал"
    if oem_kind.номер(имя, ключ):
        return "номер вместо имени"
    if re.match(ПРОЦЕНТ_RE, t) or t in codes_sql.UNIT_SHT + codes_sql.UNIT_SET:
        return "единица или процент"
    if t in _слова_оборудования():
        return "общее слово оборудования"
    if len(t.split()) > 6:
        return "описание длиннее шести слов"
    return None


def без_хвоста(имя) -> str:
    """Имя без хвоста из закрытого списка: слова ХВОСТ_ИМЕНИ, правовая форма,
    «&», многоточие и страна в конце. Ничего не снялось — пустая строка."""
    слова = [w for w in re.split(r"[\s,]+", str(имя or "").replace("…", " ")) if w]
    исходно = len(слова)
    while len(слова) > 1:
        w = слова[-1]
        n = _норм(w)
        if (n in ХВОСТ_ИМЕНИ or not codes_sql.ключ_написания(w) or oem_kind.страна(w)
                or n in ДОП_СТРАНЫ):
            слова.pop()
            continue
        break
    return " ".join(слова) if len(слова) < исходно else ""


def карта_рядов(ряды) -> dict[str, dict]:
    """Ключ написания → запись бренда справочника рядов по name, aliases и
    field_aliases. Написание двух брендов справочника не сводит ни к одному."""
    карта, спорные = {}, set()
    for б in (ряды or {}).get("brands") or []:
        for a in [б.get("name", "")] + list(б.get("aliases") or []) + list(б.get("field_aliases") or []):
            k = codes_sql.ключ_написания(a)
            if len(k) < 3:
                continue
            if k in карта and карта[k]["brand_key"] != б["brand_key"]:
                спорные.add(k)
            карта.setdefault(k, б)
    for k in спорные:
        del карта[k]
    return карта
...
def _вес(b) -> int:
    return (b.get("codes") or {}).get("any") or (b.get("parts") or {}).get("n") or 0


def облако(бренды: list[dict], карта: dict, записи: dict, ряды=None) -> list[dict]:
    """Пометить бренды (nb, cg) и собрать слова облака.

    Возвращает [{k, name, w, d?, m?}] по убыванию веса: k — карточка, в которую
    ведёт слово, w — сумма весов сведённых ключей (код, названный двумя
    написаниями, считается в каждом: сумма, а не объединение), d — марка словаря
    брендов или справочника рядов, m — ключи сведённых написаний."""
    по_ряду = карта_рядов(ряды)
    # Запись справочника рядов → ключ словаря брендов, если её написания в карте
    # словаря ведут к одному ключу: тогда ряд и словарь — одно слово.
    ряд_в_словарь = {}
    for б in {v["brand_key"]: v for v in по_ряду.values()}.values():
        цели = {карта[k] for a in [б.get("name", "")] + list(б.get("aliases") or [])
                + list(б.get("field_aliases") or [])
                for k in [codes_sql.ключ_написания(a)] if k in карта}
        ряд_в_словарь[б["brand_key"]] = next(iter(цели)) if len(цели) == 1 else б["brand_key"]

    группы: dict[str, list[dict]] = collections.defaultdict(list)
    имя_ряда = {}
    for b in бренды:
        b.pop("nb", None)
        b.pop("cg", None)
        k = b["k"]
        кандидаты = []
        for н in (k, codes_sql.ключ_написания(b.get("name")),
                  codes_sql.ключ_написания(codes_sql.без_страны(b.get("name"))),
                  codes_sql.ключ_написания(без_хвоста(b.get("name")))):
            if н and len(н) >= 2 and н not in кандидаты:
                кандидаты.append(н)
        for н in list(кандидаты):
            for ф in СЛИТНЫЕ_ФОРМЫ:
                if н.endswith(ф) and len(н) - len(ф) >= 3 and н[:-len(ф)] not in кандидаты:
                    кандидаты.append(н[:-len(ф)])
        г = None
        for н in кандидаты:
            if н in по_ряду:
                б = по_ряду[н]
                г = ряд_в_словарь[б["brand_key"]]
                имя_ряда[г] = б.get("name")
                break
        if г is None:
            for н in кандидаты[1:]:
                if карта.get(н) and карта[н] != k:
                    г = карта[н]
                    break
        if г is None:
            причина = не_марка(b.get("name"), k)
            # Марку словаря обвиняют только два самых узких списка: поле формы и
            # пометка незнания («Прочие» заведена в словарь брендом, но марки не
            # называет). Страна, материал, слово оборудования у словарной записи —
            # спор со словарём, и он решается в словаре, а не на странице.
            if причина and (not b.get("dict") or причина in ПРИЧИНЫ_СЛОВАРЯ):
                b["nb"] = причина
                continue
        группы[г or k].append(b)

    # ХВОСТ-СТРАНА К КЛЮЧУ СНИМКА (codes_sql.без_страны — то же правило, что у
    # codes_sql.свести): «Fleetguard, UNITED STATES OF AMERICA» — написание ключа
    # fleetguard, даже если марки нет в словаре. Сводится только одиночное
    # несловарное слово и только к ключу, который в снимке есть; куда свёлся тот —
    # туда и это.
    где = {b["k"]: г for г, члены in группы.items() for b in члены}
    for г in [г for г, члены in группы.items() if len(члены) == 1 and члены[0]["k"] == г]:
        b = группы[г][0]
        ц = codes_sql.ключ_написания(codes_sql.без_страны(b.get("name")))
        if b.get("dict") or not ц or ц == г or ц not in где or где[ц] == г:
            continue
        группы[где[ц]].append(b)
        где[г] = где[ц]
        del группы[г]

    по_ключу = {b["k"]: b for b in бренды}
    слова = []
    for г, члены in группы.items():
        члены.sort(key=lambda b: (-_вес(b), b["k"]))
        ведёт = г if г in по_ключу else члены[0]["k"]
        if г in записи:
            имя = записи[г].get("name") or г
        elif г in имя_ряда:
            имя = имя_ряда[г]
        else:
            имя = по_ключу[ведёт].get("name") or ведёт
            имя = codes_sql.без_страны(имя) or имя
        имя = oem_kind.без_формы(имя) or имя
        for b in члены:
            if b["k"] != ведёт:
                b["cg"] = ведёт
        слово = {"k": ведёт, "name": имя, "w": sum(_вес(b) for b in члены)}
        # Марка словаря: ключ словаря брендов, запись справочника рядов или
        # словарный бренд среди сведённых. Страница пишет такое слово прямо.
        if г in записи or г in имя_ряда or any(b.get("dict") for b in члены):
            слово["d"] = 1
        if len(члены) > 1 or члены[0]["k"] != ведёт:
            слово["m"] = sorted(b["k"] for b in члены)
        слова.append(слово)
    слова.sort(key=lambda s: (-s["w"], s["name"].casefold(), s["k"]))
    return слова


def итоги_облака(сводка: dict) -> dict:
    """Счётчики облака для журнала публикатора: только числа (правило 17).

    «Было» — первые ОБЛАКО_СЛОВ ключей по весу, как облако строилось до пометок:
    сколько среди них не марок и сколько дублей. «Стало» — то же по словам облака.
    dict_outside — марки словаря, у которых нет места в облаке (ни своего слова,
    ни слова, в которое их свели): обязан быть нуль. dict_not_brand — записи
    словаря, которые обвинил узкий список (ПРИЧИНЫ_СЛОВАРЯ), отдельным числом."""
    бренды = сводка.get("brands") or []
    слова = сводка.get("cloud") or []
    в_облаке = {k for s in слова for k in (s.get("m") or [])} | {s["k"] for s in слова}
    причины = collections.Counter(b["nb"] for b in бренды if b.get("nb"))
    верх_до = sorted(бренды, key=lambda b: (-_вес(b), b["k"]))[:ОБЛАКО_СЛОВ]
    по_ключу = {b["k"]: b for b in бренды}
    верх = слова[:ОБЛАКО_СЛОВ]
    return {
        "keys": len(бренды),
        "words": len(слова),
        "not_brand": sum(причины.values()),
        "reasons": dict(sorted(причины.items())),
        "merged_keys": sum(1 for b in бренды if b.get("cg")),
        "merged_words": sum(1 for s in слова if s.get("m")),
        "dict_outside": sum(1 for b in бренды if b.get("dict") and not b.get("nb")
                            and b["k"] not in в_облаке),
        "dict_not_brand": sum(1 for b in бренды if b.get("dict") and b.get("nb")),
        "top_not_brand_before": sum(1 for b in верх_до if b.get("nb")),
        "top_dup_before": sum(1 for b in верх_до if b.get("cg")),
        "top_dict_after": sum(1 for s in верх if s.get("d") or (по_ключу.get(s["k"]) or {}).get("dict")),
        "top": len(верх),
    }

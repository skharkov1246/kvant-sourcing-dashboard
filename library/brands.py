...
def карта_словаря(словарь) -> tuple[dict[str, str], dict[str, dict], int]:
    ...
    for r in (словарь or {}).get("records", []):
        ...
        if not ключ:
            continue
        записи[ключ] = r
        написания = {r.get("name"), ключ} | {s.get("spelling") for s in r.get("spellings", [])}
        for н in написания:
            k = codes_sql.ключ_написания(н or "")
            if len(k) >= 2:
                кандидаты[k].add(ключ)
...
def ключи_ячейки(ячейка, карта) -> list[str]:
    ...
    здесь нет: это выверенные поля каталога, а не слово поставщика."""
    out = []
...
ПОЛЯ = [
    ("names", "Имя и написания", "dict/oem.json — словарь написаний; без ключа — написание из данных"),
    ...
    ("card", "Бренд на карточке запроса",
     "СП-176 «Brands», имя из Битрикса; атрибуция уровня карточки, с кодами не складывается"),
]
...
def собрать(коды: dict, каталог: dict | None = None, *, словарь=None, цепочка=None,
    ...
    for oem, ид, имя, семья, прежнее, применение, площадок in каталог.get("models", []):
        for k in ключи_ячейки(oem, карта):
            b = бренд(k, oem)
    ...
    for m in (атлас or {}).get("makers", []):
        for k in ключи_ячейки(m.get("name"), карта)[:1]:
            if k not in бренды:
    ...
    for r in (цепочка or {}).get("records", []):
        ...
        for k in ключи_ячейки(r.get("from"), карта)[:1]:
    ...
    for c in (каналы or {}).get("brands", []):
        for k in ключи_ячейки(c.get("brand"), карта)[:1]:
            if k in бренды:

...
КЛЮЧ БРЕНДА — из dict/oem.json (план, этап 8.1). Запрос сводит написание к
ключу своим правилом (порт scripts/build_dict.nkey со свёрткой диакритики и
двойников), а таблица brand_map, собранная library/brands.py из словаря,
переводит его в oem_key словаря. Написание без ключа словаря не отбрасывается:
...
def запросы(карта=()) -> dict[str, str]:
    """Тексты запросов с подставленной картой ключей словаря."""
    вставка = карта_sql(карта)

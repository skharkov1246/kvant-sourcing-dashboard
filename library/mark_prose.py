...
def report(files: Files, scan, real, shadow):
    ...
    base_spec, base_prose = df.MAX_SPEC_SHARE, df.MIN_PROSE_SHARE
    # Ряд «проза≥ 0.00» отвечает на отдельный вопрос: сколько дал бы отказ по
    # ФАЙЛУ, которому вообще не нужна проза в строках. Это не предложение
    # ослабить правило, а замер гипотезы: тендерный PDF узнаётся не лексикой
    # строк — его строки обрывочны, — а тем, что почти ни одна строка не несёт
    # ни номера, ни единицы, ни количества. Колонка «ложных» показывает цену
    # такой гипотезы на том же эталоне, что и остальные ряды.
    for ms in (0.05, 0.10, 0.15, 0.20, 0.30, 0.50):
        for mp in (0.00, 0.12, 0.20):

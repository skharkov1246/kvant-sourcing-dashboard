...
def build():
    ...
    metrics = [
        ...
        {"key": "overhaul", "title": "Машины с разобранным капитальным ремонтом",
         "done": len(overhaul), "total": len(mach),
         "gap": "Ремонтное решение разобрано только там, где на руках была ведомость объёмов "
                "работ заказчика: объём, ведомость ЗИП с количествами и трудоёмкость по этапам. "
                "По остальным машинам капремонт существует как слово, а не как состав.",
         "next": "Разбирать каждую следующую тендерную ведомость капремонта в такой же набор — "
                 "объём, ЗИП, нормо-часы — и складывать рядом; цены добирать рассылкой, "
                 "потому что по крупным узлам рынок их не публикует."},

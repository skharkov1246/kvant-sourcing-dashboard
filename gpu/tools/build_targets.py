...
Берём реестр парка, схлопываем объекты в группы владельцев, прикладываем модель
потребления и получаем по каждому — сколько ЗИП он съедает за год в деньгах и в
штуках по ключевым позициям. Отдельно помечаем тех, кто уже наш заказчик.

Это оценка масштаба для внутреннего понимания рынка, а НЕ повод для рассылки:
адресных обращений по этим цифрам не делаем (распоряжение владельца 13.08.2026).

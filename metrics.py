...
def build(
    ...
    #   сорсер родительской сделки — поля сделки типа «сотрудник», где записан
    #                                сорсер: 99,8 % карточек с родительской
    #                                сделкой, и только сотрудники отдела;
    ...
    def owner_of(r: dict) -> tuple[str, str]:
        ...
        for поле in deal_sourcer_fields:
            v = _живой((d or {}).get(поле))
            if v:
                return v, "сорсер сделки"

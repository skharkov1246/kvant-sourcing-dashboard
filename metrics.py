...
def build(
    ...
    # 23.09.2026 по 138 карточкам робота, а не по соображениям удобства:
    #
    #   ответственный карточки            — если он живой человек, вопроса нет;
    #   кто двигал стадию (movedBy)       — 12 % карточек робота ведут в отдел;
    #   кто менял карточку (updatedBy)    — 8 %;
    #   автор последней активности        — 2 %;
    #   владелец родительской сделки      — 10 %, но это чаще КАМ, а не сорсер;
    #   автор карточки                    — у робота это он сам, 0 %.
    #
    # Тот, кто ТРОГАЛ карточку, поставлен впереди владельца сделки намеренно:
    # сделку ведёт КАМ, и по ней заслуга сорсера уходила аккаунт-менеджеру.
    # Владелец сделки остаётся, но последним из людей — как догадка, а не как
    # свидетельство.
    #
    # Для карточек, где ответственный — живой человек, цепочка не выполняется
    # вовсе: первое же звено возвращает его. То есть правка меняет судьбу
    # только карточек служебных записей и ничью больше.
    #
    # Оговорка, которую нельзя опускать: у 88 % карточек робота человеческого
    # следа нет ни одного — он их создал и сам же двигал. Такие остаются
    # нераспознанными, и это честнее, чем приписать их кому-то по догадке.

...
// Ключ берётся ТОЛЬКО из секрета SUPABASE_SERVICE_KEY: страховочного ключа в коде
// больше нет (20.09.2026), и без секрета прокси отвечает 503, прямо называя, чего
// не хватает. Доступ к базе закрыт с обеих сторон: ключ не в репозитории, а роли
// anon не отдаются ни таблицы ЗИП, ни досье машин (zip/supabase/migrations_rls_*).
...
// СТРАХОВОЧНОГО КЛЮЧА ЗДЕСЬ БОЛЬШЕ НЕТ, и это намеренно. До 20.09.2026 в этой
// строке лежал публикуемый ключ Supabase: пока владелец не завёл секрет, прокси
// ходил им, и сайт работал как раньше. Секрет SUPABASE_SERVICE_KEY задан в
// окружении Production, сайт пересобран, ступени 1 и 2 ужесточения применены —
// публикуемому ключу закрытые таблицы больше не отдаются, то есть страховка
// перестала страховать, а ключ в публичном репозитории остался бы.
//
// Отказ теперь громкий. Прежняя строка «(env.SUPABASE_SERVICE_KEY) || КЛЮЧ»
// без ключа дала бы undefined, и Supabase вернул бы невнятную ошибку авторизации
// откуда-то из глубины. Пропавший секрет — это поломка настройки, а не запроса,
...
async function proxyDb(request, env, who) {
  ...
  if (!key) return dbReject("db_key_missing: не задан SUPABASE_SERVICE_KEY в окружении проекта", 503);

...
Каталог дефектов проверен на {sum(1 for r in rows if r['v'] != 'скептик не сослался')}
строк из {len(rows)}; непроверенное, если оно осталось, помечено в каждой карточке
и лечится отдельным проходом проверки, а не молчанием.</div></div>"""

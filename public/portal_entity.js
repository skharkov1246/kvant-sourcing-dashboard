...
(function () {
  ...
  // Чем поставщик разведки сведён с компанией справочника (шаг 4) — словами.
  var СВЕДЕНО = { "инн": "по ИНН", "vat": "по налоговому номеру", "домен сайта": "по домену сайта",
                  "домен почты": "по домену почты" };
  ...
  function карточка_кода(v) {
    ...
    if (v.makers && v.makers.length) {
      кому.appendChild(таблица([
        // Сведён со справочником поставщиков по ИНН или домену — ссылка на
        // карточку компании; нет — имя разведки, как прежде.
        ["Компания", function (m) {
          ...
          if (m.link) box.appendChild(узел("span", "src", "сведено " + (СВЕДЕНО[m.link] || m.link)));
          return box;
        }],
      ...
      var сведено = v.makers.filter(function (m) { return m.company && m.company.id; }).length;
      кому.appendChild(пусто("Наличие, цена и срок — запись прошлой проверки у продавца, сейчас не перепроверены; дата проверки "
        + "не хранится. " + (сведено
          ? "Компания со ссылкой сведена со справочником поставщиков по ИНН или домену; без ссылки — связи нет."
          : "Со справочником поставщиков по ИНН и домену эти компании не сведены — карточек у них нет.")));
    } else {
  ...
  function карточка_поставщика(v) {
    ...
    out.push(кд);
    // Кто эта компания в реестре разведки (шаг 4): поставщики, сведённые с ней
    // по ИНН или домену. research_n = null — связь реестров не посчитана.
    ...
    if (v.research_n === null || v.research_n === undefined) {
      ...
      рз.appendChild(таблица([
        ...
        ["Сведено", function (x) { return x.rule ? (СВЕДЕНО[x.rule] || x.rule) : "—"; }],
      ...
      рз.appendChild(пусто("Сведены по ИНН или домену сайта и почты; по одному имени связь не ставится. "

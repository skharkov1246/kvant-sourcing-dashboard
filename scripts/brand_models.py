...
Ни наименований позиций, ни номеров сделок и карточек. Несведённые написания
изготовителя — отдельным блоком: без них словарь брендов нечем пополнять. Ключ —
тот же ключ написания, которым строка сводится к реестру; при нём одно
показательное написание (то, что встречено в наибольшем числе сделок), строки и
сделки. Печатается написание лишь от MIN_DEALS разных сделок (карточек): редкое
может выдать конкретную заявку. Род написания (форма / указание / незнание / номер / материал /
страна / марка?) судят правила library/oem_kind.py — те же, что у словаря.
...
from library import brands, codes_sql, model_series, oem_kind  # noqa: E402
...
class Итог:
    ...
    def __init__(self):
        ...
        # ключ написания → {написание как есть → {сделки}} и → строк
        self.несведённые = collections.defaultdict(lambda: collections.defaultdict(set))
        self.несведённые_строк = collections.Counter()
...
def учесть(итог: Итог, *, сделка, изготовитель, текст, карта: dict,
    ...
    if изготовитель and str(изготовитель).strip():
        # Общее правило сведения (codes_sql.свести): написание целиком, затем без
        # хвоста-страны («Sandvik Tamrock, ФИНЛЯНДИЯ» → Sandvik Tamrock).
        # Несведённое копится под ключом без страны: «…, ФИНЛЯНДИЯ» и
        # «…, FINLAND» — одно написание бренда, а не два.
        ключи = codes_sql.ключи_сведения(изготовитель)
        ключ_написания = ключи[-1] if ключи else ""
        ключ_поля = codes_sql.свести(изготовитель, карта) or ""
        if ключ_поля:
            пути[ключ_поля].add("поле")
        else:
            итог.неразрешённых_написаний += 1
            итог.несведённые_строк[ключ_написания] += 1
            сд = итог.несведённые[ключ_написания][" ".join(str(изготовитель).split())[:80]]
            if сделка:
                сд.add(сделка)
...
def род_написания(написание: str, ключ: str) -> str:
    """Похоже ли несведённое написание на марку. Правила — library/oem_kind.py
    (те же, что судят записи словаря): указание к закупке закрытым списком
    («по спецификации», «аналог», «любой», «эквивалент»), номер вместо имени,
    страна («РФ», «Россия»), материал, поле формы. Остальное — «марка?»: кандидат в словарь."""
    if not ключ:
        return "пусто"
    # Заглушка или реквизит формы («Выбрать менеджера», «БИК:», «Покупатель»):
    # разбор принял строку шаблона за позицию; в словарь такое не идёт.
    if oem_kind.поле_формы(написание):
        return "форма"
    if oem_kind.указание(написание):
        return "указание"
    if ключ.isdigit() or oem_kind.номер(написание, ключ):
        return "номер"
    низ = " ".join(str(написание).lower().replace("ё", "е").split())
    # Пометка незнания («Не указан», «Прочие», «импорт») — стоп-слово страницы
    # /brands (codes_sql.UNKNOWN_RE), тем же выражением.
    if re.match(codes_sql.UNKNOWN_RE, низ):
        return "незнание"
    if (re.match(codes_sql.COUNTRY_RE, низ)
            or all(oem_kind.страна(ч) for ч in oem_kind.части(написание) or [написание])):
        return "страна"
    if oem_kind.материал(написание):
        return "материал"
    return "марка?"


def несведённые(итог: Итог) -> tuple[list, int]:
    """Несведённые написания от MIN_DEALS сделок: [{ключ, написание, род, строк,
    сделок, написаний}] по сделкам, и сколько ключей отсеяно как редкие."""
    out, редких = [], 0
    for ключ, варианты in итог.несведённые.items():
        сделки = set().union(*варианты.values())
        if len(сделки) < MIN_DEALS:
            редких += 1
            continue
        # Показательное — написание из наибольшего числа сделок; при равенстве —
        # короткое и первое по алфавиту, чтобы вывод не зависел от порядка строк.
        показ = min(варианты, key=lambda н: (-len(варианты[н]), len(н), н))
        out.append({"ключ": ключ, "написание": показ, "род": род_написания(показ, ключ),
                    "строк": итог.несведённые_строк[ключ], "сделок": len(сделки),
                    "написаний": len(варианты)})
    out.sort(key=lambda x: (-x["сделок"], -x["строк"], x["ключ"]))
    return out, редких
...
def свод(итог: Итог, спр, имена_брендов: dict) -> dict:
    ...
    return {"строк": итог.строк, "с_брендом": итог.с_брендом, "с_моделью": итог.с_моделью,
            ...
            "несведённых_ключей": len(итог.несведённые),
            "несведённых_редких": редких,
            "несведённые_род": dict(collections.Counter(x["род"] for x in несв)),
            "несведённые": несв[:НЕСВЕДЁННЫХ],
...
def печать_несведённых(с: dict, единица: str) -> None:
    несв = с.get("несведённые") or []
    род = sorted((с.get("несведённые_род") or {}).items(), key=lambda x: (-x[1], x[0]))
    print(f"\nНесведённые написания изготовителя: ключей {с.get('несведённых_ключей', 0):,}, "
          f"из них реже {MIN_DEALS} {единица} (не печатаются) {с.get('несведённых_редких', 0):,}; "
          "род: " + (", ".join(f"{р} {n:,}" for р, n in род) or "—"))
    if not несв:
        return
    print(f"  первые {len(несв)} по {единица}:")
    print(f"  {'написание':<42} {'ключ':<30} {'род':<9} {'строк':>8} {единица:>8} {'вариантов':>9}")
    for x in несв:
        print(f"  {x['написание'][:42]:<42} {x['ключ'][:30]:<30} {x['род']:<9} "
              f"{x['строк']:>8,} {x['сделок']:>8,} {x['написаний']:>9,}")

...
может выдать конкретную заявку. Род написания (указание / незнание / номер / материал /
...
def учесть(итог: Итог, *, сделка, изготовитель, текст, карта: dict,
    ...
    if изготовитель and str(изготовитель).strip():
        ключ_написания = codes_sql.ключ_написания(изготовитель)
        ключ_поля = карта.get(ключ_написания, "")
        if ключ_поля:
...
def род_написания(написание: str, ключ: str) -> str:
    ...
    страна («РФ», «Россия»), материал. Остальное — «марка?»: кандидат в словарь."""
    if not ключ:
        return "пусто"

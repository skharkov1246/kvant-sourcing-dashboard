...
СТД_ПАРАМЕТРЫ = {
    ...
    "не_форма": r"[^9 .:/-]",
    # Прежняя, свободная редакция правила: второй токен с ЛЮБОЙ цифрой. Первый
    # замер (прогон 36114332994) показал, что так вразрядку набранный голый
    # стандарт («G B / T 5 7 8 3 - 2 0 0 0») становится кодом; счётчик держит
    # разницу на виду.
    "свободно": "[0-9][^ \\t\\r\\n\\u00a0]*" + ПРОБЕЛ + "[^0-9]*[0-9]",
}
_СТД_СТРОКА = """
  ...
  count(*) filter (where not р and w ~ %(свободно)s)::bigint as разрядка,
...
def замер_стандарта(cur, п: dict, есть_part_id: bool) -> dict:
    ...
    def свод(строки: dict, со_сделками: bool) -> dict:
        ...
        for k, v in строки.items():
            голых, размер, год, два, знак, разрядка, ф_размер, ф_голая = v[:8]
            с["ключей"] += 1
            с["строк_голых"] += голых
...
def печать_стандарта(ст: dict) -> None:
    ...
    for имя, свод in [("спрос (lib_demand_live)", ст["спрос"])] + \
        ...
        print(f"    сомнительные из вернувшихся строк: последний токен — год 19xx/20xx"
              ...
              f" числа через знак в пробелах {с['через_знак']}; отсеяно как разрядка или"
              f" одиночная цифра (не размер) {с['разрядка']}")

...
def читать_реестр(cur):
    ...
    а файл их даёт."""
    cur.execute(brands.ЕСТЬ_РЕЕСТР_SQL)
    if not cur.fetchone()[0]:
        return None
    cur.execute(brands.РЕЕСТР_БРЕНДЫ_SQL)
    бренды = cur.fetchall()
    cur.execute(brands.РЕЕСТР_НАПИСАНИЯ_SQL)
    написания = cur.fetchall()
    if not бренды or not написания:
        return None
    cur.execute(brands.РЕЕСТР_КАРТОЧКА_SQL)
    карточка, имена = {}, {}
    ...
    return {"словарь": brands.словарь_из_реестра(бренды, написания),

...
brands:codes:00…15, brands:owners:v1, brands:subs:v1
...
ЕСТЬ_РЕЕСТР_КОМПАНИЙ_SQL = ("select to_regclass('sup_entity') is not null"
                            " and to_regclass('sup_number_registry') is not null")
РЕЕСТР_КОМПАНИЙ = "sup_registry"
...
def читать_базу(dsn, карта):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Реестр компаний с номером KV-S (те же строки, что у suppliers:v1):
            # по нему субпоставщик карточки бренда получает ссылку /suppliers#e=.
            # Схемы реестра нет — ссылок нет, публикация не срывается.
            cur.execute(ЕСТЬ_РЕЕСТР_КОМПАНИЙ_SQL)
            if cur.fetchone()[0]:
                t = time.monotonic()
                cur.execute(company_names.имена_sql(ps.СУЩНОСТИ_SQL, имена))
                каталог[РЕЕСТР_КОМПАНИЙ] = [(r[0], r[1]) for r in cur.fetchall() if r[6]]
                время["реестр компаний"] = time.monotonic() - t
        conn.rollback()
...
def печать_итогов(снимки, размеры, предел):
    ...
    в = brands.итоги_владения(с)
    # Владение: только числа — ни имени бренда, ни владельца (правило 17).
    print(f"владение: карточек с полем own {в['cards']}; с нынешним владельцем {в['with_owner']}, "
          f"с бывшим {в['with_former']}, с брендами группы {в['with_group']}; ссылок {в['links']}, "
          f"из них без карточки в снимке {в['links_without_card']}")
    су = brands.итоги_субпоставщиков(с)
    # Субпоставщики: только числа — ни компании, ни узла, ни бренда (правило 17).
    print(f"субпоставщики: брендов с блоком {су['brands']}, строк {су['rows']}, из них из сверки "
          f"{су['from_file']}, со ссылкой на реестр KV-S {су['with_registry']}")
    ...
    print(f"ключ субпоставщиков {brands.КЛЮЧ_СУБПОСТАВЩИКОВ}: брендов "
          f"{len(снимки[brands.КЛЮЧ_СУБПОСТАВЩИКОВ]['brands'])}")
    for ключ, n in размеры.items():
        # Ключи владения и субпоставщиков читает каждый запрос карточки бренда на /p —
        # предел свой.
        свой = {brands.КЛЮЧ_ВЛАДЕНИЯ: brands.ПРЕДЕЛ_ВЛАДЕНИЯ,
                brands.КЛЮЧ_СУБПОСТАВЩИКОВ: brands.ПРЕДЕЛ_СУБПОСТАВЩИКОВ}.get(ключ, предел)
        доля = n / свой
...
def main(argv=None):
    ...
    try:
        ...
        # Ключ справочника рядов для владения (brand_key) есть только в словаре-файле:
        # реестр базы этого поля не знает.
        ключи_рядов = brands.карта_рядов_словаря(словарь)
        ...
        # Субпоставщики — из файлов разведки и сверки (только подтверждённые).
        субпоставщики = brands.читать_субпоставщиков()
        сч = субпоставщики["counts"]
        print(f"субпоставщики в файлах: разведка {сч.get('research', 0)}, из них подтверждено "
              f"{сч.get('research_verified', 0)}; сверка {сч.get('verified_file', 0)}; без источника "
              f"{сч.get('without_source', 0)}, без имени или узла {сч.get('without_name', 0)}, "
              f"повторов {сч.get('duplicates', 0)}; к показу {сч.get('confirmed', 0)}; реестр KV-S "
              f"{len(каталог.get(РЕЕСТР_КОМПАНИЙ) or [])} компаний")
        снимки = brands.собрать(
            ...
            ряды=brands.читать_файл(brands.ФАЙЛ_РЯДОВ), ключи_рядов=ключи_рядов,
            субпоставщики_записи=субпоставщики["records"],
            реестр_поставщиков=каталог.get(РЕЕСТР_КОМПАНИЙ),
        ...
        ps.require(len(сырые[brands.КЛЮЧ_СУБПОСТАВЩИКОВ]) <= brands.ПРЕДЕЛ_СУБПОСТАВЩИКОВ,
                   "SNAPSHOT_TOO_LARGE")
        ...
        # Ключи владения и субпоставщиков от сводки не зависят: /p читает их сам
        # по ключу бренда.
        for k in brands.КЛЮЧИ_КОРЗИН + (brands.КЛЮЧ_ВЛАДЕНИЯ, brands.КЛЮЧ_СУБПОСТАВЩИКОВ,
                                         brands.КЛЮЧ_ПАР, brands.КЛЮЧ_СВЯЗЕЙ, brands.КЛЮЧ):
            cf.put(namespace, k, сырые[k])
        print(f"опубликовано в KV: {len(сырые)} ключей ({brands.КЛЮЧ}, {brands.КЛЮЧ_СВЯЗЕЙ}, "
              f"{brands.КЛЮЧ_ПАР}, {brands.КЛЮЧ_ВЛАДЕНИЯ}, {brands.КЛЮЧ_СУБПОСТАВЩИКОВ}, "
              f"корзин кодов {brands.КОРЗИН})")
        return 0

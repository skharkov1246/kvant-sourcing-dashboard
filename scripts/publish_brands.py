...
def main(argv=None):
    ...
    try:
        карта, _, _ = brands.карта_словаря(brands.читать_файл(brands.ФАЙЛ_СЛОВАРЯ))
        коды, каталог, время = читать_базу(os.environ.get("SUPABASE_DB_URL"), sorted(карта.items()))

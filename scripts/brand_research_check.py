...
def проверить_бренд(данные, имя_файла: str, ключи: set[str], не_бренды=frozenset()) -> list[str]:
    """Нарушения одного файла бренда: строки «файл место: правило».
    не_бренды — ключи записей словаря вида не «бренд» (не_бренды_словаря)."""
    out: list[str] = []
...
def проверить_очередь(данные, ключи: set[str], бренды: dict[str, dict], не_бренды=frozenset()) -> list[str]:
...
def не_бренды_словаря(путь: Path) -> set[str]:
    """Ключи записей словаря вида не «бренд». Запись без kind — бренд (словарь
    прежней сборки)."""
    данные = json.loads(путь.read_text(encoding="utf-8"))
    return {r["oem_key"] for r in данные.get("records", [])
            if r.get("oem_key") and (r.get("kind") or "бренд") != "бренд"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    не_бренды = не_бренды_словаря(словарь)
    нарушения: list[str] = []
    ...
    for p in sorted(папка.glob("*.json")):
        ...
        try:
            ...
            continue
        нарушения += проверить_бренд(d, p.name, ключи, не_бренды)
    ...
    if очередь_путь.exists():
        try:
            ...
            нарушения += проверить_очередь(очередь, ключи, бренды, не_бренды)
    else:
    ...
    print(f"словарь dict/oem.json: ключей {len(ключи)} · не брендов по виду записи {len(не_бренды)} · "
          f"негодных для очереди {негодных} "

...
# «Co.» ПРАВОВОЙ ФОРМОЙ НЕ СЧИТАЕТСЯ, и вот почему. В «Acme Co., Ltd» это одна
# форма, записанная двумя словами, а не две: так пишется половина азиатских
# компаний. Разбор выдавал {co, ltd}, и дальше две беды сразу — «Acme Co., Ltd» и
# «Acme Ltd» не сходились по пересечению форм, а сущность с {co, ltd} считалась
# несущей два юрлица. Замер 20.09.2026 по файловым реестрам: 516 сущностей из 570
# «спорных» были именно «Co., Ltd», и 484 из них вообще ничего не сливали.
#
# Само по себе «Co.» тоже ничего не различает: «Trading Co.» против «Trading Ltd»
# — это не доказательство разных лиц, а разница в том, как написали вывеску.
# Различать юрлица должен ИНН (распоряжение владельца 20.09.2026), а не слово Co.
СЛУЖЕБНЫЕ_ФОРМЫ = frozenset({"co"})
...
def форма(s: str | None) -> frozenset[str]:
    ...
    формы = {СИНОНИМЫ_ФОРМ.get(f, f) for f in сырые if f}

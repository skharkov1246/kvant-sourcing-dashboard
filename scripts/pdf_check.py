...
def main() -> int:
    ...
    # ПОСЛЕДНЯЯ страница из проверки исключена. Признак кривого разрыва — короткая
    # страница, ЗА КОТОРОЙ есть ещё содержимое: значит что-то не влезло и уехало.
    # Хвост текущего документа короче порога просто потому, что текст кончился, и
    # ничего с этим сделать нельзя — правка вёрстки лишь передвигает хвост. До
    # 21.09.2026 проверка считала и его, и на это дважды ушла правка вёрстки там,
    # где дефекта не было (см. docs/ПРАВИЛА-PDF.md).
    sparse = [i + 1 for i, c in enumerate(counts[:-1]) if c < a.min_chars]
    if sparse:
        problems.append(f"полупустые страницы (<{a.min_chars} симв.): {sparse}")

    # ДОКУМЕНТ ЦЕЛИКОМ КОРОЧЕ ОДНОЙ СТРАНИЦЫ — это не кривой разрыв, а несобранный
    # документ. Исключение последней страницы из проверки выше (правильное само по
    # себе) сделало одностраничный PDF неуязвимым: 93 символа на единственной
    # странице проходили как «чисто». Поймано 21.09.2026 на отчёте владельцу,
    # который не отрисовался из-за упавшей базы, а проверка это подтвердила.
    if sum(counts) < a.min_chars:
        problems.append(f"документ почти пуст: {sum(counts)} симв. на {len(pages)} "
                        f"страницах — содержимое не отрисовалось")

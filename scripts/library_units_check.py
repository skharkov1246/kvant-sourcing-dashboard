...
# ОБЪЯСНЁННЫЕ РАСХОЖДЕНИЯ. Не всякое несовпадение с разметкой — ошибка правила:
# бывает, что ярлык в таблице инженеров сам спорный. Такие пары держатся здесь
# закрытым списком с причиной и печатаются отдельной строкой — спрятать их в
# «совпало» значило бы подогнать точность под себя, а не измерить её.
ОБЪЯСНЁННЫЕ: dict[tuple[str, str], str] = {
    ("tooling", "controls"):
        "автоматические и миниатюрные выключатели, предохранители и аксессуары "
        "расцепителей (Siemens 3AH/3AK) в таблице инженеров попали в «Оснастку и "
        "инструмент». Это электрика: по ней их и ищут, ей и размечаем",
}
...
def main() -> int:
    ...
    эталон = верно = неверно = промолчало = объяснённые = 0
    ...
    if путаница:
        ...
        for k, n in путаница.most_common(8):
            пара = tuple(k.split(" → "))
            print(f"   {k:34}{n}" + ("   ← спорный ярлык, см. ОБЪЯСНЁННЫЕ"
                                     if пара in ОБЪЯСНЁННЫЕ else ""))
    if объяснённые:
        остаток = неверно - объяснённые
        чистая = (верно + объяснённые) / ответов * 100 if ответов else 0
        print(f"\nиз них спорных ярлыков: {объяснённые} — "
              f"необъяснённых расхождений {остаток}")
        print(f"точность без спорных ярлыков: {чистая:.0f}% "
              f"(в гейт идёт строгая: {точность:.0f}%)")
        for (a_, b_), почему in ОБЪЯСНЁННЫЕ.items():
            print(f"   {a_} → {b_}: {почему}")

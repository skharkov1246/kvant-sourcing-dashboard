...
GROUPS = [
    ...
    ("instrum", "Приборные признаки: показание, сигнал, срабатывание"),
]
...
SYMPTOMS = [
    ...
    # ── Приборные признаки. Общая таксономия их не ловит: у прибора «неисправно»
    # выглядит не как шум и нагрев, а как уход показания, отказный ток и
    # несработавшая защита. Без этих групп 107 карточек КИПиА не находятся
    # поиском по признаку — то есть каталог по направлению бесполезен.
    ("i_drift", "instrum", "Дрейф, уход нуля, расхождение с эталоном",
     r"дрейф|уход\w* нул|ползёт|ползет|завышен\w+ показани|занижен\w+ показани|"
     r"расхожден\w+ с эталон|уход\w* калибровк|сбит\w+ привязк|сбит\w+ настройк"),
    ("i_fault", "instrum", "Отказный ток и самодиагностика прибора",
     r"namur|≤\s*3,6|≥\s*21,0|3,6\s*ма|21,0\s*ма|отказн\w+ уровен|"
     r"сигнал\w* неисправност|потер\w+ эхо|уходит в отказ|диагностик\w+ прибора"),
    ("i_trip", "instrum", "Несрабатывание или ложное срабатывание защиты",
     r"не срабатыва|ложн\w+ срабатыван|сигнализаци\w+ не|контакт\w+ не сработ|"
     r"отсечк\w+ не|залипш|подгоревш\w+ контакт|команда есть, а"),
    ("i_proof", "instrum", "Просроченная поверка и непроведённая проверка контура",
     r"просроч|поверк\w+ истек|интервал\w* проверк|сквозн\w+ проверк|proof test|"
     r"журнал\w* псб|дата последн"),
    ("i_stroke", "instrum", "Ход и позиционирование исполнительного механизма",
     r"не доходит до упор|шток стоит|ход\w* клапан|процент\w* хода|"
     r"ограничител\w+ крутящ|не отрабатыва|отрабатывает, но"),
    ("i_loopfit", "instrum", "Несогласование параметров контура",
     r"\bco\b и \blo\b|ёмкост\w+ и индуктивност|емкост\w+ и индуктивност|"
     r"падени\w+ напряжени|нагрузк\w+ контур|сопротивлен\w+ контур|"
     r"минимальн\w+ напряжени"),
    ("i_setpoint", "instrum", "Уставка задана не по своей базе",
     r"уставк\w+ задан|уставк\w+ выставлен|по таблице зон вместо|"
     r"уставк\w+ alarm|уставк\w+ danger"),
...
NODE_RULES = [
    # ── КИПиА идёт ПЕРВОЙ и правилами поуже: «регулирующий клапан» и «клапан
    # компрессора» — разные узлы, и общий шаблон арматуры забрал бы оба. Сто семь
    # карточек направления не влезают в один ящик «КИП, САУ, защиты»: по такому
    # узлу ничего не найдёшь, а вся ценность каталога — в поиске по узлу.
    ("i_press", "КИПиА: давление и перепад",
     r"датчик\w* давлени|преобразовател\w* давлени|манометр|перепад\w* давлени|"
     r"мембранн\w+ разделител|сужающ\w+ устройств|диафрагм\w* \(суж"),
    ("i_temp", "КИПиА: температура",
     r"термопар|термопреобразовател|\bтсп\b|\brtd\b|холодн\w+ спа|"
     r"компенсационн\w+ лини|удлинительн\w+ лини"),
    ("i_flow", "КИПиА: расход",
     r"расходомер|кориолис|вихрев\w+ расход|ультразвуков\w+ расход"),
    ("i_level", "КИПиА: уровень",
     r"уровнемер|буйков|радарн\w+ уровн|уравнительн\w+ лини"),
    ("i_imp", "КИПиА: импульсные линии и отборные устройства",
     r"импульсн\w+ лини|отборн\w+ устройств|трасс\w+ импульсн"),
    ("i_vibro", "КИПиА: вибромониторинг",
     r"вибромониторинг|вихретоков\w+ пробник|вихретоков\w+ канал|"
     r"пьезоакселерометр|виброконтрол\w+ стойк"),
    ("i_valve", "КИПиА: регулирующая арматура и приводы",
     r"регулирующ\w+ клапан|регулирующ\w+ арматур|пневмопривод|мембранн\w+ исполнит|"
     r"\bмим\b|стопорн\w+ клапан|антипомпажн"),
    ("i_pos", "КИПиА: позиционеры и электропневматика",
     r"позиционер|электропневмопреобразовател|\bi/p\b|электропневмораспредел|"
     r"воздух\w* кип|подготовк\w+ воздуха"),
    ("i_gas", "КИПиА: газоанализаторы и извещатели",
     r"газоанализатор|извещател\w* пламен|детектор\w* пламен|"
     r"термокаталитическ|электрохимическ\w+ ячейк"),
    ("i_paz", "КИПиА: ПАЗ и функции безопасности",
     r"\bпаз\b|противоаварийн|приборн\w+ функци\w+ безопасн|\bsil\b|"
     r"логическ\w+ устройств|плк безопасн"),
    ("i_loop", "КИПиА: контур 4–20 мА, питание, искрозащита",
     r"токов\w+ петл|4\s*[—–-]\s*20\s*ма|4\s*[—–-]\s*20 ма|контур 4|барьер\w* искро|"
     r"искрозащит|взрывозащищённ\w+ электроустановк|взрывозащищенн\w+ электроустановк|"
     r"питание контур"),
    ("i_second", "КИПиА: вторичные приборы и метрология",
     r"вторичн\w+ прибор|метрологическ|поверк|калибровк\w+ контур"),

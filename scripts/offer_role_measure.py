...
справочников, не данные заказчиков); сверка с прежним правилом; что изменила
связь дилеров разведки с компаниями по домену и ИНН (library/dealer_link.py) —
тот же итог без неё, переходы «было → стало» (правило 0). Ни имён компаний,
...
class Итог:
    def __init__(self):
        ...
        # Связь дилеров по сильному ключу: (роль и причина без неё, с ней) → строк,
        # только где ответ изменился (бренд позиции — итог).
        self.дилеры = collections.Counter()
        self.строк = 0
        self.без_сущности = 0


def без_дилеров(реестр):
    """Тот же реестр, но без сильных ключей записей dealers разведки: роль, какой
    она была до связи дилеров с компаниями (сравнение A/B в одном проходе)."""
    р = copy.copy(реестр)
    р.дилеры_ключ = collections.defaultdict(set)
    return р
...
def судить(итог: Итог, строки, sp176: dict, реестр) -> None:
    кэш: dict = {}
    кэш_без: dict = {}
    реестр_без = без_дилеров(реестр or offer_role.реестр_файлов())
    for сущ, имя, файл, карточка, каталог, домены, инн, n in строки:
        ...
        for и in ИСТОЧНИКИ:
            ...
            if и == "итог" and (домены or инн):
                б = кэш_без.get(ключ)
                if б is None:
                    б = кэш_без[ключ] = offer_role.роль_предложения(имя, бренды[и], домены=домены or (),
                                                                    инн=инн or (), реестр=реестр_без)
                if (б["role"], б["why"]) != (о["role"], о["why"]):
                    итог.дилеры[((б["role"], б["why"]), (о["role"], о["why"]))] += n
...
def печать(итог: Итог, откуда_карты: str, частей: int) -> None:
    ...
    print("\n── связь дилеров разведки с компаниями по домену и ИНН (бренд позиции — итог) ──")
    if not итог.дилеры:
        print("  ничего не изменила")
    for ((р0, п0), (р1, п1)), n in sorted(итог.дилеры.items(), key=lambda x: (-x[1], x[0])):
        print(f"  {р0}: {п0:<32} → {р1}: {п1:<28} {n:>9,}")
    сменили = sum(n for ((р0, _), (р1, _)), n in итог.дилеры.items() if р0 != р1)
    print(f"  сменили роль: {сменили:,} (было прямое → трейдер: "
          f"{sum(n for ((р0, _), (р1, _)), n in итог.дилеры.items() if р0 == offer_role.ПРЯМОЕ and р1 != р0):,}); "
          f"сменили только причину: {sum(итог.дилеры.values()) - сменили:,}")

...
СТРАНИЦА_БИБЛИОТЕКИ = ROOT / "public" / "library.html"


def требуют_проверки_страницы(html: str) -> frozenset:
    """Уровни, которые страница складывает в «Требуют проверки» — читается из
    самой страницы, а не зеркалом: проверка l.conf_hidden меряет расхождение
    данных с тем, что страница показывает. Два вида правила: pendingCount по
    VERIFIED_LEVELS («всё, что не проверено», с 26.09.2026) либо прежняя сумма
    слагаемых quality.<уровень>. Ни того ни другого — пустое множество: страница
    не считает ничего, и каждый непроверенный уровень скрыт."""
    html = re.sub(r"^\s*//.*$", "", html, flags=re.M)
    слагаемые = frozenset(re.findall(r"quality\.(\w+)\|\|0", html))
    m = re.search(r"const VERIFIED_LEVELS=\[([^\]]*)\]", html)
    if m and re.search(r"pending=pendingCount\(quality\)", html) and not слагаемые:
        return УРОВНИ_ПРОВЕРКИ - frozenset(re.findall(r"'([^']+)'", m.group(1)))
    return слагаемые


def проверено_страницы(html: str) -> frozenset | None:
    """Список VERIFIED_LEVELS страницы: зеркало ПРОВЕРЕНО обязано с ним совпадать."""
    m = re.search(r"const VERIFIED_LEVELS=\[([^\]]*)\]", html)
    return frozenset(re.findall(r"'([^']+)'", m.group(1))) if m else None
...
def скрываемые_столбцы_страницы(html: str) -> frozenset:
    """Поля списка, чей столбец страница прячет, когда в выборке он пуст у всех
    строк (СКРЫВАЕМЫЕ_СТОЛБЦЫ в public/nomenclature.html), — читается из самой
    страницы, а не зеркалом: n.col_dead обвиняет столбец, который страница
    рисует в каждой строке, а не тот, что она скрывает. Перечня нет — пустое
    множество, и каждый редкий столбец считается мёртвым."""
    html = re.sub(r"^\s*//.*$", "", html, flags=re.M)
    m = re.search(r"var СКРЫВАЕМЫЕ_СТОЛБЦЫ\s*=\s*\[([^\]]*)\]", html)
    return frozenset(re.findall(r'"([^"]+)"', m.group(1))) if m else frozenset()
...
def ревизия_поставщиков(с: Снимки, сейчас) -> Вкладка:
    ...
    if рёберные:
        ...
        т.доля("u.pos_choice", sum(1 for p in рёберные if crossref.выбор_позиции(p) >= 2), n)
...
ПРОВЕРКИ_НОМЕНКЛАТУРЫ = {
    ...
    "n.cmp_co": "«сравнимы» при одном поставщике (сущности)",
    "n.cmp_basket": "«сравнимы», а по правилу сборки (crossref.сравнима) в корзине нет двух поставщиков в одной валюте и единице",
    "n.cmp_miss": "не «сравнимы», а по правилу сборки корзина сравнима",
    "n.sup": "поставщиков (sup) не меньше карточек Битрикса (co), меньше одного или не сходится с корзиной",
    ...
    "n.col_dead": "столбец списка «Машины» или «Аналоги» заполнен меньше чем у 5 % позиций, а страница рисует его в каждой строке",
...
ПОЛЬЗА_НОМЕНКЛАТУРЫ = {
    ...
    "u.choice": "выбор: два и больше поставщика (сущности реестра)",
    "u.one_co_two": "(справочно) два КП одной компании — не выбор",
    "u.cmp": "сравнимы по цене: два поставщика в одной валюте и единице",
    ...
    "u.date_doc": "дата предложения — дата КП, а не день разбора или запроса",
    ...
    "u.o_dealer": "(справочно) предложения официальных дилеров бренда (дилер разведки, сведённый доменом или ИНН)",
...
def ревизия_номенклатуры(с: Снимки, сейчас, прошлые: Снимки | None = None, словарь=None,
                         страница: str | None = None) -> Вкладка:
    т = Вкладка("nomenclature", "Номенклатура (/nomenclature)", ПРОВЕРКИ_НОМЕНКЛАТУРЫ,
                ПОЛЬЗА_НОМЕНКЛАТУРЫ)
    if страница is None:
        страница = СТРАНИЦА_НОМЕНКЛАТУРЫ.read_text(encoding="utf-8")
    скрываемые = скрываемые_столбцы_страницы(страница)
    hdr = с.json(crossref.КЛЮЧ)
    т.снимок(crossref.КЛЮЧ, с, сейчас, (hdr or {}).get("published_at"))
    части = [с.json(k) for k in crossref.КЛЮЧИ_СПИСКА]
    корзины = _корзины(с)
    ...
    пересчёт = {
        ...
        "with_choice": sum(1 for p in позиции if crossref.выбор_позиции(p) >= 2),
    ...
    for p in позиции:
        ...
        # СРАВНИМОСТЬ И ВЫБОР — ПРАВИЛОМ СБОРКИ (crossref.сравнима, поставщиков):
        # зеркало правила здесь разошлось бы со сборкой при первой её поправке.
        # Корзина сверяется, только когда в ней все предложения позиции.
        полная = offers <= crossref.ПРЕДЛОЖЕНИЙ_НА_ПОЗИЦИЮ and bool(det)
        if "sup" in p:
            т.счёт("n.sup", not целое(p["sup"]) or not 1 <= p["sup"] < co)
        if полная:
            т.счёт("n.sup", crossref.поставщиков(список) != crossref.выбор_позиции(p))
        if p.get("cmp"):
            т.счёт("n.cmp_co", crossref.выбор_позиции(p) < 2)
            if полная:
                т.счёт("n.cmp_basket", not crossref.сравнима(список))
        elif полная:
            т.счёт("n.cmp_miss", crossref.сравнима(список))
        if p.get("cat"):
    ...
    # Мёртвый столбец: «Машины» и «Аналоги» заполнены только у позиций каталога.
    # Дефект — редкий столбец, который страница рисует в каждой строке; столбец
    # из её СКРЫВАЕМЫЕ_СТОЛБЦЫ (пусто — «—», пуст у всей выборки — скрыт) —
    # нет (с 26.09.2026). Проверка держится за поведение страницы, а не зеркало.
    if позиции:
        for поле in ("models", "alts"):
            т.счёт("n.col_dead", поле not in скрываемые
                   and sum(1 for p in позиции if p.get(поле)) < 0.05 * len(позиции))
    ...
    if n:
        ...
        без_выбора = [p for p in позиции if crossref.выбор_позиции(p) <= 1]
        ...
        т.доля("u.choice", sum(1 for p in позиции if crossref.выбор_позиции(p) >= 2), n)
    ...
    if предложения:
        ...
        # Дата из самого КП, а не день разбора и не день запроса (поле z сборки).
        т.доля("u.date_doc", sum(1 for o in предложения if o.get("d") and not o.get("z")), m)
        ...
        if с_ролью:
            ...
            # Официальный дилер — трейдер (решение владельца 26.09.2026), но
            # отдельной причиной: доля показывает, сколько дала связь дилеров
            # разведки с реестром компаний (library/dealer_link.py).
            код_дилера = crossref.код_роли({"role": offer_role.ТРЕЙДЕР, "why": "официальный дилер"})
            т.доля("u.o_dealer", sum(1 for o in с_ролью if o["h"] == код_дилера), len(с_ролью))
...
ПРОВЕРКИ_БИБЛИОТЕКИ = {
    ...
    "l.conf_hidden": "«Требуют проверки» на странице не считает непроверенный уровень (hypothesis, partial, medium…)",
    ...
    "l.oem_rule": "бренд компонента ≠ бренду общего правила /p и /brands (crossref.бренды_каталога) для того же написания",
...
def бренд_не_по_правилу(поля, проекция, реестр, lib2) -> bool:
    """Бренд компонента на /library расходится с общим правилом для того же
    написания: бренд, узнанный словарём, отсутствует в проекции library_brand
    публикатора, назван там другим ключом или именем или не помечен узнанным
    (страница не ведёт его на /p и /brands). Не узнанное словарём
    не судится: реестр базы знает больше файла."""
    ожидаемо = lib2.component_brand(поля, реестр)
    узнанные = [b for b in (ожидаемо or {}).get("brands", []) if b["known"]]
    if not узнанные:
        return False
    if not isinstance(проекция, dict) or проекция.get("rule") != lib2.BRAND_RULE:
        return True
    # Узнанный бренд обязан быть узнан и в проекции: иначе страница покажет
    # слово без ссылки на /p#brand= и /brands#b=.
    есть = {(b.get("key"), b.get("name")) for b in проекция.get("brands") or []
            if isinstance(b, dict) and b.get("known") is True}
    return any((b["key"], b["name"]) not in есть for b in узнанные)
...
def ревизия_библиотеки(с: Снимки, сейчас, словарь, lib2=None, страница: str | None = None) -> Вкладка:
    ...
    # Общее правило бренда ячейки изготовителя — то, чем /p и /brands называют
    # написание (crossref.бренды_каталога по словарю). Публикатор /library кладёт
    # его итог в проекцию library_brand; реестр базы может узнать больше словаря,
    # поэтому сверяются бренды, которые узнаёт словарь: их ключ и имя обязаны
    # совпасть.
    реестр_общий = crossref.реестр_брендов(словарь or {}, разложение=brands.разложение_словаря(словарь or {}))
    ...
    for s in статьи:
        ...
        if вид == "component":
            ...
            определён = False
            if isinstance(oem, str) and oem.strip():
                т.счёт("l.oem_rule", бренд_не_по_правилу(f, src.get(lib2.BRAND_PROJECTION), реестр_общий, lib2))
            if oem:
...
def ревизия(с: Снимки, сейчас, словарь=None, прошлые: Снимки | None = None, lib2=None,
            ...
            субпоставщики=None, страница_номенклатуры: str | None = None) -> list[Вкладка]:
    ...
    шаги = [("suppliers", "Поставщики (/suppliers)", lambda: ревизия_поставщиков(с, сейчас)),
            ("nomenclature", "Номенклатура (/nomenclature)", lambda: ревизия_номенклатуры(
                с, сейчас, прошлые, словарь, страница_номенклатуры)),

...
ПРОВЕРКИ_СЛОВАРЯ = {
    ...
    "d.desc": "имя записи — описание: пояснение в скобках, тире, двоеточие, «по/для/или», 6+ слов (без правовой формы)",
    ...
    "d.multi": "несколько брендов в одной записи («/», «или», «,» — не считая «, Inc», «Co., Ltd.» и скобок)",
...
def ревизия_словаря(словарь, корень: Path = ROOT) -> Вкладка:
    ...
    for r in записи:
        ...
        длинный = т.счёт("d.desc", похоже_на_описание(имя))
        ...
        # Запятая в скобках — расшифровка («Jichai (Jinan Diesel, CNPC)»), а
        # «, Inc» и «Co., Ltd.» — правовая форма: ни то ни другое не второй бренд.
        несколько = т.счёт("d.multi", bool(re.search(r"/|,|\sили\s", re.sub(r"\([^)]*\)", "", без_формы(имя)))))

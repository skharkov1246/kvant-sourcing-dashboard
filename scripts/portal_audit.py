...
отдельный наследник с единственным ключом в закрытом списке. Прошлый audit:v1
...
from library import brand_registry, brands, codes_sql, company_names, crossref, doc_side, docfilter  # noqa: E402
...
ПРЕДЕЛ_РЕВИЗИИ = 1 << 20
# Предел API Cloudflare — 1200 запросов за 5 минут на токен. Пауза держит
# ревизию около 1000, остаток — публикаторам, которые могут идти рядом.
ПАУЗА_KV = 0.3
...
ISO_ВАЛЮТЫ = frozenset(("RUB", "USD", "EUR", "CNY", "GBP", "CHF", "JPY", "AED", "KZT", "BYN",
                        "TRY", "INR", "KRW"))
ИНКОТЕРМС = frozenset(("EXW", "FCA", "FAS", "FOB", "CFR", "CIF", "CPT", "CIP", "DAP", "DPU",
                       "DDP", "DAT"))
...
# «Восток» у него — «сток». Границы \m и \M PostgreSQL переведены в Python.
НЕ_КОМПАНИЯ = re.compile(codes_sql.NOT_A_COMPANY.replace("\\m", r"(?<!\w)").replace("\\M", r"(?!\w)"), re.I)
...
# Словарь брендов: указание закупки вместо бренда (ключ записи содержит).
УКАЗАНИЯ_СЛОВАРЯ = ("заказпоспецификации", "потипу", "пропрофилю", "типовые",
                    "любойдистрибьютор", "закупка", "ремонт", "реверс")
...
def мусор_бренда(s) -> bool:
    t = str(s or "")
    return ((len(t) > 30 and bool(re.search(r"[а-яё]", t, re.I))) or служебное(t)
...
ПРОВЕРКИ_ПОСТАВЩИКОВ = {
    "s.present": "снимок suppliers:v1 есть и читается",
    "s.version": "version = 1 (иначе воркер отвечает 503)",
    "s.published": "published_at — ISO-8601 с Z",
    "s.age": f"снимку не больше {ВОЗРАСТ_РЕЕСТРА_Д} дней (публикуется только руками)",
    "s.cross_age": "реестр и котировки собраны с разницей не больше суток",
    "s.totals": "счётчики totals.* сходятся с пересчётом по записям",
    "s.review": "review_open не меньше очереди ИНН",
    "s.review_tail": "review_open > 0 при пустой очереди ИНН — хвост прежних прогонов",
    "s.caveat": "оговорка о склейке есть, и её число сходится с записями",
    "s.number_fmt": "номер KV-S/G-NNNNNN-C с верной контрольной цифрой",
    ...
    "s.name_dup": "одно имя у двух и более записей (несведённый дубль)",
    ...
    "s.inn_sum": "ИНН не 10/12 цифр или не сходится контрольная сумма",
    ...
    "s.domain_dup": "один домен у двух и более записей",
    ...
    "q.stale": "очередь ИНН: имя совпадает с записью, у которой номер уже есть",
    "x.present": "котировки: заголовок и 8 частей списка на месте",
    ...
    "x.pname": "наименование позиции пусто, равно коду или склеено с числами",
    "x.edge_price": "цена ребра ≤ 0 или ≥ 1e14",
    "x.edge_cur": "цена без валюты или валюта не из трёх заглавных латинских",
    "x.edge_date": "дата ребра не разбирается, в будущем или раньше 2015",
...
def ревизия_поставщиков(с: Снимки, сейчас) -> Вкладка:
    ...
    имена = collections.Counter(имя_ключ_компании(e.get("name")) for e in ents
    ...
    for e in ents:
        ...
        т.счёт("s.name_dup", имена[имя_ключ_компании(имя)] > 1)
    ...
    # Домены.
    по_домену = collections.Counter(d for e in ents for d in set(_домены(e)))
    for e in ents:
        for d in _домены(e):
            т.счёт("s.domain_host", not ХОСТ.fullmatch(d), форма=d)
            т.счёт("s.domain_mail", d.lower() in ПОЧТОВЫЕ)
            т.счёт("s.domain_dup", по_домену[d] > 1)
    ...
    # Очередь «Ждут ИНН».
    имена_с_номером = {имя_ключ_компании(e.get("name")) for e in ents if e.get("number")}
    ...
    for q in очередь:
        ...
        for n in names:
            ...
            т.счёт("q.stale", имя_ключ_компании(n) in имена_с_номером)
    ...
    for p in позиции:
        ...
        т.счёт("x.pn_class", класс_не_кода(n) in ("марка", "размер", "стандарт"), форма=n)
        т.счёт("x.pk_short", len(k) < 3 or bool(re.fullmatch(r"\d{1,3}", k)))
        имя = p.get("name")
        т.счёт("x.pname", (not имя or ключ_кода(имя) == k
                           or bool(re.search(r"\d+(?:[.,]\d+)?\s+\d+(?:[.,]\d+)?\s*$", str(имя)))))
        for r in рёбра:
            ci = r[0]
...
ПРОВЕРКИ_НОМЕНКЛАТУРЫ = {
    "n.keys": "ключ снимка есть в KV (заголовок, 8 частей, 32 корзины)",
    "n.published": "published_at заголовка — ISO-8601 с Z",
    "n.age": f"заголовку не больше {ВОЗРАСТ_НОМЕНКЛАТУРЫ_Ч} ч (ночная сборка пропущена)",
    "n.part_date": "дата части или корзины ≠ дате заголовка (смесь сборок)",
    "n.version": "version = 1, lists = 8, parts = 32, positions в заголовке нет",
    "n.totals": "итоги totals.* сходятся с пересчётом по частям",
    ...
    "n.co_ent_noname": "компания: ent без имени («карточка N в Битриксе»)",
    ...
    "n.n_words": "номер из 4+ слов — описание, а не номер",
    "n.n_key": "ключ номера ≠ ключу позиции (сцепка с чужой деталью, долей)",
    "n.name_empty": "наименование пусто",
    "n.name_glued": "наименование — склейка ячеек (два десятичных числа подряд)",
    ...
    "n.dem_glued": "слипшийся спрос: 30+ сделок у короткого или бесцифрового ключа",
    ...
    "n.e_date": "ребро: дата не ГГГГ-ММ-ДД или позже сборки",
    ...
    "n.oem_file_legal": "изготовитель со слов поставщика: > 60 знаков или правовая форма",
    ...
    "n.alt_class": "аналог: номер — марка или стандарт",
    ...
    "n.o_price": "предложение: цена ≤ 0 или > 1e8",
    "n.o_cur": "предложение: валюта вне ISO-списка",
    ...
    "n.o_total": "предложение: сумма ≤ 0 или меньше цены",
    "n.o_t_noq": "предложение: сумма без количества, и сумма/цена не целое",
    "n.o_r": "предложение: пометки условий не из четырёх букв «сфнм?» или противоречат значению",
    "n.o_basis": "предложение: базис не Incoterms",
    ...
    "n.o_m": "предложение: изготовитель по файлу — цифры, служебное, правовая форма, > 60",
    "n.o_date": "предложение: дата не ГГГГ-ММ-ДД, позже сборки или раньше 2015",
...
def ревизия_номенклатуры(с: Снимки, сейчас, прошлые: Снимки | None = None) -> Вкладка:
    ...
    for ci, c in enumerate(companies):
        ...
        if имя:
            ...
            if ent:
                т.счёт("n.co_name_dup", len(ent_по_имени[норм(имя)]) > 1)
        if ent:
    ...
    нбр = бренды_брендов(bv)
    будущее = собран.date() if собран else сейчас.date()
    for p in позиции:
        k = str(p.get("k") or "")
        ...
        if not т.счёт("n.n_empty", not n):
            т.счёт("n.n_class", класс_не_кода(n) is not None or docfilter._rubbish(str(n)), форма=n)
            т.счёт("n.n_words", len(str(n).split()) > 3)
            т.счёт("n.n_key", ключ_кода(n) != k)
        имя = p.get("name")
        if not т.счёт("n.name_empty", not имя):
            имя = str(имя)
            т.счёт("n.name_glued", bool(re.search(r"\d+[.,]\d+\D+\d+[.,]\d+", имя)))
        ...
        if isinstance(d, dict):
            ...
            т.счёт("n.dem_glued", (d.get("deals") or 0) >= 30
                   and (len(k) <= 5 or not re.search(r"\d", k)))
            if d.get("unit_name"):
                ед = str(d["unit_name"])
        ...
        if p.get("oem_cat"):
            oc = p["oem_cat"]
            т.счёт("n.oem_cat", только_цифры(oc) or company_names.как_ключ(oc) or служебное(oc))
        of = [x for x in p.get("oem_file") or [] if isinstance(x, str)]
        for x in of:
            т.счёт("n.oem_file", только_цифры(x) or служебное(x))
            т.счёт("n.oem_file_legal", len(x) > 60 or правовая_форма_в(x))
        if of:
            т.счёт("n.oem_file_case", len({норм(x) for x in of}) < len(of))
        br = [x for x in p.get("brands") or [] if isinstance(x, str)]
        for x in br:
            т.счёт("n.brands_digits", bool(re.fullmatch(r"\d+", x.strip())))
            if bv is not None:
        ...
        for a in p.get("alts") or []:
            ...
            if a.get("maker"):
                т.счёт("n.alt_maker", только_цифры(a["maker"]) or company_names.как_ключ(a["maker"]))
            if pn:
                т.счёт("n.alt_class", класс_не_кода(pn) in ("марка", "стандарт"))
    ...
    for k, det in подробно.items():
        ...
        for o in список:
            ...
            if p_ is not None:
                т.счёт("n.o_price", not число(p_) or p_ <= 0 or p_ > 1e8)
            if u is not None:
                т.счёт("n.o_cur", u not in ISO_ВАЛЮТЫ or not ВАЛЮТА_RX.fullmatch(str(u)))
            ...
            if t_ is not None:
                т.счёт("n.o_total", not число(t_) or t_ <= 0 or (число(p_) and t_ < p_))
            ...
            if o.get("s") is not None:
                т.счёт("n.o_basis", str(o["s"]).split()[0].upper() not in ИНКОТЕРМС if str(o["s"]).split() else True)
            for поле in ("l", "k"):
            ...
            for b in o.get("b") or []:
                т.счёт("n.o_b_digits", bool(re.fullmatch(r"\d+", str(b).strip())))
            if o.get("m") is not None:
                m = str(o["m"])
                имя_c = (по_co.get(str(c)) or {}).get("name")
                т.счёт("n.o_m", только_цифры(m) or служебное(m) or правовая_форма_в(m) or len(m) > 60
                       or (bool(имя_c) and норм(имя_c) == норм(m)))
        ...
        for m in makers:
            т.счёт("n.mk", not m.get("name") or company_names.как_ключ(m.get("name"))
    ...
    if n:
        def правдоподобный_бренд(x):
            return bool(x) and not только_цифры(x) and not служебное(x) and not company_names.как_ключ(x)
...
ПРОВЕРКИ_БРЕНДОВ = {
    "b.keys": "ключ снимка есть в KV (сводка, связи, пары, 16 корзин)",
    "b.published": "published_at сводки — ISO-8601 с Z",
    "b.age": f"сводке не больше {ВОЗРАСТ_БРЕНДОВ_Ч} ч (ночной шаг не отработал)",
    ...
    "b.k_desc": "бренд: ключ-описание (кириллица > 30) или в имени больше 6 слов",
    "b.name_digits": "бренд: номер вместо имени",
    "b.name_has_digit": "бренд: в имени цифра (модель или марка)",
    ...
    "b.models_shared": "машина у двух брендов (ячейка «A/B»)",
    "b.units": "бренд: узлы противоречат (undefined > parts, пустой список, критичность не A/B/C)",
    "b.units_undef": "бренд: узел не определён у больше чем 21 % деталей",
    ...
    "b.s_name_json": "поставщик: имя — JSON-массив",
    ...
    "b.c_class": "код — марка, размер или стандарт",
    ...
    "b.c_words": "в написании кода больше двух слов (наименование вместо номера)",
    "b.c_brandkey": "код равен ключу бренда (столкновение колонок)",
    "b.c_long": "код длиннее 25 знаков (склейка)",
    "b.c_dup": "код повторяется в указателе",
    "b.c_written": "ключ написания ≠ коду",
    ...
    "b.o_suspect": "цена: мин = количеству, год вместо цены или слишком велика за штуку",
    ...
    "b.o_basis": "базис не Incoterms или длиннее 60",
    "b.o_dates": "даты: первая позже последней, в будущем или раньше 2015",
...
def ревизия_брендов(с: Снимки, сейчас) -> Вкладка:
    ...
    for b in бренды_:
        ...
        т.счёт("b.k_desc", (len(str(k)) > 30 and bool(re.search(r"[а-я]", str(k)))) or len(имя.split()) > 6)
        т.счёт("b.name_digits", bool(re.fullmatch(r"[\d\s,.;]+", имя)) or bool(re.fullmatch(r"\d+", str(k))))
        т.счёт("b.name_has_digit", bool(re.search(r"\d", имя)) and not только_цифры(имя))
        ...
        for часть in (b.get("units") or {}).values():
            ...
            if часть.get("parts"):
                т.счёт("b.units_undef", (часть.get("undefined") or 0) / часть["parts"] > 0.21)
        parts = b.get("parts")
        ...
        if isinstance(al, dict):
            ...
            for m in makers:
                т.счёт("b.alts_maker", company_names.как_ключ(m[0]) or только_цифры(m[0]))
        ch = b.get("chain")
    ...
    for ид, у_брендов in машины_брендов.items():
        т.счёт("b.models_shared", len(у_брендов) > 1)
    ...
    for s in поставщики_:
        ...
        т.счёт("b.s_name_json", имя.startswith("[") or '"' in имя)
    ...
    for к in коды_указателя:
        ...
        т.счёт("b.c_class", класс_не_кода(код) is not None or (написано is not None and класс_не_кода(написано) is not None),
               форма=код)
        if написано is not None:
            ...
            т.счёт("b.c_words", len(w.split()) > 2)
    ...
    for код, (_, det) in подробно.items():
        ...
        for o in offers:
            ...
            if mn is not None or mx is not None:
                ...
                т.счёт("b.o_suspect", (число(o.get("qty")) and o["qty"] > 1 and mn == o["qty"])
                       or (o.get("rows") == 1 and число(mn) and mn == int(mn) and 1990 <= mn <= 2035)
                       or (штучно and число(md) and ((cur == "RUB" and md > 1e8)
                                                     or (cur in ("USD", "EUR", "CNY") and md > 1e7))))
            ...
            if o.get("basis") is not None:
                б = str(o["basis"])
                т.счёт("b.o_basis", len(б) > 60 or not б.split() or б.split()[0].upper().strip(",.") not in ИНКОТЕРМС)
...
ПРОВЕРКИ_СЧЁТЧИКОВ = {
    "c.present": "снимок counters:v1 есть и читается",
    "c.version": "version = 1 (иначе страница «Не открылось»)",
    "c.dropped": "публикатор отбросил нечисловые значения (dropped > 0)",
    "c.size": "размер снимка больше 100 КиБ (ошибка публикатора)",
    "c.metric": "замер «коды_и_цены» есть и в нём есть точка с asked",
        ...
        for t in точки:
            ...
            if note is not None:
                т.счёт("c.note", len(str(note)) > 500 or "@" in str(note) or "bitrix24" in str(note)
                       or bool(re.search(r"(сделк\w*|№)\s*\d{4,6}\b", str(note), re.I)))
...
ПРОВЕРКИ_БИБЛИОТЕКИ = {
    "l.present": "указатель library:v2:current есть и читается",
    "l.pointer": "указатель: version 2, ревизия по правилу ID, ссылка на манифест",
    "l.manifest": "манифест читается и сходится по sha256 и ревизии",
    ...
    "l.body_junk": "в отрывке undefined/None/NaN/null, сырой JSON или HTML-теги",
    "l.body_table": "таблица Markdown: у строк другое число ячеек, чем в шапке",
    "l.body_links": "ссылка в тексте не http(s) и не относительная",
    "l.conf": "уровень проверки ни строкой, ни объектом {level}",
    "l.conf_norefs": "«проверено», а источников нет",
    "l.pn_class": "артикул — марка, размер или стандарт",
    "l.pn_bad": "артикул неправдоподобен: короче 4, длиннее 25, без цифры, дата, пункт, список",
    ...
    "l.oem_queue": "бренда нет в словаре dict/oem.json",
    "l.oem_instruction": "бренд разрешился в указание закупки, а не в марку",
    ...
    "l.s_name": "поставщик: имя-ключ, цифры, контакт вместо имени или бренд вместо поставщика",
    ...
    "l.p_amount": "цена: не число по форме, ≤ 0 или ≥ 1e9",
    "l.p_cur": "цена: валюта не ISO или цена без валюты",
    "l.p_date": "цена: дата не разбирается, в будущем, раньше 2000 или «до» раньше даты",
    "l.p_type": "цена: тип или назначение вне списка",
    "l.p_supplier": "цена: источник — заглушка или бренд",
    "l.p_pn": "цена: артикул неправдоподобен или без компонента в сегменте",
    "l.p_unit": "цена: единица расходится с компонентом",
...
ПОЛЬЗА_БИБЛИОТЕКИ = {
    "u.brand": "компоненты с определённым брендом (разрешён по словарю в марку)",
...
def указание_закупки(ключ: str) -> bool:
    return any(x in ключ for x in УКАЗАНИЯ_СЛОВАРЯ)
...
def ревизия_библиотеки(с: Снимки, сейчас, словарь, lib2=None) -> Вкладка:
    ...
    def оценка_бренда(oem):
        ...
        if состояние == brand_registry.ОЧЕРЕДЬ:
            return "l.oem_queue", состояние
        if any(указание_закупки(k) for k in ключи):
            return "l.oem_instruction", состояние
        return None, состояние

    # Польза: копим по ходу.
    с_брендом = спорных = код_и_бренд = с_описанием = с_назначением = 0
    ...
    for s in статьи:
        ...
        т.счёт("l.body_junk", bool(re.search(r"\b(undefined|None|NaN|null)\b|^\s*\{\"|</?[a-z][^>]*>", текст, re.M)))
        строки_таблицы = [x for x in текст.splitlines() if x.strip().startswith("|")]
        if len(строки_таблицы) >= 2 and (полная is not None or not текст.endswith(строки_таблицы[-1])):
        ...
        if вид == "component":
            ...
            if pn:
                pn_s = str(pn)
                т.счёт("l.pn_class", класс_не_кода(pn_s) is not None, форма=ключ_кода(pn_s))
                k = ключ_кода(pn_s)
                т.счёт("l.pn_bad", len(k) < 4 or len(k) > 25 or not re.search(r"\d", k)
            ...
            if oem:
                ...
                for код_ in ("l.oem_digits", "l.oem_key", "l.oem_unknown", "l.oem_multi", "l.oem_queue",
                             "l.oem_instruction"):
                    т.счёт(код_, дефект == код_)
                определён = дефект is None
            ...
            if имя:
                ...
                т.счёт("l.s_name", company_names.как_ключ(имя) or bool(re.fullmatch(r"[a-z]{13,}|bitrix:\d+|[\d ,]+", str(имя)))
                       or bool(re.search(r"@|https?://|\+?\d[\d\s()-]{8,}\d", str(имя)))
                       or (состояние == brand_registry.РАЗРЕШЕНО and codes_sql.ключ_написания(имя) in ключи))
            ...
            if amount is not None:
                ...
                т.счёт("l.p_amount", not ок or not 0 < float(amount) < 1e9)
            cur = f.get("currency")
            ...
            for поле in ("price_date", "valid_until", "quote_valid_until"):
                if f.get(поле):
                    ...
                    т.счёт("l.p_date", d is None or d.year < 2000
                           or (поле == "price_date" and d > сейчас.date())
            ...
            if f.get("part_number"):
                ...
                if comp and comp.get("unit") and f.get("unit"):
                    т.счёт("l.p_unit", норм(comp["unit"]) != норм(f["unit"]))
...
ПРОВЕРКИ_СЛОВАРЯ = {
    ...
    "d.key_long": "ключ длиннее 30 знаков — описание, а не имя",
    "d.key_40": "ключ ровно 40 знаков — обрезка nkey, риск склейки",
    "d.key_cyr": "кириллица в ключе при латинском имени — пометка по профилю",
    "d.instruction": "указание закупки вместо бренда («по спецификации», «по типу», «ремонт»…)",
    "d.multi": "несколько брендов в одной записи («/», «или», «,»)",
    "d.key_digits": "в ключе пять и больше цифр — номер детали вместо бренда",
...
ПОЛЬЗА_СЛОВАРЯ = {
    "u.real": "записи — настоящие бренды (не указание, не несколько, не номер, ключ ≤ 30)",
}
...
def ревизия_словаря(словарь, корень: Path = ROOT) -> Вкладка:
    ...
    for r in записи:
        ...
        длинный = т.счёт("d.key_long", len(k) > 30)
        т.счёт("d.key_40", len(k) == 40)
        т.счёт("d.key_cyr", bool(re.search(r"[а-я]{4,}", k)) and bool(re.search(r"[A-Za-z]", имя)))
        указание = т.счёт("d.instruction", указание_закупки(k))
        несколько = т.счёт("d.multi", bool(re.search(r"/|,|\sили\s", имя)))
        номер = т.счёт("d.key_digits", len(re.findall(r"\d", k)) >= 5)
        if not (указание or несколько or номер or длинный):
            настоящих += 1
        for x in r.get("spellings") or []:
...
def ревизия(с: Снимки, сейчас, словарь=None, прошлые: Снимки | None = None, lib2=None,
    ...
    шаги = [("suppliers", "Поставщики (/suppliers)", lambda: ревизия_поставщиков(с, сейчас)),
            ("nomenclature", "Номенклатура (/nomenclature)", lambda: ревизия_номенклатуры(с, сейчас, прошлые)),
...
def печать(вкладки: list[Вкладка], прошлая=None, поток=None):
    ...
    for т in вкладки:
        ...
        p(f"  {'проверка':<14} {'подпись':<78} {'проверено':>10} {'дефектных':>10} {'доля':>8}"
          + ("  было" if было else ""))
        for к, (проверено, дефектных) in т.счета.items():
            ...
            if было and (т.ид, к) in было:
                хвост = f"  {было[(т.ид, к)]}"
            p(f"  {к:<14} {т.подписи[к]:<78} {проверено:>10} {дефектных:>10} "
              f"{доля_текстом(дефектных, проверено):>8}{хвост}")
            if к in т.формы:
                p("  " + " " * 15 + "образцы: " + ", ".join(f"{ф} ×{n}" for ф, n in т.формы[к].most_common(5)))
        ...
        for з in т.заметки:
            p(f"  · {з}")
        if т.доли:
            p(f"  {'польза':<14} {'подпись':<78} {'числитель':>10} {'из':>10} {'доля':>8}")
            for к, (ч, з) in т.доли.items():
                p(f"  {к:<14} {т.подписи_пользы[к]:<78} {ч:>10} {з:>10} {доля_текстом(ч, з):>8}")
    p("\n═══ СВОДКА: где работа ═══")
    p(f"  {'вкладка':<14} {'проверок':>9} {'с дефектом':>11} {'ср. доля дефектов':>18} {'ср. польза':>11}")
    порядок = sorted(вкладки, key=lambda т: (-(т.средняя_доля_дефектов() or 0), т.средняя_польза() or 1))
    for т in порядок:
...
def main(argv=None) -> int:
    ...
    try:
        ...
        return 1
    if КЛАСС_НЕ_КОДА is None:
        print("⚠ docfilter.класс_не_кода недоступен: проверки «марка/размер/стандарт» молчат")
    ...
    if os.environ.get("AUDIT_APPLY") == "1":
        if not isinstance(источник, ИсточникKV):
            print("::error::AUDIT_APPLY=1 с --from-dir: итог по файлам в живой KV не пишется")
            return 1
        if только:
            # Частичный итог затёр бы полный: страница показала бы одну вкладку
            # как всю ревизию.
            print("::error::AUDIT_APPLY=1 с --only: частичная ревизия в audit:v1 не пишется")
            return 1
        try:

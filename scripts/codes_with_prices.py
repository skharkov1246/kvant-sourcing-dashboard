#!/usr/bin/env python3
"""Сколько КОДОВ у нас с ценой и сколько без. Прямой ответ на вопрос владельца.

ЗАЧЕМ ОТДЕЛЬНЫЙ ЗАМЕР. Вопрос «сколько кодов с ценами и без» имеет три разных
ответа, и подменять один другим значит соврать:

  · КОДЫ, КОТОРЫЕ МЫ СПРАШИВАЛИ (спрос). Их и надо закрывать ценой — это наша
    работа. Код без цены здесь = позиция, по которой мы вышли на рынок и остались
    без ответа;
  · КОДЫ, ПО КОТОРЫМ ЕСТЬ ЦЕНА ПРЕДЛОЖЕНИЯ. Среди них бывают такие, которых мы не
    спрашивали: поставщик прислал прайс шире запроса;
  · КОДЫ КАТАЛОГА. Это наша библиотека деталей, и она пересекается с первыми двумя
    лишь частично.

Четыре ловушки, каждая даёт правдоподобное, но неверное число:

1. КОД — ЭТО КЛЮЧ, А НЕ СТРОКА. «AAA-111», «AAA 111» и «aaa111» — один код.
   Считая строки, получишь втрое больше кодов, чем есть (lib_pn_key).
2. СПРОС СЧИТАЕТСЯ ПО lib_demand_live, А НЕ lib_demand. Пункты договоров, попавшие
   в спрос и уже помеченные мусором, иначе станут «кодами без цены» и раздуют
   знаменатель.
3. ЦЕНА БЫВАЕТ НЕ ТОЛЬКО ИЗ ПРЕДЛОЖЕНИЯ. lib_prices держит и прайсы, и таможню.
   «Есть цена» по всем потокам и «есть цена от поставщика в ответ на наш запрос» —
   разные утверждения, и оба нужны: первое говорит, чем мы располагаем, второе —
   насколько закрыты заявки.
4. НЕ ВСЯКИЙ КЛЮЧ — АРТИКУЛ. Обрывок строки тоже даёт ключ. Правдоподобность
   оценивается отдельно (ключ с цифрой, от четырёх знаков), и это ОЦЕНКА, а не
   факт: ни одна строка по ней не отбрасывается.
5. В lib_demand ЛЕЖИТ НЕ ТОЛЬКО НАШ СПРОС. Разбор предложений поставщиков кладёт
   каждую позицию КП и в lib_demand, и в lib_prices одним проходом (indexer.py,
   flush). Значит без отбора по происхождению файла «код, который мы спрашивали»
   включает коды, которых мы не спрашивали, а поставщик дописал сам, — и хуже
   того, у каждого такого кода цена есть ПО ПОСТРОЕНИЮ. Покрытие тогда меряет
   само себя: эталон становится производным от правила (CLAUDE.md, правило 1), а
   разряд «прайс шире запроса» не может быть непустым. Отбор — по lib_files.origin
   ('поле сделки' — наша спецификация, 'поле запроса' — предложение поставщика);
   колонки lib_demand.source для этого НЕ ХВАТАЕТ: распознавание сканов ставит
   'распознавание скана' и нашим файлам, и файлам поставщика.
   Поймано скептической проверкой замера 22.09.2026, до того как цифры ушли
   дальше первого разговора.

Подготовка временными таблицами с индексами, а не коррелированными подзапросами
(CLAUDE.md, правило 8): иначе соединение читает полтора миллиона строк на каждый
из ста пятидесяти тысяч ключей.

По умолчанию ничего не пишет. С WRITE=1 добавляет ОДНУ строку агрегатов в
lib_metric_runs — журнал числовых замеров, из которого берётся динамика день ко
дню. До 22.09.2026 такой истории не существовало: цифры печатались в лог прогона
Actions и пропадали через девяносто дней, а сравнивать приходилось глазами.
Точка ключуется прогоном, поэтому повторный запуск того же прогона не плодит
точки, а перезаписывает свою.

В журнал прогона и в таблицу — только агрегаты (правило 17): счёт кодов и строк.

    SUPABASE_DB_URL=… python scripts/codes_with_prices.py
    SUPABASE_DB_URL=… WRITE=1 RUN_KEY=$GITHUB_RUN_ID python scripts/codes_with_prices.py
"""
from __future__ import annotations

import os

FEED_КП = "разбор КП"

# НАПРАВЛЕНИЕ ПОТОКА ЦЕН. Найдено скептической проверкой 22.09.2026: «цена есть»
# считалось по ВСЕМ потокам lib_prices, а среди них два не закупочные.
#
#   · «ТКП КВАНТ (отпускная цена)» — НАША цена заказчику (library/load_tkp_prices.py).
#     Считать её ответом на вопрос «почём нам эта деталь» — значит закрыть позицию
#     собственной ценой продажи и получить закупочный коридор из своей же наценки;
#   · «прайсы конкурентов» — розница конкурента заказчику, тоже не наша закупка.
#
# Остальные потоки закупочные: предложение поставщика, каталожный коридор, записи о
# ценах, вилки RFQ, факт ввоза по декларации.
НАША_ИСХОДЯЩАЯ = "ТКП КВАНТ (отпускная цена)"
ЧУЖАЯ_РОЗНИЦА = "прайсы конкурентов"
НЕ_ЗАКУПОЧНЫЕ = (НАША_ИСХОДЯЩАЯ, ЧУЖАЯ_РОЗНИЦА)

# Имя замера в журнале. Оно же — ключ, по которому страница счётчика отбирает
# свои точки, поэтому менять его нельзя, не переписав историю.
ЗАМЕР = "коды_и_цены"

ЗАПИСЬ = """
insert into lib_metric_runs (metric, run_key, nums, note)
     values (%s, %s, %s::jsonb, %s)
on conflict (metric, run_key)
  do update set nums = excluded.nums, note = excluded.note,
                measured_at = now()
"""

# СТОРОНА СТРОКИ СПРОСА. Соединение с lib_files один раз, во временную таблицу:
# тот же джойн внутри каждого следующего запроса читал бы полтора миллиона строк
# по разу на запрос.
#
# Строка без разрешимого файла или без проставленной стороны — ОТДЕЛЬНОЕ
# состояние, а не «наша по умолчанию». Свалить её к заявкам значило бы вернуть
# тавтологию через чёрный ход, свалить к поставщику — потерять спрос. Считается и
# печатается отдельно.
ПОДГОТОВКА = """
create temp table строки_спроса as
  select d.id,
         lib_pn_key(d.part_number) as ключ,
         d.deal_id,
         -- «when null» тут было бы мёртвой ветвью: в CASE-по-значению NULL не
         -- совпадает ни с чем, включая NULL. Отсутствие ловится coalesce.
         coalesce(f.side, case when f.file_id is null then 'без файла'
                               else 'сторона не проставлена' end) as откуда
    from lib_demand_live d
    left join lib_files f on f.file_id = d.source_file
   where length(lib_pn_key(d.part_number)) >= 2;
create index on строки_спроса (откуда, ключ);
analyze строки_спроса;

create temp table коды_спроса as
  select ключ,
         count(*)::bigint                  as строк,
         count(distinct deal_id)::bigint   as сделок
    from строки_спроса
   where откуда = 'заказчик'
   group by 1;
create index on коды_спроса (ключ);
analyze коды_спроса;

-- Коды, пришедшие из предложений поставщиков: мы их не спрашивали. Это и есть
-- «прайс шире запроса», честно и по построению непустое.
create temp table коды_поставщика as
  select ключ, count(*)::bigint as строк
    from строки_спроса
   where откуда = 'поставщик'
   group by 1;
create index on коды_поставщика (ключ);
analyze коды_поставщика;

-- КЛЮЧ СТРОКИ ЦЕНЫ — ЭТО part_number ЛИБО part_id. Часть потоков (ценовые
-- коридоры RFQ, прайсы конкурентов, записи о ценах — library/load_parts.py)
-- пишет цену БЕЗ part_number, но с part_id: цена привязана к детали каталога, а
-- не к написанию номера. part_id считается той же нормализацией, что lib_pn_key
-- (part_key в load_parts.py), поэтому это один и тот же ключ, а не два.
--
-- Считать только по part_number значило бы молча терять такие цены: код уезжал
-- бы из разряда «цена есть, но из другого потока» в заголовочный «БЕЗ ЦЕНЫ
-- ВОВСЕ», то есть ошибка падала бы ровно на то число, которое читают первым.
-- Замер в схеме (lib_prices) говорит: part_number заполнен у 96 строк из ста.
create temp table коды_кп as
  select case when length(lib_pn_key(p.part_number)) >= 2
              then lib_pn_key(p.part_number) else p.part_id end as ключ,
         count(*)::bigint as строк
    from lib_prices p
   where p.feed = %s
     and (length(lib_pn_key(p.part_number)) >= 2 or length(coalesce(p.part_id, '')) >= 2)
   group by 1;
create index on коды_кп (ключ);
analyze коды_кп;

-- Только ЗАКУПОЧНЫЕ потоки: наша отпускная цена и розница конкурента отвечают на
-- другой вопрос и в «цена есть» не идут.
create temp table коды_цен_все as
  select case when length(lib_pn_key(p.part_number)) >= 2
              then lib_pn_key(p.part_number) else p.part_id end as ключ,
         count(*)::bigint as строк,
         count(distinct coalesce(p.feed, '(без потока)'))::bigint as потоков
    from lib_prices p
   where (length(lib_pn_key(p.part_number)) >= 2
          or length(coalesce(p.part_id, '')) >= 2)
     and coalesce(p.feed, '') <> all (%s)
   group by 1;
create index on коды_цен_все (ключ);
analyze коды_цен_все;

-- Незакупочные потоки отдельно: их надо ВИДЕТЬ, а не просто исключить. Наша
-- отпускная цена по коду — сама по себе полезный факт («эту деталь мы уже
-- продавали»), просто это не ответ на «почём её купить».
create temp table коды_не_закупочные as
  select coalesce(p.feed, '(без потока)') as поток,
         count(*)::bigint as строк,
         count(distinct case when length(lib_pn_key(p.part_number)) >= 2
                            then lib_pn_key(p.part_number) else p.part_id end)::bigint as кодов
    from lib_prices p
   where (length(lib_pn_key(p.part_number)) >= 2
          or length(coalesce(p.part_id, '')) >= 2)
     and coalesce(p.feed, '') = any (%s)
   group by 1
   order by 2 desc;

-- Сколько цен опознано только по part_id: если разряд велик, а прежний замер
-- его не видел, значит «БЕЗ ЦЕНЫ ВОВСЕ» был завышен ровно на него.
create temp table цены_только_по_id as
  select count(*)::bigint as строк, count(distinct p.part_id)::bigint as кодов
    from lib_prices p
   where length(lib_pn_key(p.part_number)) < 2
     and length(coalesce(p.part_id, '')) >= 2;

create temp table коды_каталога as
  select lib_pn_key(pt.catalog_no) as ключ
    from lib_parts pt
   where length(lib_pn_key(pt.catalog_no)) >= 2
   group by 1;
create index on коды_каталога (ключ);
analyze коды_каталога;
"""

ПРОИСХОЖДЕНИЕ = """
select откуда,
       count(*)::bigint                  as строк,
       count(distinct ключ)::bigint       as кодов
  from строки_спроса
 group by 1
 order by 2 desc
"""

ОБЪЁМ = """
select (select count(*)::bigint from коды_спроса)     as кодов_спроса,
       (select sum(строк)::bigint from коды_спроса)    as строк_спроса,
       (select count(*)::bigint from коды_кп)          as кодов_кп,
       (select sum(строк)::bigint from коды_кп)        as строк_кп,
       (select count(*)::bigint from коды_цен_все)     as кодов_цен_все,
       (select count(*)::bigint from коды_каталога)    as кодов_каталога,
       (select строк from цены_только_по_id)           as цен_по_id,
       (select кодов from цены_только_по_id)           as кодов_по_id
"""

# ГЛАВНЫЙ ОТВЕТ: коды, которые мы спрашивали, — с ценой и без.
СПРОС_С_ЦЕНОЙ = """
select count(*)::bigint                                          as всего,
       count(*) filter (where кп)::bigint                         as с_ценой_кп,
       count(*) filter (where not кп and любая)::bigint           as только_чужой_поток,
       count(*) filter (where not кп and not любая)::bigint        as без_цены_вовсе,
       sum(строк) filter (where кп)::bigint                        as строк_с_ценой_кп,
       sum(строк) filter (where not кп and not любая)::bigint      as строк_без_цены
  from (select с.ключ, с.строк,
               exists (select 1 from коды_кп к where к.ключ = с.ключ)       as кп,
               exists (select 1 from коды_цен_все ц where ц.ключ = с.ключ)  as любая
          from коды_спроса с) x
"""

# Обратная сторона: коды, по которым цена есть, — спрашивали ли мы их.
#
# Разряд «не спрашивали» тут больше не тавтологичен: коды_спроса собраны из НАШИХ
# спецификаций, а цена — из предложений, и совпадение ключей стало утверждением о
# мире, а не следствием того, что обе строки родились одним проходом разбора.
ЦЕНЫ_ПРОТИВ_СПРОСА = """
select count(*)::bigint                                           as кодов_с_ценой_кп,
       count(*) filter (where спрос)::bigint                        as спрашивали,
       count(*) filter (where not спрос)::bigint                    as не_спрашивали,
       count(*) filter (where not спрос and поставщик)::bigint      as дописал_поставщик,
       count(*) filter (where каталог)::bigint                      as есть_в_каталоге
  from (select к.ключ,
               exists (select 1 from коды_спроса с where с.ключ = к.ключ)      as спрос,
               exists (select 1 from коды_поставщика п where п.ключ = к.ключ)  as поставщик,
               exists (select 1 from коды_каталога т where т.ключ = к.ключ)    as каталог
          from коды_кп к) x
"""

КАТАЛОГ = """
select count(*)::bigint                                           as кодов_каталога,
       count(*) filter (where кп)::bigint                           as с_ценой_кп,
       count(*) filter (where спрос)::bigint                        as спрашивали
  from (select т.ключ,
               exists (select 1 from коды_кп к where к.ключ = т.ключ)     as кп,
               exists (select 1 from коды_спроса с where с.ключ = т.ключ) as спрос
          from коды_каталога т) x
"""

# ПРАВДОПОДОБНОСТЬ КЛЮЧА — ОЦЕНКА, А НЕ ФАКТ. Обрывок строки тоже даёт ключ.
# Признаки те же, что у профиля спроса: есть цифра, от четырёх знаков, не длиннее
# двадцати пяти. Ни одна строка по этой оценке не отбрасывается.
ПРАВДОПОДОБНОСТЬ = """
select count(*)::bigint                                           as всего,
       count(*) filter (where ключ ~ '[0-9]'
                          and length(ключ) between 4 and 25)::bigint as правдоподобных,
       count(*) filter (where ключ !~ '[0-9]')::bigint              as без_цифры,
       count(*) filter (where length(ключ) < 4)::bigint             as короче_четырёх,
       count(*) filter (where length(ключ) > 25)::bigint            as длиннее_25
  from коды_спроса
"""


def ч(x) -> int:
    return int(x or 0)


def доля(часть, целое) -> str:
    часть, целое = ч(часть), ч(целое)
    return f"{часть:>8d}" + (f"  {100 * часть / целое:5.1f} %" if целое else "")


def ключ_прогона() -> str:
    """Ключ точки. Прогон Actions, если мы в нём, иначе метка времени UTC.

    Прогон нужен именно номером: по нему точка находится в логах, а повторный
    запуск того же прогона перезаписывает свою строку вместо новой.
    """
    из_среды = (os.environ.get("RUN_KEY") or os.environ.get("GITHUB_RUN_ID") or "").strip()
    if из_среды:
        return из_среды
    from datetime import datetime, timezone
    return "вручную-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")


def записать(cur, числа: dict, оговорка: str | None) -> None:
    import json

    ключ = ключ_прогона()
    cur.execute(ЗАПИСЬ, (ЗАМЕР, ключ, json.dumps(числа, ensure_ascii=False), оговорка))
    print(f"\n✓ точка истории записана: замер «{ЗАМЕР}», прогон {ключ},"
          f" чисел {len(числа)}")


def main() -> int:
    dsn = os.environ.get("SUPABASE_DB_URL", "").strip()
    if not dsn:
        print("нет SUPABASE_DB_URL — мерить нечего")
        return 2
    import psycopg2

    conn = psycopg2.connect(dsn, connect_timeout=20,
                            options="-c statement_timeout=600000")
    # Числа собираются по ходу печати, а не вторым проходом запросов: второй
    # проход по полутора миллионам строк спроса дал бы ДРУГИЕ числа, если между
    # проходами идёт разбор, и точка истории разошлась бы с напечатанной.
    числа: dict[str, int] = {}
    try:
        with conn.cursor() as cur:
            print("=== КОДЫ С ЦЕНОЙ И БЕЗ ЦЕНЫ ===")
            print("Код — это КЛЮЧ номера (регистр, дефисы и пробелы сняты), а не")
            print("написание: «AAA-111», «AAA 111» и «aaa111» — один код.\n")
            cur.execute(ПОДГОТОВКА, (FEED_КП, list(НЕ_ЗАКУПОЧНЫЕ),
                                     list(НЕ_ЗАКУПОЧНЫЕ)))

            # СТОРОНА ПЕЧАТАЕТСЯ ПЕРВОЙ. Из неё видно, на чём стоят все доли
            # ниже: до 22.09.2026 замер складывал заявки заказчика, наши исходящие
            # документы и предложения поставщиков в одно число, и покрытие мерило
            # само себя.
            cur.execute(ПРОИСХОЖДЕНИЕ)
            раскладка = cur.fetchall()
            всего_строк = sum(ч(x[1]) for x in раскладка)
            print("ОТКУДА ВЗЯЛАСЬ СТРОКА СПРОСА (по СТОРОНЕ документа):")
            for откуда, строк, кодов in раскладка:
                подпись = {
                    "заказчик": "заявка заказчика — ЭТО И ЕСТЬ НАШ СПРОС",
                    "мы": "наш исходящий документ — не спрос",
                    "поставщик": "предложение поставщика — не спрос",
                    "внутренний": "наш внутренний файл (экономика) — не спрос",
                    "неизвестно": "название поля правилу неизвестно — не спрос",
                    "без файла": "файл не разрешился — не спрос",
                    "сторона не проставлена": "файл разобран до правки — нужен дозаполнитель",
                }.get(откуда, "прочее")
                print(f"    {откуда:<12} строк {доля(строк, всего_строк)}"
                      f"  · кодов {ч(кодов):>8d}  — {подпись}")
            ИМЕНА = {"заказчик": "rows_customer", "мы": "rows_ours",
                     "поставщик": "rows_supplier", "внутренний": "rows_internal",
                     "неизвестно": "rows_unknown", "без файла": "rows_nofile",
                     "сторона не проставлена": "rows_noside"}
            числа.update(**{ИМЕНА[о]: ч(с) for о, с, _ in раскладка if о in ИМЕНА})
            print()

            cur.execute(ОБЪЁМ)
            кс, стрс, ккп, стркп, кц, кт, цид, кид = cur.fetchone()
            print("ОБЪЁМ:")
            числа.update(asked=ч(кс), rows_asked=ч(стрс), price_codes=ч(ккп),
                         price_rows=ч(стркп), price_codes_any=ч(кц), catalog=ч(кт))
            print(f"    кодов в спросе ЗАКАЗЧИКА            {ч(кс):>8d}"
                  f"  · строк {ч(стрс)}")
            print(f"    кодов в ценах предложений           {ч(ккп):>8d}"
                  f"  · строк {ч(стркп)}")
            print(f"    кодов в ценах всех ЗАКУПОЧНЫХ потоков {ч(кц):>6d}")
            cur.execute("select поток, строк, кодов from коды_не_закупочные")
            для_справки = cur.fetchall()
            if для_справки:
                print("    НЕ закупочные потоки (в «цена есть» не идут):")
                for поток, строк, кодов in для_справки:
                    print(f"        {поток:<34} строк {ч(строк):>7d}  · кодов {ч(кодов):>6d}")
                числа.update(**{
                    ("rows_own_outgoing" if поток == НАША_ИСХОДЯЩАЯ else
                     "rows_rival_retail" if поток == ЧУЖАЯ_РОЗНИЦА else "rows_other"): ч(строк)
                    for поток, строк, _ in для_справки})
            print(f"    кодов в каталоге деталей            {ч(кт):>8d}")
            print(f"    из цен опознано только по part_id   {ч(цид):>8d} строк"
                  f"  · {ч(кид)} кодов  — цена привязана к детали, а не к номеру")
            числа.update(prices_by_part_id=ч(цид), codes_by_part_id=ч(кид))

            cur.execute(СПРОС_С_ЦЕНОЙ)
            всего, с_кп, чужой, без, стр_с, стр_без = cur.fetchone()
            числа.update(with_kp=ч(с_кп), other_feed=ч(чужой), no_price=ч(без),
                         rows_with=ч(стр_с), rows_without=ч(стр_без))
            print("\nГЛАВНОЕ — КОДЫ, КОТОРЫЕ СПРАШИВАЛ ЗАКАЗЧИК (только его заявки):")
            print(f"    всего кодов                         {ч(всего):>8d}")
            print(f"    с ценой ОТ ПОСТАВЩИКА (разбор КП)   {доля(с_кп, всего)}")
            print(f"    цена есть, но из другого потока     {доля(чужой, всего)}")
            print(f"    БЕЗ ЦЕНЫ ВОВСЕ                      {доля(без, всего)}")
            print(f"    строк спроса за кодами с ценой      {ч(стр_с):>8d}")
            print(f"    строк спроса за кодами без цены     {ч(стр_без):>8d}")

            cur.execute(ЦЕНЫ_ПРОТИВ_СПРОСА)
            кодов, спраш, не_спраш, дописал, в_кат = cur.fetchone()
            числа.update(price_asked=ч(спраш), price_not_asked=ч(не_спраш),
                         price_supplier_added=ч(дописал), price_in_catalog=ч(в_кат))
            print("\nОБРАТНАЯ СТОРОНА — КОДЫ, ПО КОТОРЫМ ЦЕНА ЕСТЬ:")
            print(f"    всего кодов с ценой предложения     {ч(кодов):>8d}")
            print(f"    из них мы спрашивали                {доля(спраш, кодов)}")
            print(f"    НЕ спрашивали                       {доля(не_спраш, кодов)}")
            print(f"      из них дописал сам поставщик      {доля(дописал, кодов)}"
                  "  — прайс шире запроса")
            print(f"    есть в каталоге деталей             {доля(в_кат, кодов)}")

            cur.execute(КАТАЛОГ)
            кат, кат_кп, кат_спрос = cur.fetchone()
            числа.update(catalog_priced=ч(кат_кп), catalog_asked=ч(кат_спрос))
            print("\nКАТАЛОГ ДЕТАЛЕЙ:")
            print(f"    всего кодов каталога                {ч(кат):>8d}")
            print(f"    с ценой предложения                 {доля(кат_кп, кат)}")
            print(f"    спрашивали хоть раз                 {доля(кат_спрос, кат)}")

            cur.execute(ПРАВДОПОДОБНОСТЬ)
            в, прав, без_ц, кор, длин = cur.fetchone()
            print("\nОГОВОРКА О КАЧЕСТВЕ КОДОВ СПРОСА (это ОЦЕНКА, а не факт —")
            print("ни одна строка по ней не отбрасывается):")
            print(f"    правдоподобных (есть цифра, 4–25 знаков) {доля(прав, в)}")
            print(f"    без единой цифры                         {доля(без_ц, в)}")
            print(f"    короче четырёх знаков                    {доля(кор, в)}")
            числа.update(plausible=ч(прав), no_digit=ч(без_ц),
                         shorter_than_four=ч(кор), longer_than_25=ч(длин))
            print(f"    длиннее двадцати пяти знаков             {доля(длин, в)}")

            if os.environ.get("WRITE", "").strip():
                записать(cur, числа, os.environ.get("NOTE", "").strip() or None)
                conn.commit()
            else:
                print("\nточка истории НЕ записана — для записи WRITE=1")
        return 0
    finally:
        conn.close()


if __name__ == "__main__":
    raise SystemExit(main())

...
# печатается рядом со стороной поля: расхождение «поле говорит поставщик, а
# содержимое — наше ТКП» и есть то, ради чего классификатор смотрит в содержимое.
ПО_ПАПКАМ = """
        ...
        with conn.cursor() as cur:
            ...
            if {"doc_kind", "side"} <= колонки:
                ...
                if any(r[0] != "(ещё не разложен)" for r in строки):
                    print("\nПАПКИ ДОКУМЕНТОВ (папка по содержимому · сторона по полю · файлов · позиций):")

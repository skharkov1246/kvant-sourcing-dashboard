...
# ПАПКИ ДОКУМЕНТОВ. Требование владельца 23.09.2026: различать предложения
# поставщиков нам, запросы заказчиков нам и наши исходящие предложения. Разрез
# печатается рядом со стороной поля. Папку новых разборов ставит система (поле
# карточки), а расхождение с содержимым записано в doc_kind_why словом
# «расхождение:».
ПО_ПАПКАМ = """
select coalesce(doc_kind, '(ещё не разложен)')                     as папка,
       coalesce(side, '(сторона не указана)')                       as сторона_поля,
       count(*)::bigint                                             as файлов,
       sum(coalesce(rows_found, 0))::bigint                         as позиций
  from lib_files
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Папки — только если колонка уже в базе: замер не должен падать от
            # схемы, ушедшей вперёд базы (урок 23.09.2026, column subkind).
            if {"doc_kind", "side"} <= колонки:
                cur.execute(ПО_ПАПКАМ)
                строки = cur.fetchall()
                if any(r[0] != "(ещё не разложен)" for r in строки):
                    # С 23.09.2026 новые разборы пишут папку по данным системы
                    # (library/doc_folder.py); строки прежних разборов — папку по
                    # содержимому, пока их не перепишет переразбор.
                    print("\nПАПКИ ДОКУМЕНТОВ (папка: новые разборы — по системе, прежние — "
                          "по содержимому · сторона по полю · файлов · позиций):")
                    for папка, сторона, ф, поз in строки:
                        print(f"    {папка[:30]:30s} {сторона[:18]:18s} {ц(ф):>8d} {ц(поз):>10d}")
                else:

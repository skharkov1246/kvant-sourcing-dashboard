...
def build() -> str:
    ...
    if (sl.get("totals") or {}).get("matched_pns"):
        t = sl["totals"]
        bc = t.get("by_class") or {}
        nob = (bc.get("вилки нет") or {}).get("pns", 0)
        a("<div class='sec'><h2>Один сток-лист продавца закрывает 78 наших номеров</h2>")
        a(f"<p class='lead'>Самый дешёвый источник ориентира по цене из всех, что нам попадались. "
          f"Открытый сток-лист одного продавца ({E(t.get('seller'))}) несёт "
          f"{ru(t['listed_rows'])} строк, и <b>{ru(t['matched_pns'])} из них — наши номера</b>. "
          f"У <b>{ru(nob)}</b> из этих строк вилки у нас нет вовсе: оценку можно поставить прямо "
          f"из листа, не тратя разведку.</p>")
        a("<table class='k'>")
        for k in ("ask выше потолка", "ask внутри вилки", "ask ниже пола", "вилки нет"):
            v = bc.get(k) or {}
            if not v.get("pns"):
                continue
            a(f"<tr><td class='l'>{E(k)}</td><td class='v'>{ru(v['pns'])}</td>"
              f"<td class='dim'>{ru(v['usd_exposure'])} USD экспозиции по нашим вилкам</td></tr>")
        a("</table>")
        a("<div class='warn'><p><b>Чего эта цифра НЕ значит.</b> Сток-лист — это ask ОДНОГО "
          "оператора, а не рынок, и этот оператор уже в нашем списке групп: те же люди держат "
          "ещё три домена, включая тот, что выглядел независимым продавцом турбинного ЗИП. "
          "Внутри ask сидит неизмеренная премия за поставку, поэтому «ask выше потолка» означает "
          "«наша вилка ниже, чем просит этот продавец», а не «закупка дороже». Второй свидетель "
          "обязателен по каждой строке.</p></div>")
        a(f"<p class='dim'>Цен в наборе нет и быть не может: репозиторий публичный, а сток-лист — "
          ...
          f"(gt/data/ship_stocklist_cross.json), считает gt/tools/stocklist_cross.py. У "
          f"{ru(t.get('desc_starts_with_other_pn'))} строк описание начинается с ДРУГОГО "
          f"номера — это кросс продавца, и читать его как цену нашего номера нельзя.</p>")
        a("<div class='do'><b>Предложение</b><p>Прогнать этим же инструментом сток-листы "
          "остальных продавцов, которые их публикуют: один такой лист закрыл больше строк, чем "
          "любая отдельная разведка, и стоит одну загрузку страницы. По строкам без вилки — "
          "поставить оценку из листа с пометкой «ask одного продавца», это честнее прочерка. По "
          "строкам, где ask выше потолка, — искать второго продавца прежде, чем менять "
          "вилку.</p></div>")
        a("</div>")
    ...
    if (mb.get("totals") or {}).get("weak_usd_in_request"):
        ...
        a(f"<p class='lead'>Звено «у кого спрашивать» стоит ровно столько, сколько стоит его "
          ...
          f"{ru(t['maker_named'])}. Из них у <b>{ru(t['weak_rows_in_request'])}</b> строк нашей "
          f"заявки он выведен ИЗ КЛАССА изделия, а не из документа, и на этих строках "
          f"<b>{ru(t['weak_usd_in_request'])} USD</b> — "
          f"{100 * float(t['weak_usd_in_request']) / float(t['request_exposure']):.0f} % всей "
          f"экспозиции.</p>")
        ...
        a("<div class='do'><b>Предложение</b><p>Порядок работы по звену «исполнитель»: сначала "
          "закрыть строки с догадкой по классу — их меньше двухсот, а денег на них больше "
          "двух миллионов, и закрывается каждая одним открытым каталогом изготовителя. Строки, "

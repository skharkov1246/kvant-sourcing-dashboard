...
def build() -> str:
    ...
    if (mb.get("totals") or {}).get("weak_usd_in_request"):
        ...
        a(f"<div class='warn'><p><b>Два числа, и подменять одно другим нельзя.</b> "
          ...
          f"РАЗМЕТКЕ. Но перепроверка часть этих строк уже закрыла каталогом: "
          f"{ru(t['weak_closed_by_reverify_rows'])} строк на "
          f"{ru(t['weak_closed_by_reverify_usd'])} USD. Значит ОТКРЫТОЙ РАБОТЫ осталось "
          f"<b>{ru(t['weak_usd_in_request'])} USD</b> — "
          f"{100 * float(t['weak_usd_in_request']) / float(t['request_exposure']):.1f} % "
          f"экспозиции, а не двадцать три. Это моя собственная поправка: в отчёт ушло первое "
          f"число, и оно завышало открытую работу впятеро. Всего каталогом при перепроверке "

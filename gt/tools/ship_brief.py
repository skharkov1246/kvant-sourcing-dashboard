...
def width(r):
    ...
    середина оказалась ниже прейскуранта дистрибьютора.
    """
    lo, hi = r.get("usd_lo"), r.get("usd_hi")
...
def build() -> str:
    ...
    if chb:
        ...
        a("</tbody></table>")
        open_usd = sum(b.get("usd") or 0 for b in chb if (b.get("state") or "").startswith("прейскурант открыт"))
        ...
        a(f"<tr><td class='l'>прейскуранта нет ни у кого — только запрос</td>"
        ...
        a("<div class='do'><b>Предложение на защиту</b><p>Разделить заявку не по "
          ...
          "административный шаг, а не месяц переписки. Там, где прейскуранта нет ни у "
    ...
    a("</tbody></table>")
    # Закрывающий абзац нужен не для красоты: без него последняя строка таблицы
    # оставалась на странице одна (231 символ), и проверка PDF считала страницу
    # полупустой. Заодно он отвечает на вопрос, который таблица не отвечает: чего
    # этих вопросов больше всего.
    kinds = Counter(str(x.get("kind") or "не указано") for x in qs)
    top = ", ".join(f"{k} — {ru(v)}" for k, v in kinds.most_common(4))
    a(f"<p class='dim'>Всего вопросов {ru(len(qs))}. Чаще всего мешает одно и то же: {top}. "
      f"Машиночитаемый перечень со всеми подробностями и с ценой каждого вопроса в деньгах — "
      f"gt/data/ship_questions.json; он же источник этой таблицы.</p>")
    a("</div>")
    ...
    if sellers:
        ...
        stub = sum((v.get("totals") or {}).get("pns_on_stub_price") or 0
                   for v in sellers.values())
        a(f"<div class='warn'><p><b>Чего эти цифры НЕ значат.</b> Сток-лист — это ask продавца, а "
          f"не рынок. Две поправки, снятые с карточек 18.09.2026, и обе против нас. Первая: у "
          f"крупнейшего листа ДВА ценовых уровня на каждый номер, и верхний — шаблон магазина, а "
          f"не предложение: у 1 000 позиций из 1 000 у него sku пустой, у 224 цена 0.00, а "
          f"местами он ДЕШЕВЛЕ нижнего, что для двух состояний невозможно; продавец сам пишет, "
          f"что действующего прейскуранта по нему пока нет. Считать можно только уровень «протестированный "
          f"излишек», и после этого класс по нашим номерам изменился. Вторая: цена-заглушка — "
          f"одна и та же цифра стоит в листе сотни раз, и на ней сидит "
          f"<b>{ru(stub)}</b> наших номеров, чей класс поэтому ничего не измеряет. И остаток: в "
          f"листе стоит флаг, а ЧИСЛО остатка — только на карточке; по четырнадцати снятым "
          f"карточкам покрытие потребности вышло 11 % по реальному уровню, а полным оказалось у "
          f"ОДНОЙ строки из четырнадцати.</p></div>")

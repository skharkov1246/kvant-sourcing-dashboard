...
def measure() -> dict:
    ...
    gap_wait = gap_open = gap_dry = 0
    gap_wait_usd = gap_open_usd = gap_dry_usd = 0.0
    ...
    for r in ask:
        ...
        if channel and not price:
            ...
            if x and quote_only(x):
                ...
                gap_wait += 1
                gap_wait_usd += e
            elif x and x.get("price_hunt"):
                # РАЗДЕЛЕНО 18.09.2026. Эти строки стояли в одной строке отчёта с
                # неискаными и звали искать заново: «цена публикуется, её надо
                # найти». По ним добор цены УЖЕ прошёл и цены не нашёл — задание
                # gt/tools/rv_pricehunt.py их не печатает намеренно, поэтому
                # отчёт отправлял сорсера к инструменту, который выдаст 16 строк
                # из 175. Замер: 126 строк на 1 182 811 USD, то есть девять
                # десятых денег в этой строке отчёта. Работа по ним другая —
                # письмо продавцу, а не второй поиск.
                gap_dry += 1
                gap_dry_usd += e
            else:
                gap_open += 1
    ...
    return {
        ...
        "channel_without_price": {
            ...
            "of_them_hunted_dry": gap_dry,
            "usd_hunted_dry": round(gap_dry_usd, 2),
            "what_it_means": ("Строки, где канал назван, а цены нет. Это НЕ значит, что мы не "
                              ...
                              "лишняя строка здесь приукрасила бы работу. Строки, по которым "
                              "добор цены уже прошёл и цены не нашёл, вынесены отдельно: они не "
                              "«ещё не искали», и второй поиск по ним — списанный труд."),
        },
...
def main() -> None:
    ...
    if g["of_them_open_to_search"]:
        print(f"  {g['of_them_open_to_search']:>4} строк | {g['usd_open_to_search']:>11,.0f} "
              f"USD | НАША работа: цена публикуется, её надо найти — задание печатает "
              f"gt/tools/rv_pricehunt.py".replace(",", " "))
    else:
        # Ноль здесь — не пустая графа, а состояние фронта, и говорить его надо
        # словами: иначе строка «0 строк | 0 USD | надо найти» читается как сбой
        # счётчика, а не как «искать больше негде».
        print("     0 строк |           0 USD | НАША работа: фронт открытого поиска ЗАКРЫТ "
              "— по каждой строке этого разряда добор цены прошёл. Дальше цену дают только "
              "письмо продавцу, ответ заказчика и вход в кабинет изготовителя")
    print(f"  {g['of_them_hunted_dry']:>4} строк | {g['usd_hunted_dry']:>11,.0f} USD | добор "
          f"цены по ним УЖЕ прошёл и цены в открытом доступе не нашёл: второй поиск даст то "
          f"же — работа тут письмом продавцу, задание печатает gt/tools/quote_letters.py"
          .replace(",", " "))
    sd = m["stock_disputed"]

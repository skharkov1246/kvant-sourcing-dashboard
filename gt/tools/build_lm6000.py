#!/usr/bin/env python3
"""Сборка gt/public/lm6000.html — субпоставщики, MRO и склады GE LM6000."""
import json
...
def main():
    d = json.load(open(ROOT / "data/lm6000.json"))
    t = Counter(c["tier"] for c in d["companies"])
    tpl = (ROOT / "site/lm6000.template.html").read_text(encoding="utf-8")
    subs = {
        "__LM_JSON__": json.dumps(d, ensure_ascii=False),

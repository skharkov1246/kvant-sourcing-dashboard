...
def build() -> str:
    ...
    ex = d.get("template_files_excluded") or {}
    if ex.get("count"):
        a("<h2>Что из этого списка убрано и почему</h2>")
        a(f"<p><b>Исключено {ru(ex['count'])} файлов с направлением «заявка».</b> "
          f"{E(ex['why'])}</p>")
        a(f"<p class='dim'>{E(ex['will_self_correct'])}</p>")
        a("<p>Убранные файлы (первые по числу «цен»):</p>")
        a(f"<p class='dim'>{E('; '.join(ex.get('names') or []))}.</p>")

...
def build() -> str:
    ...
    a("<p><b>Оговорка, без которой документом пользоваться нельзя.</b> Номер стоит в файле, "
      "где ЕСТЬ строки с ценой. Стоит ли цена против этого номера — покажет только открытый "
      "файл. Это зацепка высокой пробы, а не готовое число.</p>")
...
def main() -> int:
    ...
    print(f"{pp.name}: файлов {doc.count('<h2>')}, {pp.stat().st_size / 1e6:.1f} МБ")
    return 0

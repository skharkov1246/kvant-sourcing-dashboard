#!/usr/bin/env python3
"""Удаление компаний-конкурентов (РФ, Казахстан, Беларусь, Узбекистан) из списков поставщиков.

Распоряжение владельца (17.08.2026): «русских всех убрать вообще отовсюду, это конкуренты»,
дополнено: «Казахстан, Белоруссию, узбеков тоже убирай». Эти компании конкурируют с КВАНТ
как сорсеры и сервисные поставщики ЗИП — им не пишут запросы, и они не должны
показываться сорсерам в подборе.

Что считается российской компанией (строгий критерий, чтобы не задеть соседей):
  · страна начинается с «Россия / РФ / Russia / Казахстан / Беларусь / Узбекистан»;
  · русская орг-форма в начале названия (ООО, ОАО, ЗАО, ПАО, АО, РУП, ФГУП, НПО, УК);
  · домен сайта в зоне .ru / .рф / .kz / .by / .uz.
Убираются: Россия, Казахстан, Беларусь, Узбекистан (все — конкуренты на одном рынке).
Прочие соседи (Азербайджан, Армения, Киргизия, Туркмения, Грузия, Молдова) пока остаются —
...
# Убираем не только РФ: распоряжение владельца 17.08.2026 расширено — Казахстан,
# Беларусь и Узбекистан тоже конкуренты (сорсеры и сервисные компании того же рынка).
RU_COUNTRY = re.compile(
    r"^\s*(Россия|РФ|Russia|Russian|Казахстан|Kazakh|Беларус|Белорус|Belarus|Узбекистан|Uzbek)", re.I)
CIS_DOM = re.compile(r"\.(kz|by|uz)(/|$|\b)", re.I)
RU_ORG = re.compile(r"(^|[\s(])(ООО|ОАО|ЗАО|ПАО|АО|РУП|ФГУП|НПО|УК)\s*[«\"„]?")
RU_DOM = re.compile(r"(^|[/.])([a-z0-9\-]+\.)?(ru|рф)(/|$|\b)", re.I)
# Остальные соседи (пока не названы владельцем) — не трогаем
NEIGHBOURS = re.compile(r"Азербайджан|Армени|Киргиз|Кыргыз|Туркмен|Грузи|Молдав", re.I)
...
def is_ru(name="", country="", site="", extra=""):
    ...
    if RU_ORG.search(name or "") and not re.match(r"^[A-Za-z]{3,}", (name or "").strip()):
        return True
    dom = domain(site)
    if RU_DOM.search(dom) or CIS_DOM.search(dom):
        return True
    return False
...
def main():
    ...
    STRONG_RU = re.compile(
        r"\bИНН\b|\bОГРН\b|Росси|\bРФ\b|Москв|Петербург|Челябинск|Казан[ьи]"
        r"|Казахстан|Беларус|Белорус|Узбекистан|Ташкент|Алмат|Минск", re.I)

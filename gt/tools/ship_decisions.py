...
def build() -> str:
    ...
    a("<tr><td><b>Отправить запрос цен на английском</b> (330 строк, 8 346 штук)</td>"
    ...
    a("<tr><td><b>Решение по остатку заявки вне карты каналов</b></td>"
      f"<td class='n'>645 543 USD<br>397 строк</td><td>ваше решение</td>"
      "<td>135 изготовителей, крупнейший даёт 84 055 USD: каналом это не закрывается. Либо "
      "считать по классу с оговоркой, либо выносить отдельным приложением</td></tr>")
    a("</table>")

    a("<h2>Что я делаю дальше без вас</h2><ol>")
    a("<li>Добираю строки из топ-60, про которые не знаем ничего: по деньгам это "
      "705 095 USD на девятнадцати строках.</li>")

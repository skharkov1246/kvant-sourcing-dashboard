...
Складывать их в одну долю — приукрашивать работу пересуда вдвое.
...
def measure() -> dict:
    ...
    for r in rows:
        ...
        item = {"pn": r.get("pn"), "was": was, "now": now, "usd_exposure": round(money, 2)}
        seen.append(item)
        if was == now:
            same.append(item)
            continue
...
def main() -> int:
    ...
    print(f"  вердикт не изменился:        {m['unchanged']}")

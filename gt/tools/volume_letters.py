...
def build() -> dict:
    ...
    return {
        ...
        "checked_and_empty": "Наша собственная база поставщиков проверена 18.09.2026 и адресов "
                             "по этим строкам не дала: из 144 записей с почтой ни одна не "
                             "совпала по ДОМЕНУ с каналом строки. Подбор по словам названия "
                             "давал пары заведомо ложные — фильтр Fleetguard к торговцу "
                             "программируемыми контроллерами, кольцо MWM к изготовителю "
                             "авиационной арматуры, — то есть ровно «родовой адрес вместо "
                             "адреса по детали». Повторять эту проверку не нужно: адрес "
                             "ищется у изготовителя и в его сети обслуживания.",
        "letters": letters,
...
def doc(d: dict) -> str:
    ...
    a(f"<p class='dim'><b>Что уже проверено и пусто.</b> {E(d['checked_and_empty'])}</p>")

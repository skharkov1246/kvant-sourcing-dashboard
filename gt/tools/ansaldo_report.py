...
def main():
    ...
    # --- навигатор ---
    a('<h2 class="pb">2. Как ориентироваться: производители, модели, узлы</h2>')
    for t in d["nav_how"]:
        a(f'<div class="box key">{t}</div>')
    a("<h3>Производители и их модели</h3>")
    for o in d["nav_oem"]:
        a(f'<h3>{e(o["o"])} · {e(o["co"])}</h3>')
        a(f'<div class="mut">{e(o["role"])}</div>')
        a(tbl([("Модель", 11), ("Другие имена", 15), ("Класс", 10), ("Мощность", 12), ("Камера сгорания", 24), ("Проточная часть", 18), ("Узлов", 5)],
              [[f'<b>{e(m["m"])}</b>', e(m["al"]), e(m["cl"]), e(m["p"]), e(m["cmb"]), e(m["st"]), str(len(m["ty"]))]
               for m in o["models"]]))
        for m in o["models"]:
            a(f'<div class="box"><b>{e(m["m"])}.</b> {m["tr"]}</div>')
    a('<h3 class="pb">Узлы и типы оборудования: где что искать</h3>')
    ALLM = [(mm, oo["o"]) for oo in d["nav_oem"] for mm in oo["models"]]
    a(tbl([("Узел", 20), ("Код / обозначение", 15), ("EN-термин для RFQ", 20), ("На каких моделях", 27), ("Разделы отчёта", 18)],
          [[f'<b>{e(x["t"])}</b>', e(x["c"]), e(x["en"]),
            ", ".join(mm["m"] for mm, _ in ALLM if x["t"] in mm["ty"]) or "—",
            ", ".join(x["w"])] for x in d["nav_types"]]))

    # --- компания ---
    a('<h2 class="pb">3. Компания: кто такая Ansaldo Energia</h2>')
    ...
    # --- модели ---
    a('<h2 class="pb">4. Модельный ряд</h2>')
    ...
    # --- наследие Alstom ---
    a('<h2 class="pb">5. Наследие Alstom: GT26, GT36, GT13E2 — что кому досталось</h2>')
    for t in d["ge_split"]:
    ...
    # --- горячий тракт ---
    a('<h2 class="pb">6. Горячий тракт: что покупаем и в каком количестве</h2>')
    ...
    # --- партномера ---
    a('<h2 class="pb">7. Партномера и шифровки</h2>')
    for t in d["pn_intro"]:
        a(f'<div class="box warn">{t}</div>')
    ...
    # --- керамика и горелки ---
    a('<h2 class="pb">8. Керамика камеры сгорания и горелки</h2>')
    for t in d["cer_spec"]:
        a(f'<div class="box warn">{t}</div>')
    ...
    a('<h2 class="pb">9. Ведомости из тендеров: количества и номера</h2>')
    ...
    # --- апгрейды и ремонт ---
    a('<h2 class="pb">10. Апгрейды и ремонтные технологии</h2>')
    ...
    # --- литьё и поковки ---
    a('<h2 class="pb">11. Литьё и поковки: где физически делается заготовка</h2>')
    ...
    # --- КВОУ ---
    a('<h2 class="pb">12. КВОУ и фильтроэлементы</h2>')
    ...
    # --- парк в РФ ---
    a('<h2 class="pb">13. Парк в России и СНГ — наш рынок спроса</h2>')
    ...
    # --- наш след ---
    a("<h2>14. Наш собственный след: сделки и таможня</h2>")
    ...
    # --- субпоставщики ---
    a('<h2 class="pb">15. Субпоставщики Ansaldo — кто физически делает детали</h2>')
    for grp in d["subs"]:
    ...
    # --- сеть и дилеры ---
    a('<h2 class="pb">16. Сеть Ansaldo: дочки, сервис, лицензиаты</h2>')
    ...
    a("<h2>17. Независимый аftermarket — кто продаст без OEM</h2>")
    ...
    # --- сервис в РФ ---
    a('<h2 class="pb">18. Кто в России реально работает по этим машинам</h2>')
    ...
    # --- генераторы ---
    a('<h2 class="pb">19. Генераторы Ansaldo</h2>')
    ...
    # --- АСУ ТП ---
    a('<h2 class="pb">20. АСУ ТП, КИП и топливная арматура</h2>')
    ...
    a('<h2 class="pb">21. Право и ограничения по позиции 8411</h2>')
    ...
    # --- логистика и таможня ---
    a('<h2 class="pb">22. Логистика и таможня</h2>')
    ...
    # --- как писать RFQ ---
    a('<h2 class="pb">23. Как писать RFQ по этим машинам</h2>')

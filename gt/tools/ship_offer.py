    ...
    for f in doc.get("files", []):
        ...
        for p in f.get("prices", []):
            ...
            if not price_graded(p):
                continue
            # ПУСТАЯ ВАЛЮТА НЕ ДОЛЛАР. Выгрузки, снятые после 18.09.2026, несут
            # поле currency_source и умеют сказать «валюта не установлена» — в
            # таких строках подстановка доллара была бы молчаливой ошибкой: замер
            # по выгрузке ЛУКОЙЛ дал 14 цен по колонке со знаком ¥ и пустой
            # валютой, то есть завышение в 6,7 раза при юане и в 154 при иене.
            # У прежних выгрузок поля нет, и для них оставлен прежний разбор:
            # иначе исправление молча обнулило бы уже снятые данные.
            if "currency_source" in p and not str(p.get("currency") or "").strip():
                continue

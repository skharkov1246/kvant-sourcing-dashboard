...
def main() -> None:
    ...
    # нашим, а не её».
    print("ОТКАЗ СТРАНИЦЫ СНАЧАЛА ПРОВЕРЬ СПОСОБОМ ЗАПРОСА. Три замера 18.09.2026: "
          "перечень на 437 позиций три прохода считался снятым по «503 трижды» — 503 "
          "отдаётся только по HTTPS, по чистому http тот же адрес отдаёт 200; сайт "
          "изготовителя отдаёт 403 по всем схемам и поддоменам, а через текстовый прокси "
          "читается целиком; у торговца, наоборот, https падает на самоподписанном "
          "сертификате, а http отдаёт 200. Прежде чем записать страницу мёртвой, смени "
          "схему (http против https), инструмент (curl, чтение страницы, текстовый "
          "прокси), поддомен (www против без) и кодировку; в разборе напиши, что именно "
          "проверено.\n")
    for i, (e, r, x) in enumerate(take, start=a.skip + 1):
        print(brief(i, e, r, x))
        print()

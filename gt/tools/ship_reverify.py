...
VERDICTS = ["ЗАНИЖЕНА", "ЗАВЫШЕНА", "ВЕРНА", "ДУБЛЬ", "НЕЧЕМ ПРОВЕРИТЬ"]
V_WHAT = {
...
def enrich(rows: list) -> tuple[list, dict]:
    ...
    for r in rows:
        ...
        r["qty"] = r.get("qty") or d.get("qty")
        r["lo"] = r.get("lo") if r.get("lo") not in (None, "") else d.get("usd_lo")
        r["hi"] = r.get("hi") if r.get("hi") not in (None, "") else d.get("usd_hi")
        r["conf"] = r.get("conf") or d.get("conf")
        if not r.get("expo"):

...
SOURCES = [
    {
        ...
        # ТРЕТЬЕ СВИДЕТЕЛЬСТВО В ПОЛЬЗУ ОБЩЕГО ПРЕДКА, 18.09.2026. Те же номера
        # с теми же количествами печатает ВТОРОЙ, не связанный с первым продавец
        # — Xiamen Jiyang (страница по M16, кодировка gb18030, коды ТН ВЭД
        # 7318.16.99 и 7318.22.00). Два независимых торговца не могли получить
        # наш запрос в одинаковом виде и с одинаковым порядком строк; зато оба
        # могли перепечатать одну и ту же ведомость рекомендованных запчастей
        # изготовителя. Это же объясняет, почему количества совпадают с нашими
        # не везде: у 266 строк из 430 совпадают, у 164 отличаются — наша заявка
        # покрывает другое число машин. Обвинения по-прежнему нет: происхождение
        # открытыми данными не устанавливается, и запись здесь — измерение, а не
        # вывод о чьей-то вине.
        "second_independent_seller": "Xiamen Jiyang печатает те же номера с теми же "
                                     "количествами по своим позициям — второй продавец, "
                                     "не связанный с первым. Совпадение количеств с нашей "
                                     "заявкой: 266 строк из 430, у 164 количества другие.",
    },
    {
...
def build() -> dict:
    ...
    return {
        ...
        # ИСПРАВЛЕНО 18.09.2026. Прежняя редакция утверждала, что ОБА домена
        # внесены в список круговых источников приёмника. Это неверно: в списке
        # только ausenist.com, и это сделано осознанно. Он печатает наши строки
        # ПО-РУССКИ вместе с нашими позиционными пометками — то есть отражает наш
        # текст. Перечень второго домена отражает не нас, а ведомость
        # изготовителя: те же номера с теми же количествами печатает второй, не
        # связанный с первым продавец, а количества совпадают с нашими у 266
        # строк из 430. Поэтому он годится подтвердить, что НОМЕР существует в
        # ведомости изготовителя, и не годится подтверждать ни спрос, ни
        # количество, ни цену. Проверено на наборе: из 275 строк с вердиктом по
        # цене ни одна цену с этих доменов не берёт; две ссылаются на второй
        # домен ровно и только для опознания номера, и говорят это прямо.
        "why_it_matters_twice": "Первое — конфиденциальность запроса. Второе — такая страница "
                                "ломает проверку источника: выглядит независимым "
                                "подтверждением, а может быть отражением. Различать надо, "
                                "ЧЕГО отражением: ausenist.com печатает наши строки "
                                "по-русски с нашими пометками, он в списке круговых "
                                "источников приёмника; перечень gas-turbine-parts.com "
                                "отражает ведомость изготовителя, поэтому он подтверждает "
                                "существование номера, но не спрос, не количество и не цену "
                                "— в круговые он намеренно НЕ внесён.",
        "sources": srcs,

...
def host(url: str) -> str:
    """Хост страницы. Снимок архива читается как САМА страница.

    Адрес запчастей Solar прочитан в снимке Internet Archive: живой сайт
    отвечает 403 на кромке Akamai из этой среды. Снимок страницы изготовителя —
    это его страница, а не посторонний источник, поэтому обёртка архива
    снимается до сверки домена. Иначе проверка отбила бы настоящий заводской
    адрес, подтверждённый вдобавок на русской версии той же страницы.
    """
    u = str(url or "")

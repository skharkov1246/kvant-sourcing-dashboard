...
ЦБ на дату создания сделки, модуль base/fx.py. Если хотя бы одна сторона валюту не
объявила, пара сохраняется, но помечается пустой валютой: сравнивать по ней
цены нельзя, потому что неизвестно, в чём они выражены.

ЧЕГО ПО ПАРАМ ВСЁ ЕЩЁ НЕЛЬЗЯ СЧИТАТЬ. Наценку. Валюта берётся по документу в
целом, а в наших ТКП сплошь оговорка «цена в евро, оплата в рублях» — документ
помечается евро, хотя таблица в рублях. Пары остаются готовым материалом
«одна и та же позиция у поставщика и у нас», но отношение цен без ручной сверки
валюты документа — не маржа.
        ...
        for pn_s, nm_s, pr_s, qty_s, tok_s, cur_s in sup[did]:
            ...
            if best and (best_kind == "pn" or best_sim >= MIN_SIM):
                ...
                if cur_s and cur_o and cur_s != cur_o:
                    ...
                    # валюта у пары проставляется, только когда её объявили ОБЕ
                    # стороны: догадка «раз у поставщика рубли, то и у нас рубли»
                    # давала пары с постоянным отношением ×0,29 на всю сделку —
                    # это перекос масштаба, а не наценка
                    cur = cur_s if (cur_s and cur_o and cur_s == cur_o) else ""
                used_our.add(best_i)
                stats[best_kind] += 1

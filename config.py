...
# Список задаётся переменной SERVICE_ACCOUNT_IDS (идентификаторы через запятую);
# кандидатов на добавление дашборд показывает сам — см. metrics.origin.
SERVICE_ACCOUNT_IDS = {
    u.strip() for u in (os.getenv("SERVICE_ACCOUNT_IDS") or "").split(",") if u.strip()
}
